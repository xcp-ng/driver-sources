...
int bnxt_hwrm_udcc_cfg(struct bnxt *bp, u32 enables, u8 mode, u8 padcnt)
{
	...
	if (enables & UDCC_CFG_REQ_ENABLES_PROBE_CFG)
		req->probe_cfg = padcnt;
...
int bnxt_alloc_udcc_info(struct bnxt *bp)
{
	...
	bp->udcc_info = udcc;
	netdev_info(bp->dev, "%s UDCC with DCQCN %s and max %d sessions per adapter\n",
		    udcc->session_type ? "Per-QP" : "Per-DestIP",
		    BNXT_UDCC_DCQCN_EN(bp) ? "enabled" : "disabled",
		    udcc->max_sessions);
