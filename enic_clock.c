...
#if ENIC_HAVE_ADJUST_BY_SCALED_PPM
static int enic_ptp_adjfine(struct ptp_clock_info *ptp_info, long scaled_ppm)
{
	struct enic_hwtstamp *tstamp;
	struct enic *enic;
	u64 mult;
	...
	tstamp = container_of(ptp_info, struct enic_hwtstamp, ptp_info);
	enic = container_of(tstamp, struct enic, tstamp);
	mult = adjust_by_scaled_ppm(tstamp->nominal_c_mult, scaled_ppm);
	write_seqlock_bh(&tstamp->lock);
	timecounter_read(&tstamp->clock);
	tstamp->cc.mult = mult;
	write_sequnlock_bh(&tstamp->lock);
...
#if ENIC_HAVE_ADJUST_BY_SCALED_PPM
	tstamp->ptp_info.adjfine = enic_ptp_adjfine;
#else
...
#endif
	snprintf(tstamp->ptp_info.name, PTP_CLOCK_NAME_LEN, "%s-ptp",
		 enic->netdev->name);
