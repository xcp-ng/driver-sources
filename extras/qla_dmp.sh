...
mpidfile=mpi_fw_dump_${1}_`eval date +%Y%m%d_%H%M%S`.txt
...
cat ${qfwd} > ${mpidfile}
echo 8 > ${qfwd}
if ! test -s "${mpidfile}" ; then
	echo "No MPI dump available at host ${1}!!!"
	rm ${mpidfile}
else
	gzip ${mpidfile}
	echo "MPI dump saved in file ${mpidfile}.gz."
fi
