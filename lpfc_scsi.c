...
static void
lpfc_external_dif_cmpl(struct lpfc_hba *phba, struct lpfc_iocbq *cmdiocb,
		       struct lpfc_iocbq *rspiocb);
extern int
lpfc_issue_scsi_inquiry(struct lpfc_vport *, struct lpfc_nodelist *, u64);
static void
...
lpfc_scsi_get_reftag(struct scsi_cmnd *sc)
{
	...
	/* We need a valid sector size for the LBA to be valid */
	if (!sc->device->sector_size)
		return T10_PI_REF_ESCAPE;

#if KERNEL_VERSION(4, 19, 0) > LINUX_VERSION_CODE
	/* truncate the LBA to 4 bytes, reftag size */
	lba = (u32)(scsi_get_lba(sc) & 0xffffffffu);
#else
#ifndef BUILD_USE_ML_PI
	...
	if (!offline) {
		for (i = 1; i <= phba->sli.last_iotag; i++) {
			...
			if (iocbq->sli4_xritag != xri)
				continue;
			psb = container_of(iocbq, struct lpfc_io_buf,
					   cur_iocbq);
			psb->flags &= ~LPFC_SBUF_XBUSY;
			spin_unlock_irqrestore(&phba->hbalock, iflag);
		...
		struct lpfc_io_buf *lpfc_cmd)
{
	...
	do {
		...
		bf_set(lpfc_sli4_sge_dif_re, diseed, checking);
#else
		if (!sc->prot_flags || sc->prot_flags & SCSI_PROT_GUARD_CHECK) {
			bf_set(lpfc_sli4_sge_dif_ce, diseed, checking);
		...
		if (!sc->prot_flags || sc->prot_flags & SCSI_PROT_REF_CHECK)
			bf_set(lpfc_sli4_sge_dif_re, diseed, checking);
		else
			bf_set(lpfc_sli4_sge_dif_re, diseed, 0);
#endif
...
lpfc_calc_bg_err(struct lpfc_hba *phba, struct lpfc_io_buf *lpfc_cmd)
{
	...
	if (sgpe && protsegcnt) {
		...
		if (start_ref_tag == T10_PI_REF_ESCAPE)
			goto out;
...
out:
	if (err_type == BGS_GUARD_ERR_MASK) {
		...
		lpfc_set_driver_byte(cmd, DRIVER_SENSE);
...
lpfc_scsi_prep_dma_buf_s4(struct lpfc_hba *phba, struct lpfc_io_buf *lpfc_cmd)
{
	...
	if (!test_bit(HBA_FCOE_MODE, &phba->hba_flag) &&
		...
		/* Word 4 */
		wqe->fcp_iwrite.total_xfer_len = scsi_bufflen(scsi_cmnd);
	}
	...
	if (resp_info & RSP_LEN_VALID) {
		...
		if (rsplen != 0 && rsplen != 4 && rsplen != 8) {
			lpfc_printf_vlog(vport, KERN_ERR, LOG_TRACE_EVENT,
					 ...
					 "x%x\n", cmnd->device->id,
					 (u64)cmnd->device->lun, cmnd->cmnd[0],
					 rsplen);
...
out:
	rdata = lpfc_cmd->rdata;
	pnode = rdata->pnode;
	if (pnode && test_bit(NLP_EXTERNAL_DIF, &pnode->nlp_flag)) {
			 ...
			 struct lpfc_iocbq *pwqeOut)
{
	...
	if (unlikely(lpfc_cmd->status)) {
		...
		lpfc_printf_vlog(vport, KERN_WARNING, logit,
				 ...
				 cmd->device ? (u64)cmd->device->lun : 0xffff,
	...
	case CQE_STATUS_DI_ERROR:
		...
		lpfc_printf_vlog(vport, KERN_WARNING, LOG_FCP | LOG_BG,
				 ...
				 wcqe->total_data_placed);
		if (lpfc_scsi_get_prot_op(cmd) != SCSI_PROT_NORMAL) {
...
lpfc_scsi_cmd_iocb_cmpl(struct lpfc_hba *phba, struct lpfc_iocbq *pIocbIn,
			struct lpfc_iocbq *pIocbOut)
{
	struct lpfc_io_buf *lpfc_cmd = pIocbIn->io_buf;
				      ...
				      uint8_t tmo)
{
	...
	if (scsi_sg_count(scsi_cmnd)) {
		if (datadir == DMA_TO_DEVICE) {
			...
			if (pnode->first_burst && test_bit(NLP_FIRSTBURST,
							   &pnode->nlp_flag)) {
				u32 xrdy_len;
				      ...
				      uint8_t tmo)
{
	...
	/* dont overwrite if wqe_cmpl already set */
	if (!pwqeq->cmd_cmpl)
		    ...
		    struct lpfc_nodelist *pnode)
{
	...
	if (vport->phba->cfg_external_dif &&
	    !test_bit(NLP_SKIP_EXT_DIF, &pnode->nlp_flag)) {
	...
	if (scan_io) {
		 ...
		 */
		if (scsi_cmd_to_rq(scsi_cmnd)->timeout > 0) {
			tmo  = (uint8_t)
				(scsi_cmd_to_rq(scsi_cmnd)->timeout / 1000) + 2;
			if (tmo == 0  || tmo > 60)
				tmo = 30;
		} else {
			tmo = 30;
		...
		tmo = lpfc_cmd->timeout;
	}

	lpfc_scsi_prep_cmnd_buf(vport, lpfc_cmd, tmo);
 ...
 * @vport: The virtual port for which this call being executed.
 * @lpfc_cmd: lpfc scsi command object pointer.
 * @cdb_ptr: pointer to SCSI command
 *
		      ...
		      uint8_t *cdb_ptr)
{
	...
	/* We are only interested in EVPD=0 && page 0 with a minimum
	 * buffer len.
	 */
	alloc_len =  scsi_bufflen(lpfc_cmd->pCmd);
	if ((cdb_ptr[1] & 0x1) || cdb_ptr[2] || alloc_len < LPFC_INQ_FDIF_SZ) {
		lpfc_printf_vlog(vport, KERN_INFO, LOG_EDIF,
				 "0916 EDSM ignore cmd x%x data: x%x x%x x%x\n",
				 cdb_ptr[0], cdb_ptr[1], cdb_ptr[2], alloc_len);
		return;
	}

	/* Setup to intercept INQUIRY completion */
	piocbq->cmd_cmpl = lpfc_external_dif_cmpl;
}
...
/**
 * lpfc_edsm_validate_target - Check a remote NPort for External DIF support
 * @vport: The virtual port for which this call being executed.
...
lpfc_issue_scsi_inquiry(struct lpfc_vport *vport,
			struct lpfc_nodelist *ndlp, u64 lunId)
{
	struct lpfc_hba *phba = vport->phba;
	struct lpfc_io_buf *lpfc_cmd = NULL;
	struct sli4_sge *sgl;
	...
	/*
	 * Setup the lpfc_cmd
	 * set LPFC_IO_DRVR_INIT in cmd_flag field to indicate driver use of
	 * iocb from scsi buf list for a driver initiated SCSI IO.
	 */
	lpfc_cmd->cur_iocbq.cmd_flag |= LPFC_IO_DRVR_INIT;
	lpfc_cmd->cur_iocbq.cmd_cmpl = lpfc_external_dif_cmpl;
	lpfc_cmd->cur_iocbq.io_buf = lpfc_cmd;
	lpfc_cmd->lun_id = lunId;
	...
	wqe = &lpfc_cmd->cur_iocbq.wqe;
	memcpy(&wqe->words[7], &lpfc_iread_cmd_template.words[7],
	       sizeof(uint32_t) * 5);
	bf_set(wqe_tmo, &wqe->fcp_iread.wqe_com, lpfc_cmd->timeout);

	/* Word 3 */
	bf_set(payload_offset_len, &wqe->fcp_icmd,
	       sizeof(struct fcp_cmnd) + sizeof(struct fcp_rsp));
	bf_set(cmd_buff_len, &wqe->fcp_iread, 0);

	/* Word 4 */
	wqe->fcp_iread.total_xfer_len = be32_to_cpu(lpfc_cmd->fcp_cmnd->fcpDl);
	...
	/* Word 8 */
	wqe->generic.wqe_com.abort_tag = lpfc_cmd->cur_iocbq.iotag;

	/* Word 9 */
	bf_set(wqe_reqtag, &wqe->generic.wqe_com, lpfc_cmd->cur_iocbq.iotag);

	err = lpfc_sli_issue_fcp_io(phba, LPFC_FCP_RING, &lpfc_cmd->cur_iocbq,
				    SLI_IOCB_RET_IOCB);

	lpfc_printf_vlog(vport, KERN_INFO, LOG_EDIF,
			 "0919 External DIF: Issue INQUIRY (%d/%lld) ret %d "
			 "flag x%x iotag x%x xri x%x\n",
			 ndlp->nlp_sid, lunId, err,
			 lpfc_cmd->cur_iocbq.cmd_flag,
			 lpfc_cmd->cur_iocbq.iotag,
			 lpfc_cmd->cur_iocbq.sli4_xritag);
 ...
 * @vport: The virtual port for which this call being executed.
 * @ndlp: Pointer to lpfc_nodelist instance.
 * @lunId: lunId for retry
 * @status: status from previous request
 *
	...
	if (dp) {
		...
		if (dp->inq_retry < LPFC_EDSM_MAX_RETRY) {
			...
			lpfc_printf_vlog(vport, KERN_INFO, LOG_EDIF,
					 ...
					 "exceeded NPort %06x: (%d/%lld) err x%x",
					 ndlp->nlp_DID, ndlp->nlp_sid,
					 lunId, status);
			rc = ENXIO;
		}
 ...
 * @phba: The Hba for which this call is being executed.
 * @cmdiocb: The command iocb for the scsi cmnd.
 * @rspiocb: The response iocb for the scsi cmnd.
 *
 * This routine processes the External DIF SCSi command cmpl before calling the
 * normal SCSI cmpl routine (lpfc_fcp_io_cmd_wqe_cmpl). There are 2 types of
...
lpfc_external_dif_cmpl(struct lpfc_hba *phba, struct lpfc_iocbq *cmdiocb,
		       struct lpfc_iocbq *rspiocb)
{
	struct lpfc_io_buf *lpfc_cmd = cmdiocb->io_buf;
	...
	u32 status = get_job_ulpstatus(phba, rspiocb);
	...
	case INQUIRY:
		...
		if (cmnd) {
			...
			while (sgde && len) {
				...
				data_inq = kmap_atomic(sg_page(sgde))
				    + sgde->offset;
#endif
				memcpy(ptr, data_inq, sg_dma_len(sgde));
				len -= sg_dma_len(sgde);
				ptr += sg_dma_len(sgde);
#if (KERNEL_MAJOR < 3) || ((KERNEL_MAJOR == 3) && (KERNEL_MINOR < 4))
		...
		/* Make sure the INQUIRY payload has our vendor specific info
		 * included. The target rport may have underflowed the response
		 * bytes.
		 */
		if (status == IOSTAT_FCP_RSP_ERROR)
		...
		/* Received standard Inquiry data means at least
		 * 36 bytes, EVPD is unset, and Page Code is 0.
		 */
		if (cnt >= LPFC_INQ_STANDARD_LEN &&
		    !(fcpcmd->fcpCdb[1] & 0x1) && !fcpcmd->fcpCdb[2]) {
			/* Jump to T10 Vendor Identification field */
			data_inq += LPFC_INQ_VID_OFFSET;
			if (strncmp(data_inq, LPFC_INQ_FDIF_VENDOR,
				    strlen(LPFC_INQ_FDIF_VENDOR))) {
				clear_bit(NLP_EXTERNAL_DIF, &ndlp->nlp_flag);
				set_bit(NLP_SKIP_EXT_DIF, &ndlp->nlp_flag);
				break;
			}
		}

		if (cnt < LPFC_INQ_FDIF_SZ) {
			/* Setting IGNORE state not required for new devices. */
			spin_lock_irqsave(&vport->external_dif_lock, flags);
		...
		if (dp) {
			dp->inq_cnt++;
			dp->inq_retry = 0;
			set_bit(NLP_EXTERNAL_DIF, &ndlp->nlp_flag);
...
out:
	kfree(buf);
	if (lpfc_cmd->cur_iocbq.cmd_flag & LPFC_IO_DRVR_INIT) {
		if (ndlp == NULL) {
			ndlp = (void *)lpfc_cmd->rdata;
			lpfc_cmd->rdata = NULL;
		}
	...
	lpfc_fcp_io_cmd_wqe_cmpl(phba, cmdiocb, rspiocb);
}
 ...
 * @lpfc_cmd: Pointer to lpfc_io_buf data structure.
 * @ndlp: Pointer to lpfc_nodelist instance
 * @cdb_ptr:  SCSI command used to format the driver io and CDB
 *
...
lpfc_queuecommand(struct Scsi_Host *shost, struct scsi_cmnd *cmnd)
{
	...
	if (cmnd->cmnd[0] == 0 || cmnd->cmnd[0] == 0x12 ||
		...
		lpfc_printf_vlog(vport, KERN_INFO, LOG_FCP,
				 ...
				 (uint32_t)
				 (scsi_cmd_to_rq(cmnd)->timeout / 1000),
				 get_job_cmnd(phba, cur_iocbq));
	}
...
{
	struct lpfc_io_buf *lpfc_cmd = p_io_in->io_buf;
	struct lpfc_wcqe_complete *wcqe = &p_io_out->wcqe_cmpl;
	u32 resp_info = lpfc_cmd->fcp_rsp->rspStatus2;
	struct sli4_sge *sgl;
	struct lpfc_nodelist *ndlp = lpfc_cmd->ndlp;
	u32 status = get_job_ulpstatus(phba, p_io_out);
	u32 result = get_job_word4(phba, p_io_out) & IOERR_PARAM_MASK;
	u32 fb = 0;
	int ret_val;
	...
	lpfc_printf_vlog(phba->pport, KERN_INFO,
			 ...
			 lpfc_cmd->cur_iocbq.cmd_flag, ret_val);
 ...
 * @cmd_req: SCSI command used to format the driver io and CDB
		   ...
		   struct lpfc_scsi_io_req *cmd_req, u32 data_len)
{
	...
	lpfc_cmd->cur_iocbq.cmd_cmpl = cmd_req->cmd_cmpl;
	...
	case MODE_SENSE:
		...
		lpfc_cmd->fcp_cmnd->fcpCntl3 = READ_DATA;
		lpfc_cmd->cur_iocbq.rsp_dmabuf = NULL;
		memcpy(&wqe->words[7], &lpfc_iread_cmd_template.words[7],
		       sizeof(uint32_t) * 5);
		/* Word 3 */
		bf_set(payload_offset_len, &wqe->fcp_icmd,
 ...
 * @vport: The virtual port for which this call being executed.
 * @ndlp: Pointer to lpfc_nodelist instance.
 * @cmd_req: pointer to a iocb structure with the command, device ids, and the
 *           associated completion routine.
 *
...
lpfc_abort_handler(struct scsi_cmnd *cmnd)
{
	...
	if (!lpfc_cmd->pCmd) {
		lpfc_printf_vlog(vport, KERN_WARNING, LOG_FCP,
			 ...
			 "x%x ID %d LUN %llu\n",
			 SUCCESS, cmnd->device->id, (u64)cmnd->device->lun);
		goto out_unlock_hba;
	}
...
wait_for_cmpl:
	...
	if (lpfc_cmd->pCmd == cmnd) {
		...
		lpfc_printf_vlog(vport, KERN_ERR, LOG_TRACE_EVENT,
				 ...
				 cmnd->device->id, (u64)cmnd->device->lun);
	}
...
lpfc_bus_reset_handler(struct scsi_cmnd *cmnd)
{
	...
	for (i = 0; i < LPFC_MAX_TARGET; i++) {
		...
		status = lpfc_send_taskmgmt(vport, ndlp->rport, i, 0,
					    FCP_TARGET_RESET);
...
#endif
#if defined(DID_TRANSPORT_MARGINAL)
