...
#define GET_CONFIG(m) \
	...
	if ((c->flags & (VFCF_FC_INITIATOR | VFCF_FC_TARGET |
		VFCF_FC_NVME_INITIATOR | VFCF_FC_NVME_TARGET)) == 0)
	{
		printk(KERN_INFO "vNIC role not defined.Defaulting to FC Initiator\n");
	...
	/*for older fw GET_CONFIG will not return anything*/
	...
	printk(KERN_INFO
		     "fNIC node wwn %llx port wwn %llx\n",
		     c->node_wwn, c->port_wwn);
	printk(KERN_INFO
		     "fNIC ed_tov %d ra_tov %d\n",
		     c->ed_tov, c->ra_tov);
	printk(KERN_INFO
		     "fNIC mtu %d intr timer %d\n",
		     c->maxdatafieldsize, c->intr_timer);
	printk(KERN_INFO
		     "fNIC flags 0x%x luns per tgt %d\n",
		     c->flags, c->luns_per_tgt);
	printk(KERN_INFO
		     "fNIC flogi_retries %d flogi timeout %d\n",
		     c->flogi_retries, c->flogi_timeout);
	printk(KERN_INFO
		     "fNIC plogi retries %d plogi timeout %d\n",
		     c->plogi_retries, c->plogi_timeout);
	printk(KERN_INFO
		     "fNIC io throttle count %d link dn timeout %d\n",
		     c->io_throttle_count, c->link_down_timeout);
	printk(KERN_INFO
		     "fNIC port dn io retries %d port dn timeout %d\n",
...
void fnic_get_res_counts(struct fnic *fnic)
{
	...
	fnic->intr_count = vnic_dev_get_res_count(fnic->vdev,
		RES_TYPE_INTR_CTRL);
	printk(KERN_INFO "vNIC resources intr_count=%d from fw\n", fnic->intr_count);
}
...
int fnic_alloc_vnic_resources(struct fnic *fnic)
{
	...
	printk(KERN_INFO "vNIC interrupt mode: %s\n",
		     intr_mode == VNIC_DEV_INTR_MODE_INTX ? "legacy PCI INTx" :
		     intr_mode == VNIC_DEV_INTR_MODE_MSI ? "MSI" :
		     intr_mode == VNIC_DEV_INTR_MODE_MSIX ?
		     "MSI-X" : "unknown");

	printk(KERN_INFO "vNIC resources avail: "
		     "wq %d cp_wq %d raw_wq %d rq %d cq %d intr %d cpy-wq desc count %d\n",
		     fnic->wq_count, fnic->wq_copy_count, fnic->raw_wq_count,
		     fnic->rq_count, fnic->cq_count, fnic->intr_count, fnic->config.wq_copy_desc_count);
