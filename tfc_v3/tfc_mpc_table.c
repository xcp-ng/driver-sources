...
static int tfc_corefile_bs_get(struct bnxt *bp, u8 tsid, u8 *data, u64 *size)
{
	struct tfc_bs_master_record_t *master_record = NULL;
	struct tfc_bs_table_record_t *table_record = NULL;
	struct tfc_ts_mem_cfg *mem_cfg;
	u64 table_compressed_size;
	u16 num_tables = 0;
	u64 table_size = 0;
	u64 total_size = 0;
	bool is_bs_owner;
	...
	for (dir = 0; dir <= 1; dir++) {
		for (type = CFA_REGION_TYPE_LKUP; type < CFA_REGION_TYPE_MAX; type++) {
			...
			table_size = (u64)mem_cfg->pg_tbl[mem_cfg->num_lvl - 1].pg_count *
				(u64)mem_cfg->pg_tbl[mem_cfg->num_lvl - 1].pg_size;
			...
			netdev_dbg(bp->dev,
				   "tsid:%d dir:%d type:%d size_in:%llu size_out:%llu\n",
	...
	*size = total_size + sizeof(struct tfc_bs_master_record_t);
	...
	netdev_dbg(bp->dev, "Total table size: %llu\n", *size);

	return 0;
}

int tfc_corefile_bs_size(struct bnxt *bp, u64 *bytes)
{
	bool ts_valid;
	u8 tsid;
	int rc;
...
int tfc_corefile_bs_save(struct bnxt *bp, u8 *data, u64 *bytes)
{
	...
	*bytes -= sizeof(struct bnxt_coredump_segment_hdr);
	...
	memcpy(data, &seg_hdr, sizeof(seg_hdr));
	data += sizeof(struct bnxt_coredump_segment_hdr);

	rc = bnxt_ulp_cntxt_tsid_get(bp->ulp_ctx, &tsid);
	if (!rc) {
		/* ULP is enabled */
		rc = tfc_corefile_bs_get(bp, tsid, data, bytes);
	} else {
		/* No ULP */
