...
#define FNIC_FC_CTL_DEV_NAME "fnic-fc-mgmt"
 ...
 *                         or fnic_fc_tracing_enabled to verify tracing
 *                         is enabled/disabled
 * Description:
 * This routine passes the value of fnic_tracing_enabled or
 * fnic_fc_tracing_enabled variable to user buffer @ubuf
 */
static int fnic_trace_enable_get(char __user *ubuf, int fnic_trace)
{
	if (fnic_trace == 1)
        	return copy_to_user(ubuf, &fnic_tracing_enabled, sizeof (int));
        else
                return copy_to_user(ubuf, &fnic_fc_tracing_enabled,
				sizeof (int));
}
 ...
 *
 * Description:
 * This routine sets value of fnic_tracing_enabled or fnic_fc_tracing_enabled
 * to value specified by userworld program @ubuf
 */
static int fnic_trace_enable_set(const char __user *ubuf, int fnic_trace)
{
	unsigned int val;
...
/*
 * fc_trace_clear_get - Routine to get the value of fnic_fc_trace_cleared
 *                         to verify tracing is enabled/disabled
 * Description:
 * This routine passes the value of fnic_fc_trace_cleared variable to user
 * buffer @ubuf
 */
static int fnic_trace_clear_get(char __user *ubuf)
{
        return copy_to_user(ubuf, &fnic_fc_trace_cleared, sizeof (int));
}

/*
 * fc_trace_clear_set - Routine to enable/disable fnic tracing from
 * userworld
 *
 * Description:
 * This routine sets value of fnic_fc_trace_cleared to value specified by
 * userworld program @ubuf
 */
static int fnic_trace_clear_set(const char __user *ubuf)
{
	unsigned int val;
 ...
 */
static int fnic_trace_get_max_size(char __user *arg, int fnic_trace)
{
        unsigned long trace_size;
        if (fnic_trace == 1)
                trace_size = trace_max_pages * PAGE_SIZE;
        else
                trace_size = fnic_fc_trace_max_pages * PAGE_SIZE;
                      ...
                      unsigned long arg)
{
	...
	case FNIC_SET_TRACE_ENABLE:
		fnic_trace = 1;
            	rc = fnic_trace_enable_set(_arg, fnic_trace);
		break;
	case FNIC_GET_TRACE_ENABLE:
		fnic_trace = 1;
            	rc = fnic_trace_enable_get(_arg, fnic_trace);
		break;
	case FNIC_GET_TRACE_BUF_SIZE:
            	fnic_trace = 1;
            	rc = fnic_trace_get_max_size(_arg, fnic_trace);
		break;
    	case FNIC_FC_SET_TRACE_ENABLE:
            	fnic_trace = 0;
            	rc = fnic_trace_enable_set(_arg, fnic_trace);
            	break;
    	case FNIC_FC_GET_TRACE_ENABLE:
            	fnic_trace = 0;
            	rc = fnic_trace_enable_get(_arg, fnic_trace);
            	break;
    	case FNIC_FC_SET_TRACE_CLEAR:
        	rc = fnic_trace_clear_set(_arg);
            	break;
    	case FNIC_FC_GET_TRACE_CLEAR:
            	rc = fnic_trace_clear_get(_arg);
            	break;
    	case FNIC_FC_GET_TRACE_BUF_SIZE:
            	fnic_trace = 0;
            	rc = fnic_trace_get_max_size(_arg, fnic_trace);
            	break;
	case FNIC_GET_TRACE_DATA:
            	fnic_trace = 1;
            	rdata_flag = 0;
            	rc = fnic_trace_get_data(_arg, fnic_trace, rdata_flag);
		break;
    	case FNIC_FC_GET_TRACE_RDATA:
            	rdata_flag = 1;
            	fnic_trace = 0;
            	rc = fnic_trace_get_data(_arg, fnic_trace, rdata_flag);
            	break;
    	case FNIC_FC_GET_TRACE_DATA:
            	rdata_flag = 0;
            	fnic_trace = 0;
            	rc = fnic_trace_get_data(_arg, fnic_trace, rdata_flag);
            	break;
...
{
	fnic_unreg_char_dev();
	printk(KERN_INFO "Removed ioctl char device");
	return 0;
}

int fnic_fc_ioctl_init()
{
	int rc = 0;

	fnic_fc_major = register_chrdev(0,
				FNIC_FC_CTL_DEV_NAME, &fnic_file_ops);
	if (fnic_fc_major < 0) {
		printk(KERN_DEBUG "Failed to register the control device\n");
		rc = -ENODEV;
		goto err_reg_char_dev;
	}
	printk(KERN_INFO "Successfully created ioctl char device /dev/%s using"
		  "major number %d\n", FNIC_FC_CTL_DEV_NAME, fnic_fc_major);
	return rc;
...
int fnic_fc_ioctl_exit()
{
	unregister_chrdev(fnic_fc_major, FNIC_FC_CTL_DEV_NAME);
	printk(KERN_INFO "Removed ioctl char device fnic-fc-mgmt");
	return 0;
}
