...
/* 64 bytes cq entries are same as 32 bytes cq desc. Lower 32 bytes and
 * upper 32 bytes are same.
 */
struct cq_enet_rq_desc_64 {
	struct cq_enet_rq_desc_32 a;
	struct cq_enet_rq_desc_32 b;
};
...
(le16_to_cpu(((struct cq_enet_rq_desc *)desc)->bytes_written_flags) &	\
...
(le32_to_cpu(((struct cq_enet_rq_desc *)desc)->rss_hash))

#define CQ_DESC_RSS_TYPE(desc)						\
((le16_to_cpu(((struct cq_enet_rq_desc *)desc)->q_number_rss_type_flags) >> \
	CQ_DESC_Q_NUM_BITS) & CQ_ENET_RQ_DESC_RSS_TYPE_MASK)

#define CQ_DESC_FCOE(desc)						\
(!!(le16_to_cpu(((struct cq_enet_rq_desc *)desc)->completed_index_flags) & \
 CQ_ENET_RQ_DESC_FLAGS_FCOE))

#define CQ_DESC_FC_CRC_OK(desc)						\
(!!(((struct cq_enet_rq_desc *)desc)->flags & CQ_ENET_RQ_DESC_FCOE_FC_CRC_OK))
...
(!!(((struct cq_enet_rq_desc *)desc)->flags &				\
...
(!!(((struct cq_enet_rq_desc *)desc)->flags & CQ_ENET_RQ_DESC_FLAGS_IPV6))

#define CQ_DESC_VLAN_STRIPPED(desc)					\
(!!(le16_to_cpu(((struct cq_enet_rq_desc *)desc)->bytes_written_flags) & \
    CQ_ENET_RQ_DESC_FLAGS_VLAN_STRIPPED))

#define CQ_DESC_SOP(desc)						\
((((struct cq_enet_rq_desc *)desc)->completed_index_flags &		\
	CQ_ENET_RQ_DESC_FLAGS_SOP) ? 1 : 0)

#define CQ_DESC_EOP(desc)						\
((((struct cq_enet_rq_desc *)desc)->completed_index_flags &		\
	CQ_ENET_RQ_DESC_FLAGS_EOP) ? 1 : 0)

#define CQ_DESC_VLAN_TCI(desc)						\
(le16_to_cpu(((struct cq_enet_rq_desc *)desc)->vlan))

#define CQ_DESC_FCS_OK(desc)						\
(!!(((struct cq_enet_rq_desc *)desc)->flags & CQ_ENET_RQ_DESC_FLAGS_FCS_OK))

#define CQ_DESC_TS(desc)						\
({									\
	struct cq_enet_rq_desc_32 *desc32;				\
	u64 ts;								\
	desc32 = (struct cq_enet_rq_desc_32 *)desc;			\
	ts = desc32->time_stamp2;					\
	ts <<= 32;							\
	ts |= desc32->time_stamp;					\
})

#define CQ_WQ_DESC_INDEX(desc)						\
(le16_to_cpu(((struct cq_enet_wq_desc *)desc)->completed_index))

#define CQ_WQ_DESC_TS(desc)						\
({									\
	struct cq_enet_wq_desc *cq_wq_desc;				\
	u64 ts;								\
	cq_wq_desc = (struct cq_enet_wq_desc *)desc;			\
	ts = le32_to_cpu(cq_wq_desc->time_stamp2);			\
	ts <<= 16;							\
	ts |= cq_wq_desc->time_stamp;					\
})

#define CQ_WQ_DESC_TS_VAL_BIT		BIT(15)
#define CQ_WQ_DESC_TS_VAL(desc)						\
(!!(le16_to_cpu(((struct cq_enet_wq_desc *)desc)->q_number) &		\
		CQ_WQ_DESC_TS_VAL_BIT))

#define CQ_DESC32_TS_VAL_MASK	BIT(6)
#define CQ_DESC_TS_VAL(desc)						\
(!!(((struct cq_enet_rq_desc_32 *)desc)->fetch_index_flags &		\
	CQ_DESC32_TS_VAL_MASK))
