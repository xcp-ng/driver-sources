 ...
 * 01/12/22:  This structure consumes x1f8 (504) bytes
 * and on x86_64 gets allocated from the 512 byte slab.
 * If you expand the structure beyond 512 bytes, it will
 * come from the 1K slab and double lpfc's memory consumption
 */
struct lpfc_iocbq {
	...
	/* All members below 'wqe' gets cleaned up during release */
...
#define LPFC_IO_VMID            0x2000000 /* VMID tagged IO */
...
typedef struct lpfcMboxq {
	...
	union {
		...
		struct lpfc_iocbq *save_iocb;	/* Used in defer_plogi_acc and
						 ...
						 */
		struct list_head *dmabuf_list;	/* Used in auth.c */
	} ctx_u;

	void (*mbox_cmpl) (struct lpfc_hba *, struct lpfcMboxq *);
	uint8_t mbox_flag;
	bool clean_up_flag; /* This flag will be removed by the end of the mbox cleanup */
...
#define LPFC_STOP_IOCB_EVENT     0x020	/* Stop processing IOCB cmds event */
	...
	u16 iocb_continueq_cnt;	/* current length of queue */
