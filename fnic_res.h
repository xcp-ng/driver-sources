                                                    ...
                                                    u32 ratov, u32 edtov)
{
	...
	desc->hdr.tag.u.req_id = req_id; /* id for this request */
     ...
     //   desc->u.nvcmnd.special_req_flags = spl_flags; /* exch req flags */
	...
	//     desc->u.nvcmnd._resvd2 = 0;           /* reserved */
	hton24(desc->u.nvcmnd.d_id, d_id);    /* FC vNIC only: Target D_ID */
	//      desc->u.nvcmnd.mss = mss;             /* FC vNIC only: max burst */
	//     desc->u.nvcmnd.r_a_tov = ratov; /*FC vNIC only: Res. Alloc Timeout */
	//    desc->u.nvcmnd.e_d_tov = edtov; /*FC vNIC only: Err Detect Timeout */
        /*
        ptr = (uint8_t *)desc->u.nvcmnd.nvme_cmnd;
        printk("dumping in copy desc\n");
        for (i = 0; i < cmd_len; i = i + 4) {
                printk("%02x %02x %02x %02x\n",
                        ptr[i], ptr[i+1], ptr[i+2], ptr[i+3]);
        }

        printk("fnic_queue_wq_copy_desc_nvme_io :%02x \n", desc->hdr.type);
        */
	vnic_wq_copy_post(wq);
}
