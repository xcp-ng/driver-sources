...
#else
	struct nvmefc_tgt_ls_req *ls_rsp = &axchg->ls_rsp;
#endif
...
#else
		       struct nvmefc_tgt_ls_req *ls_rsp,
#endif
			  ...
			  struct lpfc_queue *wq)
{
	...
	while (!list_empty(&wq->wqfull_list)) {
		list_remove_head(&wq->wqfull_list, nvmewqeq,
				 struct lpfc_iocbq,
				 list);
		spin_unlock_irqrestore(&pring->ring_lock, iflags);
...
#else
	rc = nvmet_fc_rcv_ls_req(phba->targetport, &axchg->ls_rsp,
				 axchg->payload, axchg->size);
#endif
 ...
 **/
static void
lpfc_nvmet_sol_fcp_abort_cmp(struct lpfc_hba *phba,
			     struct lpfc_iocbq *cmdwqe,
			     struct lpfc_iocbq *rspwqe)
{
	struct lpfc_async_xchg_ctx *ctxp;
 ...
 **/
static void
lpfc_nvmet_unsol_fcp_abort_cmp(struct lpfc_hba *phba,
			       struct lpfc_iocbq *cmdwqe,
			       struct lpfc_iocbq *rspwqe)
{
	struct lpfc_async_xchg_ctx *ctxp;
 ...
 **/
static void
lpfc_nvmet_xmt_ls_abort_cmp(struct lpfc_hba *phba,
			    struct lpfc_iocbq *cmdwqe,
			    struct lpfc_iocbq *rspwqe)
{
	struct lpfc_async_xchg_ctx *ctxp;
