...
static int ioctl_send_fib(struct aac_dev * dev, void __user *arg)
{
	...
	adbg_ioctl(dev,KERN_DEBUG,  "ioctl_send_fib(%p,%p)\n",
	  dev, arg);
...
cleanup:
	if (hw_fib) {
		aac_pci_free_consistent(dev->pdev, size, kfib, fibptr->hw_fib_pa);
...
static int open_getadapter_fib(struct aac_dev * dev, void __user *arg)
{
	...
	if (fibctx == NULL) {
		 ...
		 */
		aac_init_completion(&fibctx->wait_completion);
...
static int next_getadapter_fib(struct aac_dev * dev, void __user *arg)
{
	...
	if (copy_from_user((void *)&f, arg, sizeof(struct fib_ioctl))) {
		adbg_ioctl_and_aif(dev, KERN_INFO,
			"next_getadapter_fib(%p,%p)=-EFAULT\n", dev, arg);
		return -EFAULT;
	}
...
//		struct list_head * entry;
		...
		if (copy_to_user(f.fib, fib->hw_fib_va, sizeof(struct hw_fib))) {
			...
			kfree(fib);
			adbg_ioctl_and_aif(dev, KERN_INFO,
			  "(%p)=-EFAULT copy_to_user(%p,%p,%u)\n",
			  arg, f.fib, fib->hw_fib_va,
		...
		kfree(fib);
		status = 0;
		adbg_ioctl_and_aif(dev, KERN_INFO, "(%p)=0\n", arg);
	} else {
		spin_unlock_irqrestore(&dev->fib_lock, flags);
		...
		if (f.wait) {
			if(aac_wait_for_completion_interruptible(&fibctx->wait_completion) < 0) {
				adbg_ioctl_and_aif(dev, KERN_INFO,
					"(%p)=-EINTR\n", arg);
				status = -ERESTARTSYS;
			} else {
...
static int aac_send_raw_srb(struct aac_dev* dev, void __user * arg)
{
	...
	memset(sg_user, 0, sizeof(sg_user));
	memset(&sg_count, 0, sizeof(sg_count));
	...
	adbg_ioctl_print_raw_srb(dev, (void *)user_srbcmd, fibsize);
	...
	/*
	 * smartmontools has fixed channel of 0 which is incompatible with
	 * thor. So only for thor change the channel to 1 if it is 0
	 */
	if (dev->sa_firmware && chn == 0) {
		chn = 1;
		adbg_ioctl(dev, KERN_INFO, "Changed chn 0 to 1\n");
	}

	if (chn < AAC_MAX_BUSES && cid < AAC_MAX_TARGETS &&
		dev->hba_map[chn][cid].devtype == AAC_DEVTYPE_NATIVE_RAW) {
		is_native_device = 1;
		...
		hbacmd->it_nexus = dev->hba_map[chn][cid].rmw_nexus;
		...
		srbcmd->channel	 = cpu_to_le32(chn);
		srbcmd->id	 = cpu_to_le32(cid);
	...
	if (is_native_device) {
		...
		adbg_ioctl(dev, KERN_INFO, "aac_fib_send(ScsiPortCommand64,,%d,...)\n", actual_fibsize);

		status = aac_fib_send(ScsiPortCommand64, srbfib, actual_fibsize, FsaNormal, 1, 1,NULL,NULL);
	} else { // 32-BIT DMA OPTION
		...
		adbg_ioctl_print_raw_fib(dev, (u8 *)srbfib->hw_fib_va,
				actual_fibsize + sizeof(struct aac_fibhdr));
		adbg_ioctl(dev, KERN_INFO, "aac_fib_send(ScsiPortCommand,,%d,...)\n",
							actual_fibsize);
	...
	if (is_native_device) {
		...
		adbg_smp_dump_ioctl_srb_reply(dev, user_srb, "CSMI SRB Reply",
				(void *)&reply, sizeof(struct aac_srb_reply));
...
cleanup:
	...
	if (rcode != -ERESTARTSYS) {
		for(i=0; i <= sg_indx; i++){
			adbg_smp_dump_ioctl_srb_csmi(dev, user_srb,
						"CSMI Response SRB",
						sg_list[i], sg_count[i]);
...
static int aac_send_reset_adapter(struct aac_dev* dev, void __user *arg)
{
	...
	aac_info(dev, "Issuing IOCTL reset Type-%d\n", reset.reset_type);
