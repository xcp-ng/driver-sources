...
u32 adapter_count;
DEFINE_MUTEX(bnx2fc_dev_lock);
DEFINE_PER_CPU(struct bnx2fc_percpu_s, bnx2fc_percpu);

#define DRV_MODULE_NAME		"bnx2fc"
#define DRV_MODULE_VERSION	BNX2FC_VERSION
#define DRV_MODULE_RELDATE	BNX2FC_RELDATE
...
static void __bnx2fc_destroy2(struct bnx2fc_interface *interface);
#endif
...
#ifdef _DEFINE_FCOE_TRANSPORT_ALLOC_
static int bnx2fc_ctlr_alloc(struct net_device *netdev);
#endif
#ifdef _DEFINE_USE_FCOE_SYSFS_CTLR_IF_
...
#ifdef _DEFINE_FCOE_SYSFS_
void bnx2fc_ctlr_get_lesb(struct fcoe_ctlr_device *ctlr_dev);
#endif
...
uint bnx2fc_link_down_timeout;
module_param_named(link_down_timeout, bnx2fc_link_down_timeout, uint,
	S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(link_down_timeout, " Delay sending link down notification"
	"to libfcoe and recovery using only FLOGI/ADISC.\n");

#ifndef _DEFINE_CPUHP_SETUP_STATE_
 ...
 *
 * NOTE: Copied directly from the SLES 11 SP3 inbox driver.
 */
		...
		struct packet_type *ptype, struct net_device *olddev)
{
	...
	fr->fr_dev = lport;
#if (defined(__BNX2FC_SLES__) && (__BNX2FC_SLES__ < 0x1102)) || \
    (defined(__BNX2FC_RHEL__) && (__BNX2FC_RHEL__ < 0x0602))
	fr->ptype = ptype;
#endif
...
#ifdef _DEFINE_FC_PERCPU_KTHREAD_
...
static void bnx2fc_recv_frame(struct sk_buff *skb)
{
	...
	/* Check for end of frame abort */
	if (fr_eof(fp) == FC_EOF_A)
		interface->hba->stats.fc_eof_a++;
	...
	if (ntoh24(&dest_mac[3]) != ntoh24(fh->fh_d_id)) {
		BNX2FC_HBA_DBG(lport, "FC frame d_id mismatch with MAC %pM.\n",
		    dest_mac);
		interface->hba->stats.drop_d_id_mismatch++;
		kfree_skb(skb);
		return;
	}
	...
	if (vn_port) {
		...
		if (ether_addr_equal(port->data_src_addr, dest_mac)
			...
			interface->hba->stats.drop_fpma_mismatch++;
			kfree_skb(skb);
			return;
		}
	}
	...
	if ((fh->fh_type == FC_TYPE_BLS) && (f_ctl & FC_FC_SEQ_CTX) &&
		...
		interface->hba->stats.drop_abts_seq_ex_ctx_set++;
		kfree_skb(skb);
		return;
	}
	...
	if (lport->port_id != ntoh24(fh->fh_d_id) && !vn_port) {
		...
		interface->hba->stats.drop_lport_d_id_mismatch++;
		kfree_skb(skb);
		return;
	}
...
/*
 * Only use bnx2fc_link_speed_update if fcoe_link_speed_update is not
 * defined
 */
#ifndef _DEFINE_FCOE_LINK_SPEED_UPDATE_
...
static int bnx2fc_get_wwn(struct net_device *netdev, u64 *wwn, int type)
{
...
static int bnx2fc_net_config(struct fc_lport *lport, struct net_device *netdev)
{
...
#ifdef _DEFINE_TIMER_SETUP_
	timer_setup(&port->timer, fcoe_queue_timer, 0);
#else
	setup_timer(&port->timer, fcoe_queue_timer, (unsigned long) lport);
#endif

#ifdef _DEFINE_FCOE_LINK_SPEED_UPDATE_
	...
	if (!lport->vport) {
		if (bnx2fc_get_wwn(netdev, &wwnn, NETDEV_FCOE_WWNN))
#ifdef _DEFINE_FCOE_SYSFS_
		...
		if (bnx2fc_get_wwn(netdev, &wwpn, NETDEV_FCOE_WWPN))
#ifdef _DEFINE_FCOE_SYSFS_
			wwpn = fcoe_wwn_from_mac(ctlr->ctl_src_addr, 2, 0);
#else
...
static void bnx2fc_clear_all_srr(struct bnx2fc_hba *hba)
{
	struct bnx2fc_rport *tgt;
	...
	for (i = 0; i < BNX2FC_NUM_MAX_SESS; i++) {
		...
		if (!tgt)
			continue;
		list_for_each_entry(io_req, &tgt->els_queue, link) {
			clear_bit(BNX2FC_FLAG_SRR_SENT, &io_req->req_flags);
		}
	}
	mutex_unlock(&hba->hba_mutex);
}

static void bnx2fc_link_down_work(struct work_struct *work)
{
	struct bnx2fc_interface *interface =
	    container_of(work, typeof(*interface), link_down_work.work);
	...
	BNX2FC_HBA_DBG(lport, "Link down timer expired\n");

	fcoe_ctlr_link_down(ctlr);
}

static int bnx2fc_link_down(struct fcoe_ctlr *ctlr)
{
	struct bnx2fc_interface *interface = bnx2fc_from_ctlr(ctlr);
	struct fc_lport *lport;

	if (!interface || !bnx2fc_link_down_timeout)
		return fcoe_ctlr_link_down(ctlr);
	...
	/* Set this so we know link_down_timeout is in effect */
	atomic_set(&interface->link_down_work_valid, 1);

	if (queue_delayed_work(interface->timer_work_queue,
	    &interface->link_down_work, bnx2fc_link_down_timeout * HZ)) {
		BNX2FC_HBA_DBG(lport, "Link down timer started\n");
	}

	bnx2fc_clear_all_srr(interface->hba);

	return 0; /* not yet timed out */
}

static int bnx2fc_send_flogi(struct fc_lport *lport,
	struct bnx2fc_interface *interface)
{
	struct fc_frame *fp;
	...
	if (!fp) {
		BNX2FC_HBA_DBG(lport, "fc_frame_alloc failed.\n");
		return -ENOMEM;
	}

	BNX2FC_HBA_DBG(lport, "Sending FLOGI to reestablish session with "
	    "switch.\n");
	lport->tt.elsct_send(lport, FC_FID_FLOGI, fp,
	    ELS_FLOGI, bnx2fc_flogi_resp, lport, lport->r_a_tov);

	init_completion(&interface->flogi_compl);

	return 0;
}

/*
 * This function is called when we emply the link_down_timeout infrastructure.
 * Essentially what happens is that we prevent calling fcoe_ctlr_link_down
 * for a specificed number of seconds.  If the link comes back up before the
 * timeout expires, the targets will not have been offloaded.  However for
 * traffic to continue to flow we will need issue a FLOGI to the switch and
 * then send an ADISC to all the offloaded targets to reestablish the sessions
 * we had going.
 */
static void bnx2fc_link_recovery_work(struct work_struct *work)
{
	struct bnx2fc_interface *interface =
	    container_of(work, typeof(*interface), link_recovery_work.work);
	struct fcoe_ctlr *ctlr = bnx2fc_to_ctlr(interface);
	struct fc_lport *lport;
	struct bnx2fc_rport *tgt;
	...
	rc = bnx2fc_send_flogi(lport, interface);
	if (rc)
		return;

	i = wait_for_completion_timeout(&interface->flogi_compl,
		lport->r_a_tov);

	if (i == 0) {
		BNX2FC_HBA_DBG(lport, "flogi_compl timed out.\n");
		return;
	}

	/* Need to give things a chance to settle after FLOGI acc */
	msleep(1000);
	BNX2FC_HBA_DBG(lport, "Sending ADISC to all offloaded ported.\n");

	/* Send ADISC to all offloaded ports */
	for (i = 0; i < BNX2FC_NUM_MAX_SESS; i++) {
		tgt = interface->hba->tgt_ofld_list[i];
		if (tgt) {
			rdata = tgt->rdata;
			if (rdata) {
#ifdef _DEFINE_FC_RPORT_LOGIN_
...
static void bnx2fc_link_up(struct fcoe_ctlr *ctlr)
{
	struct bnx2fc_interface *interface = bnx2fc_from_ctlr(ctlr);
	struct fc_lport *lport;
	...
	if (!cancel_delayed_work(&interface->link_down_work))
		flush_workqueue(interface->timer_work_queue);
	else {
		BNX2FC_HBA_DBG(lport, "Link down timer cancelled\n");
	}

	fcoe_ctlr_link_up(ctlr);

	if (bnx2fc_link_down_timeout > 0 &&
	    atomic_read(&interface->link_down_work_valid) != 0) {
		atomic_set(&interface->link_down_work_valid, 0);
		queue_delayed_work(interface->timer_work_queue,
		    &interface->link_recovery_work, 1 * HZ);
	}
}
				     ...
				     u16 vlan_id)
{
    ...
    (defined(__BNX2FC_RHEL__) && (__BNX2FC_RHEL__ >= 0x0700))
...
#else /* Not _DEFINE_USE_FCOE_SYSFS_CTLR_IF_ */
...
#else
				bnx2fc_link_up(&interface->ctlr);
		} else if (bnx2fc_link_down(&interface->ctlr)) {
#endif
			mutex_lock(&lport->lp_mutex);
			list_for_each_entry(vport, &lport->vports, list)
				fc_host_port_type(vport->host) =
...
#endif /* End of _DEFINE_USE_FCOE_SYSFS_CTLR_IF_ */
	}
	mutex_unlock(&bnx2fc_dev_lock);
...
static int bnx2fc_libfc_config(struct fc_lport *lport)
{
	...
	fc_elsct_init(lport);
	fc_exch_init(lport);
#ifdef _DEFINE_FC_RPORT_INIT_
	fc_rport_init(lport);
#endif
	fc_disc_init(lport);
#ifdef _DEFINE_FC_DISC_CONFIG_
			   ...
			   struct net_device *orig_dev)
{
	...
	size_t dlen, rlen;
	int flogi_resp = 0, fdisc_resp = 0;
	u32 desc_cnt = 0;
	u16 flags;
	u64 fabric_name;
#endif
	struct fip_header *fiph = (struct fip_header *)skb->data;
	u16 op = ntohs(fiph->fip_op);
	u8 sub = fiph->fip_subcode;

	interface = container_of(ptype, struct bnx2fc_interface,
				 fip_packet_type);

	if (op == FIP_OP_CTRL && sub == FIP_SC_CLR_VLINK &&
	    atomic_read(&interface->link_down_work_valid) > 0) {
		BNX2FC_MISC_DBG("Not servicing clear virtual link while link "
		    "down timer active.\n");
		kfree_skb(skb);
		return 0;
	}
	...
	/*
	 * workaround for HP issue to not consider an FCF for selection if the
	 * fabric name is 0 and not available for login
	 */
	if (op == FIP_OP_DISC && sub == FIP_SC_ADV) {
		flags = ntohs(fiph->fip_flags);
		...
		while (rlen > 0) {
			...
			case FIP_DT_FAB:
				...
				fabric_name = get_unaligned_be64(&fab->fd_wwn);
				if (!fabric_name &&  !(flags  & FIP_FL_AVAIL)) {
					printk(KERN_ERR PFX "fabric name is 0, and FCF not available\n");
					goto out;
				}
				break;
			default:
		...
		case FIP_DT_MAC:
			...
			if (desc_cnt == 3) {
				if (flogi_resp) {
					...
					BNX2FC_MISC_DBG("FIP dt_mac: FDISC resp\n");
					memcpy(interface->dest_addr,
					       ((struct fip_mac_desc *)desc)->fd_mac,
					       ETH_ALEN);
				}
...
out:
#else //#ifdef _DEFINE_FCOE_SYSFS_
	ctlr = bnx2fc_to_ctlr(interface);
...
static int bnx2fc_validate_vport_create(struct fc_vport *vport)
{
	...
	if (!memcmp(&n_port->wwpn, &vport->port_name, sizeof(u64))) {
		BNX2FC_MISC_DBG("vport WWPN 0x%s is same as that of the "
				"base port WWPN\n", buf);
		rc = -EINVAL;
		goto out;
	}
	...
	list_for_each_entry(vn_port, &n_port->vports, list) {
		if (!memcmp(&vn_port->wwpn, &vport->port_name, sizeof(u64))) {
			BNX2FC_MISC_DBG("vport with given WWPN 0x%s already "
					"exists\n", buf);
			rc = -EINVAL;
			break;
		}
...
static int bnx2fc_vport_create(struct fc_vport *vport, bool disabled)
{
	...
	rc = bnx2fc_validate_vport_create(vport);
	if (rc) {
		wwn_to_str(vport->port_name, buf, sizeof(buf));
...
static int bnx2fc_vport_destroy(struct fc_vport *vport)
{
	...
	if (!found) {
		printk(KERN_ERR PFX "vport_destroy: vport already deleted\n");
		mutex_unlock(&n_port->lp_mutex);
		return -ENOENT;
	}
...
static void bnx2fc_set_vport_symbolic_name(struct fc_vport *vport)
{
	struct fc_lport *lport = vport->dd_data;
	struct fcoe_port *port = lport_priv(lport);
	struct bnx2fc_interface *interface = port->priv;
...
static void bnx2fc_hba_destroy(struct bnx2fc_hba *hba)
{
	...
	/* Free any aggregate stat structures */
	list_for_each_safe(slist, tmp, &hba->bnx2fc_stat_list) {
		struct bnx2fc_tgt_dbg_stats *dbg_stats =
					   (struct bnx2fc_tgt_dbg_stats *)slist;
		list_del(&dbg_stats->list);
		kfree(dbg_stats);
	}
...
static struct bnx2fc_hba *bnx2fc_hba_create(struct cnic_dev *cnic)
{
	...
	INIT_LIST_HEAD(&hba->bnx2fc_stat_list);
			...
			enum fip_state fip_mode)
{
	...
	ctlr_dev = fcoe_ctlr_device_add(&netdev->dev, &bnx2fc_fcoe_sysfs_templ,
					 size);
	if (!ctlr_dev) {
		printk(KERN_ERR PFX "Unable to allocate ctlr structure\n");
		return NULL;
	}
	ctlr = fcoe_ctlr_device_priv(ctlr_dev);
#ifdef _DEFINE_FCOE_CTLR_CDEV_
	ctlr->cdev = ctlr_dev;
#endif
	...
	/* Initialize work structs for link_down_timeout infrastructure */
	INIT_DELAYED_WORK(&interface->link_down_work, bnx2fc_link_down_work);
	INIT_DELAYED_WORK(&interface->link_recovery_work,
	    bnx2fc_link_recovery_work);
	init_completion(&interface->flogi_compl);
				  ...
				  struct device *parent, int npiv)
{
	...
	port->lport = lport;
	port->priv = interface;
#ifdef _DEFINE_PORT_GET_NETDEV_
...
static int bnx2fc_destroy(struct net_device *netdev)
{
	...
	if (!cancel_delayed_work(&interface->link_down_work) ||
	    !cancel_delayed_work(&interface->link_recovery_work))
		flush_workqueue(timer_work_queue);
...
#if (defined(__BNX2FC_SLES__) && (__BNX2FC_SLES__ == 0x1104)) || defined(_DEFINE_FCOE_SYSFS_)
...
static void bnx2fc_npiv_create_from_nvram(
    struct bnx2fc_interface *interface)
{
	struct fc_lport *lport;
	struct bnx2fc_hba *hba;
	...
	lport = bnx2fc_to_ctlr(interface)->lp;
	if (!lport)
		return;
...
static void bnx2fc_stop(struct bnx2fc_interface *interface)
{
	...
	fc_host_port_type(lport->host) = FC_PORTTYPE_UNKNOWN;
#ifdef _DEFINE_FCOE_SYSFS_
	bnx2fc_link_down(ctlr);
#else
	bnx2fc_link_down(&interface->ctlr);
#endif
	fcoe_clean_pending_queue(lport);
}
...
#if (defined(__BNX2FC_SLES__) && (__BNX2FC_SLES__ == 0x1104))
void bnx2fc_delayed_static_npiv_create(struct work_struct *work)
{
	struct bnx2fc_interface *interface =
	    container_of(work, typeof(*interface), static_npiv_work);

	if (!interface)
		return;

	bnx2fc_npiv_create_from_nvram(interface);
}
#endif
...
#if (defined(__BNX2FC_SLES__) && (__BNX2FC_SLES__ == 0x1104))
	INIT_WORK(&interface->static_npiv_work,
	    bnx2fc_delayed_static_npiv_create);
	queue_work(interface->timer_work_queue,
	    &interface->static_npiv_work);
#endif
}
...
#else
	if (!interface || !interface->ctlr.lp) {
		pr_err(PFX "bnx2fc_disable: interface or lport not found\n");
#endif
		rc = -ENODEV;
	} else {
#ifdef _DEFINE_FCOE_SYSFS_
...
/**
 * Deprecated (for new sysfs control): Use bnx2fc_enabled()
 */
static int bnx2fc_enable(struct net_device *netdev)
{
	struct bnx2fc_interface *interface;
...
#else
	if (!interface || !interface->ctlr.lp) {
		pr_err(PFX "bnx2fc_enable: interface or lport not found\n");
#endif
		rc = -ENODEV;
#ifdef _DEFINE_FCOE_SYSFS_
	} else {
...
int bnx2fc_validate_netdev(struct net_device *netdev)
{
			  ...
			  enum bnx2fc_create_link_state link_state)
{
	...
	rc = bnx2fc_validate_netdev(netdev);
	if (rc) {
		module_put(THIS_MODULE);
		goto mod_err;
	}
...
#ifdef _DEFINE_CPUHP_SETUP_STATE_
/**********
 * This is the new style CPU hotplug code from Linux kernel 4.6 and onward
 **********/
...
#else
/**********
 * This is the old style CPU hotplug code before Linux kernel 4.6
 **********/
...
static int bnx2fc_slave_configure(struct scsi_device *sdev)
{
	...
	/* scsi_*_tcq were removed in 3.19.0 */
	if (sdev->tagged_supported)
		scsi_activate_tcq(sdev, bnx2fc_queue_depth);
	else
		scsi_deactivate_tcq(sdev, bnx2fc_queue_depth);
#else
...
static void __exit bnx2fc_mod_exit(void)
{
	...
	cnic_unregister_driver2(CNIC_ULP_FCOE);
...
static struct fc_function_template bnx2fc_transport_function = {
	...
	.set_vport_symbolic_name = bnx2fc_set_vport_symbolic_name,
...
/*
 * Additional SCSI host attributes
 */
static ssize_t
bnx2fc_stats_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct bnx2fc_hba *hba = interface->hba;
	int i;
	u32 rec_expirations = 0;
	u32 ulp_tov_expirations = 0;
	u32 srrs = 0;
	u32 seq_timeouts = 0;
	...
	for (i = 0; i < BNX2FC_NUM_MAX_SESS; i++) {
		...
		if (tgt) {
			...
			ulp_tov_expirations += tgt->stats.num_cmd_timeouts;
			ulp_tov_expirations +=
			    tgt->stats.num_eh_abort_timeouts;
			ulp_tov_expirations += tgt->stats.num_abts_timeouts;
			ulp_tov_expirations += tgt->stats.num_els_abts_timeouts;
			rec_expirations += (tgt->stats.num_rec_issued -
			    tgt->stats.num_rec_compl);
			srrs += tgt->stats.num_srr_issued;
			seq_timeouts += tgt->stats.num_seq_cleanup_issued;
			spin_unlock_bh(&tgt->tgt_lock);
		}
	}
	spin_unlock_bh(&hba->hba_lock);

	sprintf(buf, "FLOGICount: %u\n", hba->stats.flogis);
	sprintf(buf + strlen(buf), "FDISCCount: %u\n", hba->stats.fdiscs);
	sprintf(buf + strlen(buf), "ABTSCount: %u\n", hba->stats.abts);
	sprintf(buf + strlen(buf), "ResetTargetCount: %u\n",
	    hba->stats.target_resets);
	sprintf(buf + strlen(buf), "ResetLUNCount: %u\n",
	    hba->stats.lun_resets);
	sprintf(buf + strlen(buf), "RECExpirations: %u\n", rec_expirations);
	sprintf(buf + strlen(buf), "ULPTOVExpirations: %u\n",
	    ulp_tov_expirations);
	sprintf(buf + strlen(buf), "SessionLevelRecoveryCount: %u\n", srrs);
	sprintf(buf + strlen(buf), "SequenceTimeouts: %u\n", seq_timeouts);
	sprintf(buf + strlen(buf), "EOFAFrames: %u\n", hba->stats.fc_eof_a);
	sprintf(buf + strlen(buf), "Drop D_ID mismatch: %u\n",
	    hba->stats.drop_d_id_mismatch);
	sprintf(buf + strlen(buf), "Drop FPMA mismatch: %u\n",
	    hba->stats.drop_fpma_mismatch);
	sprintf(buf + strlen(buf), "Drop wrong source MAC: %u\n",
	    hba->stats.drop_wrong_source_mac);
	sprintf(buf + strlen(buf), "Drop ABTS SEQ/EX CTX set: %u\n",
	    hba->stats.drop_abts_seq_ex_ctx_set);
	sprintf(buf + strlen(buf), "Drop lport D_ID mismatch: %u\n",
	    hba->stats.drop_lport_d_id_mismatch);
...
static DEVICE_ATTR(stats, S_IRUGO, bnx2fc_stats_show, NULL);
static DEVICE_ATTR(tm_timeout, S_IRUGO|S_IWUSR, bnx2fc_tm_timeout_show,
	bnx2fc_tm_timeout_store);

#ifdef _SHOST_ATTRS_
struct device_attribute *bnx2fc_host_attrs[] = {
	&dev_attr_stats,
	&dev_attr_tm_timeout,
	NULL,
};
...
static struct scsi_host_template bnx2fc_shost_template = {
	...
	.lockless		= 1,
#endif
#if (!defined(__BNX2FC_UEK__) && !defined(__BNX2FC_CITRIX__) && (defined(__BNX2FC_RHEL__) && ((__BNX2FC_RHEL__ >= 0x0700) && (__BNX2FC_RHEL__ < 0x0800))))
	.no_async_abort		= 1,
#endif
...
#if (defined(__BNX2FC_SLES__) && (__BNX2FC_SLES__ < 0x1104))
	/* SLES 11 SP3 and below do not have fcoe_get_lesb defined */
