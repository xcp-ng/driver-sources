...
int bnx2fc_send_fw_fcoe_init_msg(struct bnx2fc_hba *hba)
{
	...
	fcoe_init3.dst_mac_addr_lo = (cnic->fip_mac[4] << 8) | cnic->fip_mac[5];
	fcoe_init3.dst_mac_addr_mid = (cnic->fip_mac[2] << 8) | cnic->fip_mac[3];
	fcoe_init3.dst_mac_addr_hi = (cnic->fip_mac[0] << 8) | cnic->fip_mac[1];

	BNX2FC_MISC_DBG("Setting init3 FIP MAC filter to %pM.\n",
	    cnic->fip_mac);
...
#else
	if (ether_addr_equal(port->data_src_addr, tgt->src_addr) == 0) {
#endif
		...
		unsigned char *rq_data, u8 num_rq, struct fcoe_task_ctx_entry *task)
{
	...
	case BNX2FC_SCSI_CMD:
		if (rx_state == FCOE_TASK_RX_STATE_COMPLETED) {
			bnx2fc_process_scsi_cmd_compl(io_req, task, num_rq, rq_data);
			spin_unlock_bh(&tgt->tgt_lock);
			return;
		}
...
num_rq_zero:
			...
			work = bnx2fc_alloc_work(tgt, wqe, rq_data_buff, num_rq, task);
			if (work)
...
unlock:
			...
			if (fps->iothread && work)
				...
				bnx2fc_process_cq_compl(tgt, wqe, rq_data_buff, num_rq, task);
			num_free_sqes++;
		}
