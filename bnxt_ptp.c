...
static int bnxt_get_target_cycles(struct bnxt_ptp_cfg *ptp, u64 target_ns,
				  u64 *cycles_delta)
{
	u64 cycles_now, nsec_now, nsec_delta;
	int rc;
...
skip_target_cycles:
	...
	req->ptp_freq_adj_ext_phase_lower = cpu_to_le32(lower_32_bits(delta));
	req->ptp_freq_adj_ext_phase_upper = cpu_to_le32(upper_32_bits(delta));
