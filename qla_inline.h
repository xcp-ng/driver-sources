...
static inline void
qla2xxx_atomic_sub(atomic_t *v, int new)
{
	int old;

	do
		old = atomic_read(v);
	while (old != atomic_cmpxchg(v, old, old - new));
}

static inline void
qla2xxx_atomic_add(atomic_t *v, int new)
{
	int old;
...
static inline void
qla2xxx_atomic64_sub(atomic64_t *v, int new)
{
	uint64_t old;

	do
		old = atomic64_read(v);
	while (old != atomic64_cmpxchg(v, old, old - new));
}

static inline void
qla2xxx_atomic64_add(atomic64_t *v, int new)
{
	uint64_t old;
...
qla2xxx_rel_qpair_sp(struct qla_qpair *qpair, srb_t *sp)
{
	...
	sp->done_jiffies = jiffies; /* for crash debugging */
...
{
	/*
	 * Some paths (like EDC/RDF) uses temporary fcport that would be
	 * freed before this call, so do not use sp->fcport here blindly.
	 */
	ql_srb_trace_ext(ql_dbg_disc, sp->vha, NULL,
		"sp=%px type=%d", sp, sp->type);
...
qla_mapq_init_qp_cpu_map(struct qla_hw_data *ha,
       	struct qla_msix_entry *msix, struct qla_qpair *qpair)
{
