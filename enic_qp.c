...
static const char qp_type_names[ENIC_MAX_QP_TYPE][8] = {
	"DATA",
	"ADMIN"
};
static const char unknown_str[] = "Unknown";

const char *enic_qp_type_to_str(enum enic_qp_type type)
{
	if (type < ENIC_MAX_QP_TYPE)
		return qp_type_names[type];

	return unknown_str;
}

static const char intr_res_type_names[ENIC_MAX_INTR][24] = {
	"ALL_INTR",
	"DATA_QP_INTR",
	"ADMIN_QP_INTR",
	"ENIC_ERR_NOTIFY_INTR"
};

const char *enic_intr_res_type_to_str(enum enic_intr_res_type type)
{
	if (type < ENIC_MAX_INTR)
		return intr_res_type_names[type];
...
int enic_is_res_intr_open(struct enic *enic, enum enic_intr_res_type res_type)
{
	...
	case ENIC_ERR_NOTIFY_INTR:
		return !!(enic->res_status_flags &
			  (ENIC_RES_OPEN(res_type)));
	default:
		break;
	}
...
void enic_deinit_qps(struct enic *enic, enum enic_qp_type qp_type)
{
	...
	ret = enic_get_qps_range(enic, qp_type, &qp_from, &qp_to);
	if (ret) {
		netdev_err(netdev, "%s - BUG: unable to get QP range for type %s(%u)\n",
			   __func__, enic_qp_type_to_str(qp_type), qp_type);
		return;
	}

	count = enic->wq_count[qp_type];
	for (i = 0; i < count; i++) {
		qp = &enic->qp[qp_from + i];
		qp_ring = &enic->qp_ring[qp_from + i];
...
static void enic_wq_init(struct enic_qp *qp, int cq_index)
{
	u64 paddr;
	...
	enic_wq_init_ctrl(ctrl, paddr, count, cq_index,
...
static void enic_rq_init(struct enic_qp *qp, int cq_index)
{
	u64 paddr;
	struct enic_qp_ring *qp_ring = &qp->enic->qp_ring[qp->index];
	...
	enic_rq_init_ctrl(ctrl, paddr, count, cq_index,
			  error_interrupt_enable, error_interrupt_offset,
			  0,	/* error_status */
			  0,	/* fetch_index */
			  0	/* posted_index */);
...
static int enic_rq_fill_bufs(struct enic_qp *qp)
{
	...
	unsigned int len = get_max_pkt_len(netdev->mtu);
	dma_addr_t dma_addr;
	struct enic_rq_buf *buf;
	int ret = 0;
...
static int enic_rq_fill_buf_pages(struct enic_qp *qp)
{
	...
	while (buf->next != qp->rq.to_clean) {
		...
		rq_enet_desc_enc(&qp->rq.rq_base[buf->index],
				 ...
				 VNIC_PADDR_TARGET, qp->rq.rq_type,
				 qp->rq.seg_len);
		trace_enic_rq_desc(qp, buf);
...
static int enic_alloc_wq(struct enic_qp *qp, enum enic_qp_type qp_type)
{
	struct enic *enic = qp->enic;
	int index = qp->index;
	struct enic_qp_ring *qp_ring = &enic->qp_ring[index];
	enum vnic_res_type wq_res_type, cq_res_type;
	int wq_index, cq_index;
	int ret;

	switch (qp_type) {
	case ENIC_DATA_QP:
		wq_res_type = RES_TYPE_WQ;
		cq_res_type = RES_TYPE_CQ;
		wq_index = index;
		cq_index = index;
		break;
	case ENIC_ADMIN_QP:
		wq_res_type = RES_TYPE_ADMIN_WQ;
		cq_res_type = RES_TYPE_ADMIN_CQ;
		/* Assumption: we only support one ADMIN QP */
		wq_index = 0;
		cq_index = 0;
		break;
	default:
		netdev_err(enic->netdev, "%s - Unsupported QP type %s(%u)\n",
			   __func__, enic_qp_type_to_str(qp_type), qp_type);
		return -EINVAL;
	}

	qp->wq.wq_res_type = wq_res_type;
	qp->wq.wq_res_index = wq_index;
	qp->wq.cq_res_type = cq_res_type;
	qp->wq.cq_res_index = cq_index;

	qp->wq.ctrl = vnic_dev_get_res(enic->vdev, wq_res_type, wq_index);
	if (!qp->wq.ctrl) {
		netdev_err(qp->netdev, "%s[%d] ctrl not found\n",
			   enic_get_vnic_res_name(wq_res_type), wq_index);
		return -ENODEV;
	}

	qp_ring->wcq_ctrl = vnic_dev_get_res(enic->vdev, cq_res_type,
					     cq_index);
	if (!qp_ring->wcq_ctrl) {
		netdev_err(qp->netdev, "W%s[%d] ctrl not found\n",
			   enic_get_vnic_res_name(cq_res_type), cq_index);
		return -ENODEV;
	}
...
static int enic_alloc_rq(struct enic_qp *qp, enum enic_qp_type qp_type)
{
	struct enic *enic = qp->enic;
	int index = qp->index;
	int ret;
	int cq_desc_size;
	struct enic_qp_ring *qp_ring = &enic->qp_ring[index];
	enum vnic_res_type rq_res_type, cq_res_type;
	int rq_index, cq_index;
	...
	switch (qp_type) {
	case ENIC_DATA_QP:
		rq_res_type = RES_TYPE_RQ;
		cq_res_type = RES_TYPE_CQ;
		rq_index = index;
		cq_index = enic->wq_count[ENIC_DATA_QP] + index;
		break;
	case ENIC_ADMIN_QP:
		rq_res_type = RES_TYPE_ADMIN_RQ;
		cq_res_type = RES_TYPE_ADMIN_CQ;
		/* Assumption: we only support one ADMIN QP */
		rq_index = 0;
		cq_index = 1;
	...
	qp->rq.rq_res_type = rq_res_type;
	qp->rq.rq_res_index = rq_index;
	qp->rq.cq_res_type = cq_res_type;
	qp->rq.cq_res_index = cq_index;

	qp->rq.ctrl = vnic_dev_get_res(enic->vdev, rq_res_type, rq_index);
	if (!qp->rq.ctrl) {
		netdev_err(qp->netdev, "%s[%d] ctrl not found\n",
			   enic_get_vnic_res_name(rq_res_type), rq_index);
		return -ENODEV;
	}

	qp_ring->rcq_ctrl = vnic_dev_get_res(enic->vdev, cq_res_type,
					     cq_index);
	if (!qp_ring->rcq_ctrl) {
		netdev_err(qp->netdev, "R%s[%d] ctrl not found\n",
			   enic_get_vnic_res_name(cq_res_type), cq_index);
		return -ENODEV;
	}
...
/* Since QPs are allocated at probe time and freed at remove time, there is no
 * need for now to teach enic_alloc_qp() and enic_free_qp() about the different
 * QP types.
 * We can use a single QP array for all QPs, and alloc/free all QPs at the same
 * time.
 *
 * enic->qp_count includes all QP types, not just DATA QPs.
 */
...
static void enic_rq_set_fns(struct enic_qp *qp, bool mixed_rq_descs)
{
	 ...
	 */
	if (mixed_rq_descs) {
		max_pkt_len = get_max_pkt_len(qp->netdev->mtu);
		qp->rq.service = enic_rq_service_page;
		qp->rq.fill_bufs = enic_rq_fill_buf_pages;
		qp->rq.split_pages = max_pkt_len <= (PAGE_SIZE / 2);
		qp->rq.buf_size = (qp->rq.split_pages) ? PAGE_SIZE / 2 :
				   PAGE_SIZE;
		if (max_pkt_len > qp->rq.buf_size) {
			/* Packets may span multiple pages so use the RQ type
			 * that enables spill over into the next buffer.
			 */
			qp->rq.seg_len = qp->rq.buf_size;
			qp->rq.rq_type = RQ_ENET_TYPE_SOP_OR_EOP;
		} else {
			qp->rq.seg_len = max_pkt_len;
			qp->rq.rq_type = RQ_ENET_TYPE_ONLY_SOP;
		}
...
int enic_init_err_notify_intrs(struct enic *enic)
{
	...
	case VNIC_DEV_INTR_MODE_MSIX:
	case VNIC_DEV_INTR_MODE_INTX:
		ret = enic_get_intr_range(enic, ENIC_ERR_NOTIFY_INTR, &intr_from, &intr_to);
		if (ret) {
			netdev_err(enic->netdev, "%s - Unable to get intr range for err/notify\n",
				   __func__);
			return ret;
		}

		enic->err_ctrl = enic->intr[intr_from].ctrl;
		enic->notify_ctrl = enic->intr[intr_to].ctrl;
...
/* Return the 'qp_index'th QP of type 'qp_type'
 */
					    ...
					    int qp_index)
{
	...
	if (qp_index > qp_to - qp_from)
		return NULL;

	return &enic->qp[qp_from + qp_index];
}

int enic_get_qps_range(struct enic *enic, enum enic_qp_type qp_type,
		       unsigned int *from, unsigned int *to)
{
	int admin_qp_present;
	int ret = 0;

	admin_qp_present = enic_has_admin_res(enic);

	/* enic->qp_count includes the ADMIN QP when present
	 */
	switch (qp_type) {
	case ENIC_DATA_QP:
		*from = 0;
		*to = enic->qp_count - 1 - (admin_qp_present ? 1 : 0);
		break;
	case ENIC_ADMIN_QP:
		if (!admin_qp_present)
			return -ENOENT;

		*from = enic->qp_count - 1;
		*to = enic->qp_count - 1;
		break;
	default:
		ret = -EINVAL;
			...
			unsigned int *from, unsigned int *to)
{
	...
	/* enic->qp_count includes the ADMIN QP count when present */
	switch (res_type) {
	/* NOTE: This case is not used yet. It may go away */
	case ENIC_ALL_INTR:
		*from = 0;
		*to = enic->qp_count + 1;
		break;
	...
	case ENIC_ADMIN_QP_INTR:
		...
		*to = enic->qp_count - 1;
		break;
	case ENIC_ERR_NOTIFY_INTR:
		/* err/notify are always the last two */
		*from = enic->qp_count;
		*to = enic->qp_count + 1;
		break;
	default:
...
int enic_init_qps(struct enic *enic, enum enic_qp_type qp_type)
{
	 ...
	 */
	if (qp_type == ENIC_DATA_QP)
		mixed_rq_descs = ENIC_SETTING(enic, MIXED_RQ_DESC_TYPE);
	else
		mixed_rq_descs = false;

	ret = enic_get_qps_range(enic, qp_type, &qp_from, &qp_to);
	if (ret) {
		netdev_err(netdev, "%s - Unable to get range for QP type %s(%u)\n",
			   __func__, enic_qp_type_to_str(qp_type), qp_type);
		return ret;
	}
	...
	for (i = 0; i < enic->wq_count[qp_type]; i++) {
		ret = enic_alloc_wq(&enic->qp[qp_from + i], qp_type);
		if (ret)
			goto out;
	}

	for (i = 0; i < enic->rq_count[qp_type]; i++) {
		ret = enic_alloc_rq(&enic->qp[qp_from + i], qp_type);
		if (ret)
			goto out;
	}
...
again:
	...
	if (CQ_DESC_COLOR_16(cq_desc) != qp->wq.last_color) {
		...
		while (buf->index != toclean) {
			...
			if (enic_qp_is_admin(qp))
				enic_mbox_free_skb(netdev_priv(qp->netdev),
						   buf->skb);
			else
...
static int enic_mbox_enqueue_rx_msg(struct enic_qp *qp, u16 vf_id,
				    struct sk_buff *skb)
{
	struct net_device *netdev = qp->netdev;
	...
	WARN_ON(!enic_is_pf(enic) && !enic_is_vf_v2(enic));
	...
	if (atomic_read(&enic->remove_in_progress)) {
		...
		/* PF case
		 */
		if ((msg_type != ENIC_MBOX_VF_UNREGISTER_REQUEST) &&
		    /* VF_V2 case
		     */
		    (msg_type != ENIC_MBOX_VF_UNREGISTER_REPLY)) {
			netdev_info(netdev, "%s -  Discarding msg %s from VF %u\n",
				    __func__,
	...
	msg_skb = enic_mbox_alloc_rx_msg(enic);
	if (!msg_skb) {
		napi_consume_skb(skb, 0);
		return -ENOMEM;
	}
	...
	enic_mbox_queue_msg_skb(enic, msg_skb);
	schedule_work(&enic->sriov.mbox_work);
	...
	if (unlikely(CQ_DESC_PKT_ERR(cq_desc))) {
		if (!CQ_DESC_FCS_OK(cq_desc)) {
			if (CQ_DESC_PKT_LEN(cq_desc) > 0) {
				...
				/* For MTU > PAGE_SIZE, the RQ descriptor types
				 * are "SOP or EOP" and a truncation means
				 * there is at least 1 RQ desc, but not enough
				 * for the entire packet. These should be
				 * considerd no buf drops. For MTU <= PAGE_SIZE
				 * the RQ desc types are "SOP only" and
				 * truncations are due to oversized packets
				 * and are counted as rx_over_errors.
				 */
				if (qp->rq.rq_type == RQ_ENET_TYPE_SOP_OR_EOP)
					qp->rq.stats.pkt_trunc_drop++;
				else
					qp->rq.stats.pkt_truncated++;
				ret = 2;
			}
		} else
			net_err_ratelimited("%s: rq[%d] MAC error\n",
					    qp->netdev->name, qp->index);
	}
	return ret;
}
...
static void enic_update_eg_frame_byte_counters(struct enic_qp *qp,
					       struct sk_buff *skb)
{
	struct ethhdr *eth = (struct ethhdr *)skb_mac_header(skb);

	if (is_broadcast_ether_addr((char *)&eth->h_dest)) {
		qp->wq.stats.tx_broadcast_frames++;
		qp->wq.stats.tx_broadcast_bytes += skb->len;
	} else if (is_multicast_ether_addr((char *)&eth->h_dest)) {
		qp->wq.stats.tx_multicast_frames++;
		qp->wq.stats.tx_multicast_bytes += skb->len;
	} else {
		qp->wq.stats.tx_unicast_frames++;
		qp->wq.stats.tx_unicast_bytes += skb->len;
	}
}

static void enic_update_ig_frame_byte_counters(struct enic_qp *qp,
					       const struct ethhdr *eth,
					       u32 pkt_len)
{
	if (enic_hw_rx_stats_ok(netdev_priv(qp->netdev)))
		return;

	if (is_broadcast_ether_addr(eth->h_dest)) {
		qp->rq.stats.rx_broadcast_frames++;
		qp->rq.stats.rx_broadcast_bytes += pkt_len;
	} else if (is_multicast_ether_addr(eth->h_dest)) {
		qp->rq.stats.rx_multicast_frames++;
		qp->rq.stats.rx_multicast_bytes += pkt_len;
	} else {
		/* The ucast counters could also be derived from the other
		 * counters: ucast = total - mcast - bcast
		 */
		qp->rq.stats.rx_unicast_frames++;
		qp->rq.stats.rx_unicast_bytes += pkt_len;
	}
}

static void enic_update_ig_pkt_size_counters(struct enic_qp *qp,
					     u32 pkt_len)
{
	if (enic_hw_rx_stats_ok(netdev_priv(qp->netdev)))
		return;
	...
	if (pkt_len > 1518)
		qp->rq.stats.rx_frames_to_max++;
	else if (pkt_len > 1023)
		qp->rq.stats.rx_frames_1518++;
	else if (pkt_len > 511)
		qp->rq.stats.rx_frames_1023++;
	else if (pkt_len > 255)
		qp->rq.stats.rx_frames_511++;
	else if (pkt_len > 127)
		qp->rq.stats.rx_frames_255++;
	else if (pkt_len > 64)
		qp->rq.stats.rx_frames_127++;
	else
		qp->rq.stats.rx_frames_64++;
}
...
next_buf:
	...
	if (unlikely(buf->index != index)) {
		...
		netdev_warn_once(netdev,
				 "rq[%d] index mismatch: CQ desc: %u, RQ buf: %u\n",
	...
	enic_update_ig_pkt_size_counters(qp, pkt_len);

	if (enic_qp_is_admin(qp)) {
		qp->rq.stats.bytes += pkt_len;

		sender_id = CQ_DESC_VLAN_TCI(cq_desc);
		/* Since sender_id==0 means 'pkt coming from PF', the
		 * vlan_stripped counter will be zero for VFs (which can
		 * only talk to the PF).
		 */
		if (sender_id)
			qp->rq.stats.vlan_stripped++;

		enic_mbox_enqueue_rx_msg(qp,
					 (sender_id == 0) ?
					 ENIC_DST_PARENT_PF : sender_id-1,
					 skb);
		goto next_cq_desc;
	}
	...
	enic_update_ig_frame_byte_counters(qp, (struct ethhdr *)skb_mac_header(skb), pkt_len);
...
/* Unmap and free pages fragments making up the error packet. Set the
 * to_clean pointer to expect an SOP type descriptor next.
 */
static void enic_rq_error_reset_frags(struct enic_qp *qp, u16 *work)
{
...
static int enic_rq_service_page(struct enic_qp *qp, u16 budget)
{
	...
	while ((cq_desc = enic_rq_cq_get(qp, budget, work, &completed_index,
		...
		if (unlikely(buf->index != completed_index)) {
			...
			qp->rq.stats.desc_skip++;
			enic_rq_error_reset_frags(qp, &work);
			continue;
		}
		...
		/* Check for oversized multi-page packets here. All other oversized
		 * packets are dropped by the VIC and counted as pkt_truncated.
		 */
		if (skb->len <= get_max_pkt_len(qp->netdev->mtu)) {
			qp->rq.bytes += skb->len;
...
void enic_adaptive_coal(struct enic_qp *qp)
{
	 ...
	 */
	traffic <<= 3;
	traffic = div64_u64(traffic, delta);
				    ...
				    bool loopback)
{
	...
	desc->mss_loopback = cpu_to_le16(((mss & WQ_ENET_MSS_MASK) << WQ_ENET_MSS_SHIFT) |
					 (loopback ? (1 << WQ_ENET_LOOPBACK_SHIFT) : 0));
	...
	hlf = (header_length & WQ_ENET_HDRLEN_MASK) |
	       ...
	       */
	      /* The vlan tag (VLAN_TAG_INSERT) needs to be set in two cases:
	       * - when the host is sending out tagged traffic
	       * - when the egress packet is an SRIOV MBOX msg (used for PF<->VF
	       *   communication). In this case:
	       *   a) the loopback flag is set and
	       *   b) the vlan id is used as the dst vnic id (VF or parent PF).
	       * The above two cases are mutually exclusive.
	       */
	      ((loopback || vlan_tag) ? (1 << WQ_ENET_FLAGS_VLAN_TAG_INSERT_SHIFT) : 0);
				     ...
				     u16 dst_vnic)
{
	...
	/* Case 1: Stack tagged packet
	 */
	if (skb_vlan_tag_present(skb)) {
		/* DEBUG ONLY */
		if (dst_vnic) {
			netdev_err_once(qp->netdev, "%s - BUG: stack vlan and dst_vnic set at the same time\n",
					__func__);
			err = -EINVAL;
			goto drop;
		}
	...
	/* Case 2: ADMIN QP msg between PF<->VFs: the dst vnic is encoded in
	 * the vlan field
	 */
	} else if (dst_vnic) {
		vlan_tag = (dst_vnic == 0xFFFF) ? 0 : dst_vnic;
		qp->wq.stats.add_vlan++;
	}
	/* Case 3: Stack untagged packet: nothing to do
	 */

	if (mss) {
		offload_mode = WQ_ENET_OFFLOAD_MODE_TSO;
		rwe_tso = qp->enic->rwe_tso && !!(skb_shinfo(skb)->gso_type & SKB_GSO_TCPV4);
		if (rwe_tso)
			qp->wq.stats.rwe_tso_skbs++;
		tx_tstamp = false;
...
again:
	 ...
	 */
	if (rwe_tso) {
		qp->wq.stats.rwe_tso_frags++;
		qp->wq.stats.rwe_tso_bytes += skb_head_len;
	}
	...
	while (skb_head_len) {
		...
		cq_entry = eop;
		if (rwe_tso) {
			qp->wq.stats.rwe_tso_segs++;
			if (length == WQ_ENET_MAX_DESC_LEN)
				qp->wq.stats.rwe_tso_segs_16k++;
			else if (length < 512)
				qp->wq.stats.rwe_tso_segs_lt_512++;
			WARN_ONCE(mss * VNIC_RWE_TSO_MSS_MULTIPLIER > VNIC_MAX_PKT_SIZE,
				 "RWE TSO MSS too big");
			enic_enc_wq_desc(&qp->wq.wq_base[buf->index],
					 dma_addr + offset, length,
					 mss * VNIC_RWE_TSO_MSS_MULTIPLIER, header_length,
...
#define NO_DST_VNIC	0 /* pkt not addressed at a local vnic (ie, not mbox) */
...
netdev_tx_t enic_hard_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	txq_map = skb_get_queue_mapping(skb) % enic->wq_count[ENIC_DATA_QP];
	qp = &enic->qp[txq_map];
	...
	enic_update_eg_frame_byte_counters(qp, skb);

	if (enic_post_xmit_skb(qp, skb, NO_DST_VNIC))
		return NETDEV_TX_OK;
...
int enic_admin_xmit(struct sk_buff *skb, struct net_device *netdev, u16 dst_vnic)
{
	...
	if (!enic_is_res_intr_open(enic, ENIC_ADMIN_QP_INTR)) {
		netdev_err(netdev, "%s - BUG: called with the ADMIN QP closed\n",
			   __func__);
		return -1;
	}

	/* The ADMIN QP can be used only between a PF and its VFs, in both
	 * directions (not between PFs and not between VFs)
	 */
	switch (enic_get_vnic_type(enic)) {
	case ENIC_VF_V2:
		if (dst_vnic != ENIC_DST_PARENT_PF) {
			netdev_err(netdev, "%s - VF_V2 vnics can only talk to the PF (attempted TX to VF#%u)\n",
				   __func__, dst_vnic);
			return -EINVAL;
		}
		break;
	case ENIC_PF:
		if (dst_vnic >= enic->sriov.pf.num_vfs) {
			netdev_err(netdev, "%s - dst_vnic %u out of allowed range 0-%u\n",
				   __func__, dst_vnic, enic->sriov.pf.num_vfs - 1);
			return -ERANGE;
		}
		break;
	default:
		netdev_err(netdev, "%s - BUG: ADMIN QP not supported for vnic type %u\n",
			   __func__, enic_get_vnic_type(enic));
		return -EOPNOTSUPP;
	}

	qp = enic_get_qp_from_type_index(enic, ENIC_ADMIN_QP, 0);
	if (!qp) {
		netdev_err(netdev, "%s - Unable to get the ADMIN QP\n",
			   __func__);
		return -1;
	}

	if (skb->len <= 0) {
		netdev_err(netdev, "%s - pkt len = %d <= 0\n", __func__,
			   skb->len);
		qp->wq.stats.null_pkt++;
		return -1;
	}

	/* The ADMIN QP will at most use one fragment
	 */
	if (unlikely(skb_shinfo(skb)->nr_frags > 1)) {
		netdev_err(netdev, "%s - fragmented message (nr_frags = %u > 1)\n",
	...
	wq_desc_avail = enic_wq_desc_avail(qp);
	if (unlikely(wq_desc_avail == 0)) {
		netdev_err(netdev, "BUG! ADMIN Tx ring full when queue awake!\n");
		qp->wq.stats.desc_full_awake++;
		return -1;
	}

	/* The input dst_vnic param is a 0-based VF id.
	 * When the dst_vnic is the PF, dst_vnic = 0xFFFF (ENIC_DST_PARENT_PF).
	 * Since the vlan field is not used by the ADMIN QP, we use it to pass
	 * the dst_vnic param.
	 * And since vlan = 0 means "vlan not set", we change the VF ID
	 * from 0-based to 1-based when we call enic_xmit_post_skb().
	 */
	err = enic_post_xmit_skb(qp, skb,
				 (dst_vnic == ENIC_DST_PARENT_PF) ?
				 dst_vnic : (dst_vnic + 1));
	if (err)
		return err;
