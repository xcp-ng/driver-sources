...
static int __vnic_wq_alloc(struct vnic_dev *vdev, struct vnic_wq *wq,
			   unsigned int index, unsigned int desc_count,
			   unsigned int desc_size, enum vnic_res_type type)
{
	int err;
	...
	err = vnic_wq_get_ctrl(vdev, wq, index, type);
	if (err) {
...
int vnic_wq_alloc(struct vnic_dev *vdev, struct vnic_wq *wq,
		  unsigned int index, unsigned int desc_count,
		  unsigned int desc_size)
{
	return __vnic_wq_alloc(vdev, wq, index, desc_count,
			       desc_size, RES_TYPE_WQ);
}

int vnic_rdma2_data_wq_alloc(struct vnic_dev *vdev, struct vnic_wq *wq,
			     unsigned int index, unsigned int desc_count,
			     unsigned int desc_size)
{
	return __vnic_wq_alloc(vdev, wq, index, desc_count,
			       desc_size, RES_TYPE_RDMA2_DATA_WQ);
}

int vnic_rdma2_reg_wq_alloc(struct vnic_dev *vdev, struct vnic_wq *wq,
			    unsigned int index, unsigned int desc_count,
			    unsigned int desc_size)
{
	return __vnic_wq_alloc(vdev, wq, index, desc_count,
			       desc_size, RES_TYPE_RDMA2_REG_WQ);
}
