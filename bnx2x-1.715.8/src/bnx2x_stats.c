...
static void bnx2x_hw_stats_post(struct bnx2x *bp, bool b_can_sleep)
{
...
static void bnx2x_stats_pmf_update(struct bnx2x *bp, bool b_can_sleep)
{
	...
	*stats_comp = 0;
	bnx2x_hw_stats_post(bp, b_can_sleep);
	bnx2x_stats_comp(bp);
}
...
/* should be called under bnx2x_lock_stat_transition */
static void bnx2x_stats_start(struct bnx2x *bp, bool b_can_sleep)
{
	if (IS_PF(bp)) {
		if (bp->port.pmf)
...
static void bnx2x_stats_pmf_start(struct bnx2x *bp, bool b_can_sleep)
{
	bnx2x_stats_comp(bp);
	bnx2x_stats_pmf_update(bp, b_can_sleep);
	bnx2x_stats_start(bp, b_can_sleep);
}

static void bnx2x_stats_restart(struct bnx2x *bp, bool b_can_sleep)
{
...
static void bnx2x_stats_update(struct bnx2x *bp, bool b_can_sleep)
{
	u32 *stats_comp = bnx2x_sp(bp, stats_comp);
...
#endif
#else
				if (!bp->esx.tx_to_delay) {
					bnx2x_esx_trigger_grcdump(bp, ESX_CXT_STSTO_TRIGGER);
				} else {
					BNX2X_ESX_TRIGGER_TX_TO(bp, ESX_ERR_STATS_TO);
				}
#endif
			}
			return;
...
static void bnx2x_stats_stop(struct bnx2x *bp, bool b_can_sleep)
{
	bool update = false;
...
static void bnx2x_stats_do_nothing(struct bnx2x *bp, bool b_can_sleep)
{
}

static const struct {
	void (*action)(struct bnx2x *bp, bool b_can_sleep);
			...
			bool b_can_sleep)
{
	...
	bnx2x_stats_stm[state][event].action(bp, b_can_sleep);
