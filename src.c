...
#define AAC_SRC_RESET_MASK  (0x100)
...
#endif
{
	...
	if (mode & AAC_INT_MODE_SYNC) {
		...
		if (!aac_sync_mode && !dev->msix_enabled) {
			src_writel(dev, MUnit.ODR_C, bellbits);
			src_readl(dev, MUnit.ODR_C);
		}
		...
		if (dev->sync_fib) {	
			...
			if (dev->sync_fib->flags & FIB_CONTEXT_FLAG_WAIT) {
				...
				aac_complete(&dev->sync_fib->event_wait);
			}
...
{
	adbg_shut(dev, KERN_INFO, "Disable INT\n");
	...
	u32 *status, u32 * r1, u32 * r2, u32 * r3, u32 * r4)
{
	...
	adbg_fib_or_msix(dev, KERN_INFO, "%p\n", dev);
	adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", command);
	adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", p1);
	adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", p2);
	adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", p3);
	adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", p4);
...
#endif
	adbg_fib_or_msix(dev, KERN_INFO, "OutMailBoxes=\n");
	...
	if ((!dev->sync_mode || command != SEND_SYNCHRONOUS_FIB) && !dev->in_soft_reset) {
		ok = 0;
		start = jiffies;
		...
		adbg_fib_or_msix(dev, KERN_INFO, "InMailBoxes=\n");
		adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", readl(&dev->IndexRegs->Mailbox[0]));
		adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", readl(&dev->IndexRegs->Mailbox[1]));
		adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", readl(&dev->IndexRegs->Mailbox[2]));
		adbg_fib_or_msix(dev, KERN_INFO, "0x%x\n", readl(&dev->IndexRegs->Mailbox[3]));
...
static void aac_src_start_adapter(struct aac_dev *dev)
{
	...
	if (dev->comm_interface == AAC_COMM_MESSAGE_TYPE3) {
		init->r8.HostElapsedSeconds = cpu_to_le32(ktime_get_real_seconds());
		src_sync_cmd(dev, INIT_STRUCT_BASE_ADDRESS, (u32)(ulong)dev->init_pa,
			(u32)((ulong)(((dev->init_pa) >> 16) >> 16)),
			sizeof(struct _r8) + (AAC_MAX_HRRQ - 1) * sizeof(struct _rrq),
			0, 0, 0, NULL, NULL, NULL, NULL, NULL);
	} else {
		init->r7.HostElapsedSeconds = cpu_to_le32(ktime_get_real_seconds());
...
static void arc_src_take_ctrl_offline(struct aac_dev * dev) {
	...
	dev_err(&pdev->dev,"%s: controller offline\n",__FUNCTION__);
	aac_release_resources(dev);
	aac_fib_map_free(dev);
	aac_pci_free_consistent(dev->pdev, dev->comm_size, dev->comm_addr, dev->comm_phys);
	dev->comm_addr = NULL;
	dev->comm_phys = 0;
	kfree(dev->queues);
	dev->queues = NULL;
...
static int aac_src_check_health(struct aac_dev *dev)
{
	...
	if (unlikely(status & KERNEL_PANIC)) {
		aac_err(dev, "KERNEL_PANIC detected - %llx\n", status);
		if((!dev->adapter_panic)==1) {
			dev->adapter_panic=1;
			arc_src_take_ctrl_offline(dev);
		}
		goto err_blink;
	}
	...
	if (unlikely(status & SELF_TEST_FAILED)) {
		aac_err(dev, "SELF_TEST_FAILED detected - %llx\n", status);
		goto err_out;
	}

	status = src_readl(dev, MUnit.OMR);
	if(unlikely(status & MONITOR_PANIC)) {
		aac_err(dev, "MONITOR_PANIC detected - %llx\n", status);
		goto err_out;
	}
	...
	if (unlikely(!(status & KERNEL_UP_AND_RUNNING))) {
		aac_err(dev, "KERNEL not up and running - %llx\n", status);
		return -3;
	}
...
static inline u32 aac_get_msix_vector(struct fib *fib)
{
	struct aac_dev *dev = fib->dev;
	u32 vector_no = fib->vector_no;

	if(dev->comm_interface == AAC_COMM_MESSAGE_TYPE3 && dev->sa_firmware) {
		unsigned cpu = 0;
		cpu = smp_processor_id();
		if(dev->cpu_msix_tbl[cpu].is_valid)
			vector_no =  dev->cpu_msix_tbl[cpu].msix;
	}
...
static int aac_src_deliver_message(struct fib * fib)
{
	...
	if (dev->msix_enabled && dev->max_msix > 1 &&
		(native_hba || fib->hw_fib_va->header.Command != AifRequest)) {

#if (LINUX_VERSION_CODE < KERNEL_VERSION(4,11,0))
		vector_no = aac_get_msix_vector(fib);
#else
		...
		if (native_hba) {
			if (fib->flags & FIB_CONTEXT_FLAG_NATIVE_HBA_TMF) {
				if (((struct aac_hba_tm_req *)fib->hw_fib_va)->iu_type == HBA_IU_TYPE_SCSI_TM_REQ) {

					// Task Management Commands
					((struct aac_hba_tm_req *)fib->hw_fib_va)->reply_qid = vector_no;
					((struct aac_hba_tm_req *)fib->hw_fib_va)->request_id += (vector_no << 16);

				} else {
					// Reset Command
					((struct aac_hba_reset_req *)fib->hw_fib_va)->reply_qid = vector_no;
					((struct aac_hba_reset_req *)fib->hw_fib_va)->request_id += (vector_no << 16);

				}
			} else {
					
			 ((struct aac_hba_cmd_req *)fib->hw_fib_va)->reply_qid = vector_no;
			 ((struct aac_hba_cmd_req *)fib->hw_fib_va)->request_id += (vector_no << 16);
...
static void aac_clear_omr(struct aac_dev *dev)
{
	...
	if((omr_value == 0xffffffff) || (omr_value & KERNEL_PANIC))
		omr_value=0; 
	/*Attempt to preserve previous MSIX value, if any*/
	src_writel(dev, MUnit.OMR,omr_value & AAC_INT_MODE_MSIX);
	src_readl(dev, MUnit.OMR);
}

static int aac_is_ctrl_up_and_running(struct aac_dev *dev)
{
	int ctrl_up = 1;
...
static void aac_dump_fib_iop_reset(struct aac_dev *dev)
{
	...
	// Sleep for 10 seconds to allow firmware to dump the FIBs
	ssleep(10);
}
...
{
	aac_err(dev, "Issuing Controller IOP Reset\n");
	aac_adapter_sync_cmd(dev, IOP_RESET_ALWAYS,
				0, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL);
...
/*
 * aac_send_iop_reset: Issue an IOP reset
*/
static void aac_send_iop_reset(struct aac_dev *dev)
{
	aac_dump_fib_iop_reset(dev);

	aac_notify_fw_of_iop_reset(dev);

	aac_set_intx_mode(dev);
	aac_clear_omr(dev);
	src_writel(dev, MUnit.IDR, AAC_SRC_RESET_MASK);
...
static int aac_src_restart_adapter(struct aac_dev *dev, int bled, u8 reset_type)
{
	...
	if (bled)
		aac_err(dev,"Driver Init: Adapter kernel panic'd - %x\n", bled);

	/* When there is a BlinkLED, HW_IOP_RESET will not have an effect */
	if (bled >= 2 && dev->sa_firmware && reset_type & HW_IOP_RESET)
			reset_type &= ~HW_IOP_RESET;

	aac_err(dev,"Controller reset type is %d\n", reset_type);

	dev->a_ops.adapter_enable_int = aac_src_disable_interrupt;
	if (reset_type & HW_IOP_RESET) {
		aac_info(dev, "Issuing IOP reset \n");
		aac_send_iop_reset(dev);

		is_ctrl_up =  aac_is_ctrl_up_and_running(dev);
		if (!is_ctrl_up)
			aac_err(dev, "IOP reset failed\n");
		else {
        		aac_err(dev, "IOP RESET succeeded\n");
			goto set_startup;
		}
	}

	if (!dev->sa_firmware) {
		aac_err(dev,"ARC Reset attempt failed\n");
		ret = -ENODEV;
		goto set_startup;
	}

	if (reset_type & HW_SOFT_RESET) {
		aac_err(dev, "Issuing SOFT reset\n");
		aac_send_hardware_soft_reset(dev);
		dev->msix_enabled = 0;

		is_ctrl_up = aac_is_ctrl_up_and_running(dev);
		if (!is_ctrl_up) {
			aac_err(dev, "SOFT reset failed\n");
			ret = -ENODEV;
			goto out;
		} else
			aac_info(dev, "SOFT reset succeded\n");
	}
...
#	define reset_devices aac_reset_devices
	...
	if (status & KERNEL_PANIC) {
		aac_err(dev, "Driver Init: KERNEL_PANIC detected -%llx\n",status);
		if (aac_src_restart_adapter(dev, aac_src_check_health(dev), IOP_HWSOFT_RESET))
			goto error_iounmap;
		++restart;
	}
	/*
	...
	if (status & SELF_TEST_FAILED) {
		aac_err(dev, "Driver Init: SELF_TEST_FAILED detected - %llx\n",status);
		goto error_iounmap;
	}
	/*
	...
	if (status & MONITOR_PANIC) {
		aac_err(dev, "Driver Init: MONITOR_PANIC detected - %llx\n", status);
		goto error_iounmap;
	}
	start = jiffies;
	/*
	...
	while (!((status = src_readl(dev, MUnit.OMR)) & KERNEL_UP_AND_RUNNING))
	{
		if ((restart &&
		  ...
		  time_after(jiffies, start+HZ*startup_timeout)) {
			aac_err(dev, "Driver Init: Adapter INIT TIMEOUT - %llx.\n", 
				status);
			goto error_iounmap;
		}
		if (!restart &&
		  ...
		  ((startup_timeout > 60)
		    ? (startup_timeout - 60)
		    : (startup_timeout / 2))))) {
			if (likely(!aac_src_restart_adapter(dev, aac_src_check_health(dev), IOP_HWSOFT_RESET)))
				start = jiffies;
			++restart;
		}
		msleep(1);
...
static int aac_src_soft_reset(struct aac_dev * dev)
{
    ...
    dev->in_soft_reset = 1;
    dev->msix_enabled = status_omr & AAC_INT_MODE_MSIX;

    /* Get adapter properties */

    rc = aac_adapter_sync_cmd(dev, GET_ADAPTER_PROPERTIES,
    ...
    state++;
    if(!(status[1] & le32_to_cpu(AAC_OPT_EXTENDED) && (status[4] & le32_to_cpu(AAC_EXTOPT_SOFT_RESET)))) {
        rc = 2;
        goto out;
    }

    if((status[1] & le32_to_cpu(AAC_OPT_EXTENDED)) && (status[4] & le32_to_cpu(AAC_EXTOPT_SA_FIRMWARE)))
        dev->sa_firmware = 1;

    state++;
    rc = aac_adapter_sync_cmd(dev, DROP_IO,
         0,0,0,0,0,0, status+0, status+1, status+2, status+3, status+4);
    ...
    if (status[1])
        aac_err(dev, "soft_reset: %d outstanding I/O pending\n", status[1]);
...
out:
    ...
    dev->msix_enabled = 0;
    if (rc) 
        aac_err(dev, "%s, status = %d", state_str[state], rc);
    return rc;
}
...
#	define reset_devices aac_reset_devices
	...
	if (aac_adapter_ioremap(dev, dev->base_size)) {
		aac_warn(dev,"Unable to map adapter.\n");
		goto error_iounmap;
	}
	...
	if (status & FLASH_UPD_PENDING) {
		...
		do {
			...
			if (time_after(jiffies, start+HZ*FWUPD_TIMEOUT)) {
				aac_err(dev, "Driver Init: FLASH_UPD_PENDING failed-%llx\n",status);
				goto error_iounmap;
			}
	...
	aac_adapter_enable_int(dev);
	ret = aac_init_adapter(dev);
	if (ret) {
		aac_warn(dev,"aac_init_adapter failed-%d\n", ret);
		goto error_iounmap;
	}

	if (dev->comm_interface != AAC_COMM_MESSAGE_TYPE2 &&
		dev->comm_interface != AAC_COMM_MESSAGE_TYPE3) {
		aac_warn(dev, "Unkown Comm type\n");
		goto error_iounmap;
	}
	...
	ret = aac_acquire_irq(dev);
	if(ret) {
		aac_warn(dev, "aac_acquire_irq failed-%d",ret);
		goto error_iounmap;
	}
...
void aac_src_access_devreg(struct aac_dev *dev, int mode) {
	...
	case AAC_ENABLE_INTERRUPT:
		src_writel(dev, MUnit.OIMR, dev->OIMR = (dev->msix_enabled ? AAC_INT_ENABLE_TYPE1_MSIX :
...
static int aac_src_get_sync_status(struct aac_dev *dev) {
	int msix_val=0, legacy_val=0;

	msix_val = src_readl(dev,MUnit.ODR_MSI) & SRC_MSI_READ_MASK ? 1 : 0;

	if (!dev->msix_enabled) {
		/*
		* If the legacy int status indiacates cmd is NOT complete,
		* sample the MSIx register to see if that indicates cmd completion.
		* If yes - flag the contoller is in MSIx and consider the cmd complete.
		*/
