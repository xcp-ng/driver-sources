...
static bool bnxt_is_bond_active(struct net_device *bond_dev,
				struct bnxt *bp)
{
	...
	if (binfo && binfo->bond_active &&
		...
		netdev_dbg(bp->dev, "Truflow hardware LAG is enabled\n");
		return true;
	}

	netdev_dbg(bp->dev, "Truflow hardware LAG is NOT enabled\n");
	return false;
}
...
#endif
{
	...
	if (!bnxt_is_vxlan_lower_dev(netdev, bp) && !bnxt_is_bond_active(netdev, bp))
		return -EOPNOTSUPP;
...
{
	return (netif_is_vxlan(netdev) || netif_is_lag_master(netdev));
}
