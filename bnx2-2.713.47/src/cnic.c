...
unsigned int cnic_l2_buf_sz = 0x400;
module_param(cnic_l2_buf_sz, int, 0644);
MODULE_PARM_DESC(cnic_l2_buf_sz, "parameter to set ping packet size,"
...
static bool is_cdev_register = false;
...
static struct workqueue_struct *cnic_mtu_wq;
...
static void cnic_mtu_change(struct work_struct *work)
{
        struct cnic_local *cp = container_of(work, struct cnic_local, mtu_task);
	...
	rtnl_lock();
	rc = dev_set_mtu(dev->netdev, (int)cp->cp_mtu);
	rtnl_unlock();
	if (!rc)
		netdev_err(dev->netdev, "MTU Change [%d] failed\n", cp->cp_mtu);
}

static int cnic_handle_garp(struct iscsi_path *path_rsp, struct cnic_dev *dev);
				  ...
				  char *buf, u16 len)
{
	...
	case ISCSI_UEVENT_PATH_UPDATE: {
		...
		/* Handle Gratuitous ARP request for destination MAC update */
		if (path_resp->handle == CNIC_PATH_GARP_HANDLE) {
			rc = cnic_handle_garp(path_resp, dev);
			break;
		}
		...
		if (path_resp->pmtu != csk->mtu) {
			netdev_info(dev->netdev,
				    "iface MTU [%d] and netdev MTU are not equal, changing netdev MTU to %d",
				    path_resp->pmtu, csk->mtu);
			cp->cp_mtu = path_resp->pmtu;	
			INIT_WORK(&cp->mtu_task, cnic_mtu_change);
			queue_work(cnic_mtu_wq, &cp->mtu_task);
			return 0;
		}
		...
		if (cnic_in_use(csk) &&
			...
			if (test_bit(SK_F_IPV6, &csk->flags))
				...
				memcpy(&csk->src_ip[0], &path_resp->src.v4_addr,
						sizeof(struct in_addr));
				memcpy(&csk->subnet_mask[0], &cp->udev->nw_info->netmask,
						sizeof(cp->udev->nw_info->netmask));
				memcpy(&csk->gateway[0], &cp->udev->nw_info->gateway,
						sizeof(cp->udev->nw_info->gateway));
			}
...
static int cnic_get_sfp_diagnostic(struct cnic_dev *dev,
				   struct sfp_diagnostic *sfp_diag)
{
	struct cnic_local *cp = dev->cnic_priv;
	int rc = 0;

	if (cp->ethdev && cp->ethdev->drv_get_sfp_diagnostic)
		rc = cp->ethdev->drv_get_sfp_diagnostic(dev->netdev, sfp_diag);

	return rc;
}

static int cnic_get_link_error(struct cnic_dev *dev,
				struct link_error_status *lesb)
{
	struct cnic_local *cp = dev->cnic_priv;
	int rc = 0;

	if (cp->ethdev && cp->ethdev->drv_get_link_error)
		rc = cp->ethdev->drv_get_link_error(dev->netdev, lesb);
...
{
	if (udev->nw_info) {
		dma_free_coherent(&udev->pdev->dev, (sizeof(struct cnic_nw_info)),
				  udev->nw_info, udev->nw_info_map);
		udev->nw_info = NULL;
	}
...
static int __cnic_alloc_uio_rings(struct cnic_uio_dev *udev, int pages)
{
	...
	udev->l2_ring_size = pages * CNIC_PG_SZ_L2;
#ifdef _HAS_UIO_MEM_DMA
...
#ifdef _HAS_UIO_MEM_DMA
	udev->nw_info = dma_alloc_coherent(&udev->pdev->dev,
					   sizeof(struct cnic_nw_info),
					   &udev->nw_info_map, GFP_KERNEL);
#else
	udev->nw_info = dma_alloc_coherent(&udev->pdev->dev,
					   sizeof(struct cnic_nw_info),
					   &udev->nw_info_map,
...
static int cnic_init_uio(struct cnic_dev *dev)
{
	...
	uinfo->mem[4].addr = (unsigned long) udev->nw_info;
	uinfo->mem[4].dma_addr = udev->nw_info_map;
	uinfo->mem[4].size = PAGE_ALIGN(sizeof(*nw_info));
	uinfo->mem[4].memtype = UIO_MEM_DMA_COHERENT;
#else
	...
	uinfo->mem[4].addr = (unsigned long) udev->nw_info;
	uinfo->mem[4].size = sizeof(*nw_info);
	uinfo->mem[4].memtype = UIO_MEM_LOGICAL;
#endif
...
static void cnic_init_bnx2x_mac(struct cnic_dev *dev)
{
	...
	if (IS_MF_ISCSI_SD(bp) && IS_MF_BD(bp)) {
		/* replace port config mac with mf bd mac */
		memcpy(&mf_mac, &mac[0], sizeof(u16));
		mf_mac = cpu_to_be16((u16)mf_mac);
		SHMEM_WR(bp, dev_info.port_hw_config[port].iscsi_mac_upper,
			 mf_mac);
		wmb();
		mf_mac = 0;
		memcpy(&mf_mac, &mac[2], sizeof(mf_mac));
		mf_mac = cpu_to_be32(mf_mac);
		SHMEM_WR(bp, dev_info.port_hw_config[port].iscsi_mac_lower,
			 mf_mac);
	}
...
static int cnic_handle_garp(struct iscsi_path *path_rsp, struct cnic_dev *dev)
{
	...
	cp = dev->cnic_priv;
	for (l5_cid = 0; l5_cid < MAX_CM_SK_TBL_SZ; l5_cid++) {
		csk = &cp->csk_tbl[l5_cid];
		csk_hold(csk);
		if (cnic_in_use(csk) &&
				test_bit(SK_F_CONNECT_START, &csk->flags)) {

			/* find valid qedi_ep matching ip and older mac of dst */
			if (!memcmp(&path_rsp->dst.v4_addr.s_addr,
						&csk->dst_ip[0],
						sizeof(path_rsp->dst.v4_addr.s_addr)) &&
					ether_addr_equal(&path_rsp->mac_addr_old[0],
						&csk->ha[0])) {
				memset(&l5_data, 0, sizeof(l5_data));
				l5_data.remote_mac_addr.addr_hi =
					(u16) path_rsp->mac_addr[0] << 8;
				l5_data.remote_mac_addr.addr_hi |=
					path_rsp->mac_addr[1];
				l5_data.remote_mac_addr.addr_mid =
					(u16) path_rsp->mac_addr[2] << 8;
				l5_data.remote_mac_addr.addr_mid |=
					path_rsp->mac_addr[3];
				l5_data.remote_mac_addr.addr_lo =
					(u16) path_rsp->mac_addr[4] << 8;
				l5_data.remote_mac_addr.addr_lo |=
					path_rsp->mac_addr[5];

				rc = cnic_submit_kwqe_16(dev,
						L5CM_RAMROD_CMD_ID_MAC_UPDATE,
						csk->cid, ISCSI_CONNECTION_TYPE,
						&l5_data);
				if (!rc) {
					memcpy(csk->ha, path_rsp->mac_addr,
							ETH_ALEN);
					netdev_info(dev->netdev,
							"l5_cid=%u new dst mac=%pM\n",
							l5_cid, path_rsp->mac_addr);
				}
				csk_put(csk);
				break;
			}
...
static int cnic_ctl(void *data, struct cnic_ctl_info *info)
{
	...
	case CNIC_CTL_FCOE_STATS_GET_CMD:
		ulp_type = CNIC_ULP_FCOE;
#ifdef _HAS_COMPILER_GCC_HEADER
		/* fall through */
#endif
#ifdef _HAS_COMPILER_ATTRIBUTES_HEADER
		fallthrough;
#endif
				    ...
				    struct client_init_ramrod_data *data)
{
	...
	val = (u64) (ring_map + CNIC_PG_SZ_L2) >> 32;
	rxbd->addr_hi = cpu_to_le32(val);
	data->rx.bd_page_base.hi = cpu_to_le32(val);

	val = (u64) (ring_map + CNIC_PG_SZ_L2) & 0xffffffff;
	rxbd->addr_lo = cpu_to_le32(val);
	data->rx.bd_page_base.lo = cpu_to_le32(val);

	rxcqe += BNX2X_MAX_RCQ_DESC_CNT;
	val = (u64) (ring_map + (2 * CNIC_PG_SZ_L2)) >> 32;
	rxcqe->addr_hi = cpu_to_le32(val);
	data->rx.cqe_page_base.hi = cpu_to_le32(val);

	val = (u64) (ring_map + (2 * CNIC_PG_SZ_L2)) & 0xffffffff;
...
static void cnic_shutdown_rings(struct cnic_dev *dev)
{
	...
	rx_ring = udev->l2_ring + CNIC_PG_SZ_L2;
	memset(rx_ring, 0, CNIC_PG_SZ_L2);
}
				       ...
				       struct pci_dev *pdev)
{
	...
	cp = cdev->cnic_priv;
	cp->dev = cdev;
	cp->l2_single_buf_size = cnic_l2_buf_sz;
...
static struct cnic_dev *init_bnx2x_cnic(struct net_device *dev)
{
	...
	if (CNIC_SUPPORTS_FCOE(bp)) {
		...
		/* Copy fip_mac stored in shared memory */
		memcpy(cdev->fip_mac, bp->fip_mac, ETH_ALEN);
	}
...
static int __init cnic_init(void)
{
	...
	cnic_mtu_wq = create_singlethread_workqueue("cnic_mtu_wq");
	if (!cnic_mtu_wq) {
		cnic_release();
		unregister_netdevice_notifier(&cnic_netdev_notifier);
