...
lpfc_config_port_post(struct lpfc_hba *phba)
{
	...
	if (phba->cfg_hba_queue_depth > le32_to_cpu(mb->un.rd_cfg.max_xri)) {
		lpfc_printf_log(phba, KERN_WARNING, LOG_INIT,
				"3359 HBA queue depth changed from %d to %d\n",
				phba->cfg_hba_queue_depth,
				le32_to_cpu(mb->un.rd_cfg.max_xri));
		phba->cfg_hba_queue_depth = le32_to_cpu(mb->un.rd_cfg.max_xri);
	}

	phba->lmt = le32_to_cpu(mb->un.rd_cfg.lmt);
...
lpfc_sli4_refresh_params(struct lpfc_hba *phba)
{
	...
	phba->sli4_hba.pc_sli4_params.E2E_cap =
		bf_get(cfg_e2em_attr_value, mbx_sli4_parameters);

	/* Are we forcing E2E off via module parameter? */
	if (phba->cfg_enable_e2e)
		phba->sli4_hba.pc_sli4_params.E2E_attr =
			bf_get(cfg_e2em_attr_value, mbx_sli4_parameters);
	else
		phba->sli4_hba.pc_sli4_params.E2E_attr = 0;
 ...
 *
 * Note: Will skip irq polling decisions if CGN mgt is enabled.
 *
 * Return codes:
 *   None
 **/
...
lpfc_get_hba_model_desc(struct lpfc_hba *phba, uint8_t *mdp, uint8_t *descp)
{
	...
	case PCI_DEVICE_ID_LANCER_FC_VF:
		m = (typeof(m)){"LPe16000", "PCIe",
				"Obsolete, Unsupported Fibre Channel Adapter"};
		break;
#if !defined(BUILD_RHEL8) && !defined(BUILD_RHEL9)
	...
	case PCI_DEVICE_ID_LANCER_G7P_FC:
		...
		break;
	case PCI_DEVICE_ID_LANCER_G7S_FC:
		m = (typeof(m)){"LPe38104S-LP-Q", "PCIe",
...
lpfc_post_rcv_buf(struct lpfc_hba *phba)
{
	...
	lpfc_sli3_post_buffer(phba, &psli->sli3_ring[LPFC_ELS_RING],
			      LPFC_BUF_RING0);
...
lpfc_cleanup(struct lpfc_vport *vport)
{
	...
	list_for_each_entry_safe(dp, next_dp, &vport->external_dif_list,
				 listentry) {
		spin_lock_irqsave(&vport->external_dif_lock, iflags);
		list_del(&dp->listentry);
		spin_unlock_irqrestore(&vport->external_dif_lock, iflags);
		kfree(dp);
	}
}
...
lpfc_create_port(struct lpfc_hba *phba, int instance, struct device *dev)
{
	...
	if (phba->sli_rev == LPFC_SLI_REV4) {
#ifndef BUILD_MQ_ONLY
		if (!(shost_use_blk_mq(shost) && phba->cfg_enable_scsi_mq))
			phba->cfg_fcp_mq_threshold = LPFC_FCP_SQ_THRESHOLD;
#endif
		...
		/* Always hardcode 1 nr_hw_queue for SLI3 */
...
{
	int instance = 0;
	instance = idr_alloc(&lpfc_hba_index, NULL, 0, 0, GFP_KERNEL );
	if (instance < 0)
		return -1;
	return instance;
}
...
lpfc_vmid_poll(unsigned long ptr)
#else
...
#if defined(BUILD_3_10_KERN)
	time_to_tm(cur_time.tv_sec, 0, &tm_val);
#else
			      ...
			      struct lpfc_acqe_fc_la *acqe_fc)
{
	...
	phba->fc_eventTag = acqe_fc->event_tag;
	lpfc_printf_log(phba, KERN_ERR, LOG_LINK_EVENT,
			"2910 Async FC Trunking Event %d - Speed:%d\n"
			"\tLogical speed:%d "
	...
	case LPFC_SLI_EVENT_TYPE_CGN_SIGNAL:
		...
		if (cgn_signal->alarm_cnt) {
			if (phba->cgn_reg_signal == EDC_CG_SIG_WARN_ALARM) {
				...
				/* Do NOT track of alarm cnt for cgn_info
				 * here, we are ONLY tracking FPIN alarms
				 * for cgn_info.
				 */
			}
		} else if (cnt) {
			...
			if (phba->cgn_reg_signal == EDC_CG_SIG_WARN_ONLY ||
				...
				/* Do NOT track of warn cnt for cgn_info
				 * here, we are ONLY tracking FPIN warnings
				 * for cgn_info.
				 */
			}
		}
		      ...
		      struct lpfc_cgn_param *p_cgn_param, uint32_t len)
{
	...
	if (p_cgn_param->cgn_param_magic == LPFC_CFG_PARAM_MAGIC_NUM) {
		lpfc_printf_log(phba, KERN_INFO, LOG_CGN_MGMT | LOG_INIT,
				...
				"acr %d rx_burst_sz %d "
				"rx_bw_rate_max %d acr_burst_rate_max %d "
				"acr_bw_rate %d acr_burst_rate_min %d "
				"acr_bw_min %d scale %d activeMode %d\n",
				...
				p_cgn_param->cgn_param_acr_max_rxburst,
				p_cgn_param->cgn_param_acr_max_rxbw,
				p_cgn_param->cgn_param_acr_min_rxburst,
				p_cgn_param->cgn_param_acr_min_rxbw,
				p_cgn_param->cgn_param_acr_scale,
				phba->cmf_active_mode);
...
lpfc_sli4_driver_resource_setup(struct lpfc_hba *phba)
{
	...
	if (lpfc_is_vmid_enabled(phba))
#if KERNEL_VERSION(4, 15, 0) > LINUX_VERSION_CODE
		setup_timer(&phba->inactive_vmid_poll, lpfc_vmid_poll,
			    (unsigned long)phba);
#else
	...
	/* Initialise active auth configuration list */
	INIT_LIST_HEAD(&phba->lpfc_auth_active_cfg_list);
	...
	if (rc) {
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
				"2999 Get SLI4 Parameters failed\n");
		rc = -EIO;
		mempool_free(mboxq, phba->mbox_mem_pool);
		goto out_free_bsmbx;
	}
	...
	if (phba->sli3_options & LPFC_SLI3_BG_ENABLED) {
		...
		if (phba->cfg_enable_bg) {
			/*
			 * Advertise to SCSI Layer less s/g support then
			 * we configure due to DIF setup overhead.
			 */
			if (phba->cfg_sg_seg_cnt > LPFC_MAX_BG_SLI4_SEG_CNT_DIF)
			...
			/*
			 * If just External DIF is enabled, there is
			 * no protection data to deal with, so we have less
			 * SGE overhead in the SGL.
			 */
			if (phba->cfg_sg_seg_cnt > LPFC_MAX_ED_SLI4_SEG_CNT_DIF)
				phba->cfg_scsi_seg_cnt =
					LPFC_MAX_ED_SLI4_SEG_CNT_DIF;
			else
				phba->cfg_scsi_seg_cnt =
					phba->cfg_sg_seg_cnt;
		}
...
lpfc_init_iocb_list(struct lpfc_hba *phba, int iocb_count)
{
	...
	for (i = 0; i < iocb_count; i++) {
		iocbq_entry = kzalloc(sizeof(*iocbq_entry), GFP_KERNEL);
		if (!iocbq_entry) {
 ...
 * @phba: pointer to lpfc hba data structure.
 * @rd_cfg: pointer to read config data
 *
 ...
 **/
static void
lpfc_map_topology(struct lpfc_hba *phba, struct lpfc_mbx_read_config *rd_cfg)
{
	u8 ptv, tf, pt;

	ptv = (rd_cfg->ptv_tf_pt & RD_CFG_PTV) >> RD_CFG_PTV_SHIFT;
	tf = (rd_cfg->ptv_tf_pt & RD_CFG_TF) >> RD_CFG_TF_SHIFT;
	pt = (rd_cfg->ptv_tf_pt & RD_CFG_PT) >> RD_CFG_PT_SHIFT;
...
{
	LPFC_MBOXQ_t *pmb;
	struct lpfc_mbx_read_config *rd_cfg;
	...
	if (rc != MBX_SUCCESS) {
		...
		rd_cfg = &pmb->u.mqe.un.rd_cfg;
		if (rd_cfg->trunk_ldv & RD_CFG_LDV) {
			sli4_hba->lnk_info.lnk_dv = LPFC_LNK_DAT_VAL;
			sli4_hba->lnk_info.lnk_tp =
				(rd_cfg->link_type_num & RD_CFG_LINK_TYPE) >>
				RD_CFG_LINK_TYPE_SHIFT;
			sli4_hba->lnk_info.lnk_no =
				rd_cfg->link_type_num & RD_CFG_LINK_NUM;
			lpfc_printf_log(phba, KERN_INFO, LOG_SLI,
		...
		if (rd_cfg->bbscn_def) {
			...
			sli4_hba->bbscn_params.word0 =
				(rd_cfg->bbscn_def << 8) |
				rd_cfg->bbscn_max_min;
		}

		fawwpn = (rd_cfg->ext_fawwpn_fabl_wcs_acs & RD_CFG_FAWWPN) >>
			RD_CFG_FAWWPN_SHIFT;
		...
		if (fawwpn) {
			lpfc_printf_log(phba, KERN_INFO,
					...
					"configured on\n");
			sli4_hba->fawwpn_flag |= LPFC_FAWWPN_CONFIG;
		...
		sli4_hba->conf_trunk =
			(rd_cfg->trunk_ldv & RD_CFG_TRUNK) >>
			RD_CFG_TRUNK_SHIFT;
		sli4_hba->extents_in_use =
			(rd_cfg->ext_fawwpn_fabl_wcs_acs & RD_CFG_EXT) >>
			RD_CFG_EXT_SHIFT;

		max_cfg_param->max_xri = le16_to_cpu(rd_cfg->xri_count);
		/* Reduce resource usage in kdump environment */
		if (is_kdump_kernel() && max_cfg_param->max_xri > 512)
			max_cfg_param->max_xri = 512;
		max_cfg_param->xri_base = le16_to_cpu(rd_cfg->xri_base);
		max_cfg_param->max_vpi = le16_to_cpu(rd_cfg->vpi_count);
		/* Limit the max we support */
		if (max_cfg_param->max_vpi > LPFC_MAX_VPORTS)
			max_cfg_param->max_vpi = LPFC_MAX_VPORTS;
		max_cfg_param->vpi_base = le16_to_cpu(rd_cfg->vpi_base);
		max_cfg_param->max_rpi = le16_to_cpu(rd_cfg->rpi_count);
		max_cfg_param->rpi_base = le16_to_cpu(rd_cfg->rpi_base);
		max_cfg_param->max_vfi = le16_to_cpu(rd_cfg->vfi_count);
		max_cfg_param->vfi_base = le16_to_cpu(rd_cfg->vfi_base);
		max_cfg_param->max_fcfi = le16_to_cpu(rd_cfg->fcfi_count);
		max_cfg_param->max_eq = le16_to_cpu(rd_cfg->eq_count);
		max_cfg_param->max_rq = le16_to_cpu(rd_cfg->rq_count);
		max_cfg_param->max_wq = le16_to_cpu(rd_cfg->wq_count);
		max_cfg_param->max_cq = le16_to_cpu(rd_cfg->cq_count);
		phba->lmt = le16_to_cpu(rd_cfg->lmt);
		sli4_hba->next_xri = max_cfg_param->xri_base;
		phba->vpi_base = max_cfg_param->vpi_base;
		phba->vfi_base = max_cfg_param->vfi_base;
		phba->max_vpi = (max_cfg_param->max_vpi > 0) ?
				(max_cfg_param->max_vpi - 1) : 0;
		...
		if (lpfc_use_cgn_signal) {
			if (rd_cfg->ext_fawwpn_fabl_wcs_acs & RD_CFG_WCS) {
				phba->cgn_reg_signal = EDC_CG_SIG_WARN_ONLY;
				phba->cgn_reg_fpin &= ~LPFC_CGN_FPIN_WARN;
			}
			if (rd_cfg->ext_fawwpn_fabl_wcs_acs & RD_CFG_ACS) {
		...
		lpfc_printf_log(phba, KERN_INFO, LOG_SLI,
				...
				sli4_hba->extents_in_use,
				max_cfg_param->xri_base,
				max_cfg_param->max_xri,
				max_cfg_param->vpi_base,
				max_cfg_param->max_vpi,
				max_cfg_param->vfi_base,
				max_cfg_param->max_vfi,
				max_cfg_param->rpi_base,
				max_cfg_param->max_rpi,
				max_cfg_param->max_fcfi,
				max_cfg_param->max_eq,
				max_cfg_param->max_cq,
				max_cfg_param->max_wq,
				max_cfg_param->max_rq,
				phba->lmt);
		 ...
		 */
		qmin = max_cfg_param->max_wq;
		if (max_cfg_param->max_cq < qmin)
			qmin = max_cfg_param->max_cq;
		/*
		 * Reserve 4 (ELS, NVME LS, MBOX, plus one extra) and
		 * the remainder can be used for NVME / FCP.
		 */
		qmin -= 4;
		if (max_cfg_param->max_eq < qmin)
			qmin = max_cfg_param->max_eq;
	...
	if (if_type >= LPFC_SLI_INTF_IF_TYPE_2) {
		forced_link_speed = le16_to_cpu(rd_cfg->link_speed);
	...
	/* Reset the DFT_HBA_Q_DEPTH to the max xri  */
	length = max_cfg_param->max_xri - lpfc_sli4_get_els_iocb_cnt(phba);
...
lpfc_sli4_queue_setup(struct lpfc_hba *phba)
{
	...
	if (rc) {
		lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
			"4101 Failed setup of ELS WQ/CQ: rc = 0x%x\n",
			(uint32_t)rc);
		goto out_destroy;
	}
	lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
	...
	if (phba->cfg_enable_fc4_type & LPFC_ENABLE_NVME) {
		...
		if (rc) {
			lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
				"4102 Failed setup of NVVME LS WQ/CQ: "
				"rc = 0x%x\n", (uint32_t)rc);
			goto out_destroy;
		}
...
wait:
		...
		if (!port_reset) {
			...
			rc = -ENODEV;
			/* RN bit still set, try IP reset again */
			if (max_ip_retry--) {
				port_reset = 0;
				rc = 0;
				goto wait;
...
{
#if defined(BUILD_3_10_KERN)
	struct irq_desc *desc = irq_to_desc(eqhdl->irq);
...
#if defined(BUILD_3_10_KERN)
	chip->irq_set_affinity(&desc->irq_data, &eqhdl->aff_mask, 0);
#endif
	irq_set_status_flags(eqhdl->irq, IRQ_NO_BALANCING);
#ifdef BUILD_NO_IRQ_HINT
...
lpfc_get_sli4_parameters(struct lpfc_hba *phba, LPFC_MBOXQ_t *mboxq)
{
	...
	sli4_params->E2E_cap = bf_get(cfg_e2em_attr_value, mbx_sli4_parameters);
...
fcponly:
	...
	if (unlikely(rc)) {
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
				"6400 cann't set dma maximum segment size\n");
		return rc;
	}
...
#if (KERNEL_MAJOR < 5) && !defined(BUILD_RHEL8_NZ)
...
#if !defined(BUILD_3_10_KERN)
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	setup_timer(&phba->cpuhp_poll_timer, lpfc_sli4_poll_hbtimer,
		    (unsigned long)phba);
#else
...
#if defined(BUILD_3_10_KERN) /* RH7 needed for PPC */
	idx = (unsigned int)atomic_inc_return(&phba->dbg_log_idx) %
		DBG_LOG_SZ;
#else
