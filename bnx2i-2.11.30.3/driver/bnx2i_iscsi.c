/*
 * bnx2i_iscsi.c: QLogic iSCSI driver.
 *
...
static int bnx2i_split_bd(struct bnx2i_cmd *cmd, u64 addr, u32 sg_len,
			  int bd_index)
{
	...
	while (sg_len) {
		frag_size = (sg_len > BD_SPLIT_SIZE) ? 0 :
			    (sg_len % BD_SPLIT_SIZE);

		if (frag_size == 0)
			frag_size = BD_SPLIT_SIZE;

		bd[bd_index + sg_frags].buffer_addr_lo = addr & 0xffffffff;
		bd[bd_index + sg_frags].buffer_addr_hi = addr >> 32;
		bd[bd_index + sg_frags].buffer_length = frag_size;
		bd[bd_index + sg_frags].flags = 0;

		if ((sg_frags + bd_index) == 0)
			bd[bd_index + sg_frags].flags =
...
static int bnx2i_setup_free_cid_que(struct bnx2i_hba *hba)
{
	...
	mem_size = (mem_size + (CNIC_PAGE_SIZE - 1)) & CNIC_PAGE_MASK;
...
static int bnx2i_setup_mp_bdt(struct bnx2i_hba *hba)
{
	...
	hba->dummy_buffer = dma_alloc_coherent(&hba->pcidev->dev, CNIC_PAGE_SIZE,
...
struct bnx2i_hba *bnx2i_alloc_hba(struct cnic_dev *cnic)
{
	...
	hba->conn_recov_list = kmalloc(CNIC_PAGE_SIZE, GFP_KERNEL);
	if (!hba->conn_recov_list)
		goto free_dump_mem;
	hba->conn_recov_max_idx = CNIC_PAGE_SIZE / sizeof (struct iscsi_conn *) - 1;
	...
	bnx2i_rdp_stats_init(shost);
	bnx2i_login_stats_init(shost);
	return hba;
...
static int bnx2i_task_xmit(struct iscsi_task *task)
{
	...
	if (sc->sc_data_direction == DMA_TO_DEVICE) {
		...
		if (cmd->io_tbl.bd_valid > BNX2I_FW_MAX_BDS_PER_CMD) {
			printk(KERN_ERR "bnx2i: ERROR - Unsupported fw bd count 0x%x\n",
		       	       cmd->io_tbl.bd_valid);
			return -ENOMEM;
		}
	} else {
					       ...
					       int non_blocking, u32 iface_num)
{
	...
	BNX2I_DBG_LOG(hba, "CNIC connect complete for dst=%pI4.\n",
	    bnx2i_ep->cm_sk->dst_ip)
...
#ifndef _DEFINE_UNBIND_CONN_
		iscsi_suspend_queue(bnx2i_conn->cls_conn->dd_data);
#endif
	}
	hba = bnx2i_ep->hba;
...
static int bnx2i_nl_set_path(struct Scsi_Host *shost, struct iscsi_path *params)
{
	...
	if (params->ip_addr_len == 4) {
		BNX2I_DBG_LOG(hba, "Update path message src=%pI4.\n",
		    &params->src.v4_addr);
	} else if (params->ip_addr_len == 16) {
		BNX2I_DBG_LOG(hba, "Update path message src=%pI6.\n",
		    &params->src.v4_addr);
	}
...
#else
#ifdef _DEFINE_SHOST_ATTRS_
	.shost_attrs		= bnx2i_dev_attributes,
#else
	.shost_groups		= bnx2i_dev_groups,
#endif
#endif
#ifdef _DEFINE_ISCSI_CMD_
