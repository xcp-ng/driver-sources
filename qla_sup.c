...
try_fast:
	if (offset & 0xff || ha->flags.maint_mode)
		goto slow_read;
...
int qla2x00_sys_ld_info(bsg_job_t *bsg_job)
{
	...
	struct qla_lockdown_info *i;
	int rval;
	u16 mpi_ld_status[4], sz;
	u8 *f, ld_feature;
	dma_addr_t fdma;


	if (!IS_SYS_LOCKDOWN_CAPABLE(vha->hw)) {
		ql_dbg(ql_dbg_user, vha, 0x7045,
		    "%s: unsuport adapter\n", __func__);
		return -EINVAL;
	}
	if (bsg_job->reply_payload.payload_len < sizeof(*i)) {
		ql_dbg(ql_dbg_user, vha, 0x7047,
		    "%s: rsp payload too small %d < %ld\n",
		    __func__, bsg_job->reply_payload.payload_len, sizeof(i));
		return -EINVAL;
	}

	sz = max(FW_FEATURES_SIZE, (int)sizeof(*i));
	f = dma_alloc_coherent(&vha->hw->pdev->dev, sz,
			&fdma, GFP_KERNEL);
	if (!f) {
		ql_dbg(ql_dbg_user, vha, 0x702e,
		    "DMA alloc failed for feature buf.\n");
		return -ENOMEM;
	}
	i = (struct qla_lockdown_info*)f;

	rval = qla_get_features(vha, fdma, FW_FEATURES_SIZE);
	ld_feature = f[0];
	/*
	 * Check lockdown functionality supported or not
	 * before requesting lockdown feature.
	 *
	 * BIT_0 (Firmware flash Lockdown feature supported).
	 * If this bit is set, it reflects that the current level of FC
	 * firmware supports the flash lockdown feature.
	 */

	if (rval != QLA_SUCCESS || (!(ld_feature & BIT_0))) {
		ql_dbg(ql_log_warn, vha, 0x0067,
			"Lockdown features are not supported rval=%d \
			ld_feature=%u\n", rval, (u16)ld_feature);
		memset(f, 0, sz);
		rval = 0;
		goto done_release;
	...
	i->isp_fw_lockdown = ld_feature & BIT_0;

	/*
	 * BIT_1(MPI flash Lockdown feature supported). If this bit
	 * is set, it reflects that the current level of MPI firmware
	 * supports the flash lockdown feature.
	 */
	...
	rval = qla_mpipt_get_status(vha, MPIPT_LOCKDOWN_STATUS,
			mpi_ld_status, sizeof(mpi_ld_status));
	if (rval != QLA_SUCCESS) {
		ql_dbg(ql_dbg_user, vha, 0x703b,
		    "Unable to read mpi status=%x, %x, %x, %x.\n",
		    mpi_ld_status[0], mpi_ld_status[1], mpi_ld_status[2],
		    mpi_ld_status[3]);
		    rval = 0;
		goto done_release;
	}

	i->mpi_fw_lockdown.lockdown_support = mpi_ld_status[0];
	i->mpi_fw_lockdown.config_disable_flags = mpi_ld_status[1];
	i->mpi_fw_lockdown.fw_update_disable_flags = mpi_ld_status[2];
	i->mpi_fw_lockdown.mpi_disable_flags = mpi_ld_status[3];

done_release:
	bsg_reply->reply_payload_rcv_len =
		sg_copy_from_buffer(bsg_job->reply_payload.sg_list,
				    bsg_job->reply_payload.sg_cnt, i,
				    sizeof(*i));
