...
create_nodes:
	...
	ha->dfs_tgt_port_database = debugfs_create_file("tgt_port_database",
	       S_IRUSR,  ha->dfs_dir, vha, &qla2x00_dfs_tgt_port_database_fops);

	ha->dfs_fce = debugfs_create_file("fce", S_IRUSR, ha->dfs_dir, vha,
	    &dfs_fce_ops);

	ha->dfs_tgt_sess = debugfs_create_file("tgt_sess",
...
qla2x00_dfs_remove(scsi_qla_host_t *vha)
{
	...
	if (ha->dfs_tgt_port_database) {
		debugfs_remove(ha->dfs_tgt_port_database);
		ha->dfs_tgt_port_database = NULL;
	}
	...
	if (ha->dfs_tgt_sess) {
		debugfs_remove(ha->dfs_tgt_sess);
		ha->dfs_tgt_sess = NULL;
	}
