...
static void bnxt_hwrm_get_peer_bar_maps(struct bnxt *bp)
{
	struct hwrm_fw_get_structured_data_input *get;
	struct hwrm_struct_data_peer_mmap_v2 *mmap_v2;
	struct bnxt_en_dev *en_dev = bp->edev_rdma;
...
static char *bnxt_link_down_reason(struct bnxt_link_info *link_info)
{
	...
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_RF:
		return " (Remote fault)";
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_OTP_SPEED_VIOLATION:
	...
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_MODULE_FAULT:
		return " (Module fault)";
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_BMC_REQUEST:
		return " (BMC request down)";
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_TX_LASER_DISABLED:
