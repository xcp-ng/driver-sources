			      ...
			      struct sk_buff *skb, __le32 lflags, u32 kid)
{
	...
	flags |= (len << TX_BD_LEN_SHIFT) | TX_BD_TYPE_LONG_TX_BD |
		((last_frag + txr->bd_base_cnt) << TX_BD_FLAGS_BD_CNT_SHIFT);
...
static inline struct sk_buff *__bnxt_alloc_rx_frag(struct bnxt *bp,
						   dma_addr_t *mapping,
						   gfp_t gfp)
{
	struct sk_buff *skb;
...
bnxt_process_vf_flr(struct bnxt *bp, u32 data1)
{
	...
	if (!BNXT_TRUFLOW_EN(bp) || !(bp->flags & BNXT_FLAG_CHIP_P7))
		return;
	...
	rc = tfc_tbl_scope_func_reset(bp->tfp, vfid);
	if (!rc)
...
#else
		dma_unmap_single_attrs(&pdev->dev, mapping, bp->rx_buf_use_size,
				       bp->rx_dir, DMA_ATTR_WEAK_ORDERING);
...
{
	bp->flags &= ~(BNXT_FLAG_PORT_STATS | BNXT_FLAG_PORT_STATS_EXT |
		       BNXT_FLAG_ECN_STATS);
	bp->fw_cap &= ~BNXT_FW_CAP_GENERIC_STATS;
...
static int bnxt_hwrm_port_ecn_qcfg(struct bnxt *bp)
{
	...
	if (bp->fw_cap & BNXT_FW_CAP_ECN_STATS) {
...
static int bnxt_alloc_stats(struct bnxt *bp)
{
	...
	if (!BNXT_ASIC(bp) && !(bp->flags & BNXT_FLAG_CHIP_P7))
		return 0;
...
vnic_mru:
	...
	if (!vnic->vnic_id && bnxt_ulp_registered(bp->edev))
			     ...
			     u32 map_idx, struct bnxt_ring_struct *ring)
{
	...
	case BNXT_PUSH_MODE_WCB:
		...
		if ((dpi * DB_WCB_PAGE_SIZE) > (bp->db_size - bp->db_size_nc))
			return;
	...
	case BNXT_PUSH_MODE_PPP:
		...
		if ((offset + 2 * DB_PPP_SIZE) > (bp->db_size - bp->db_size_nc))
			return;
...
static int __bnxt_hwrm_func_qcaps(struct bnxt *bp)
{
	...
	if (BNXT_PF(bp) && (flags_ext & FUNC_QCAPS_RESP_FLAGS_EXT_ECN_STATS_SUPPORTED))
		bp->fw_cap |= BNXT_FW_CAP_ECN_STATS;
...
static int bnxt_hwrm_ver_get(struct bnxt *bp, bool silent)
{
	...
	if (!BNXT_ASIC(bp) && !BNXT_CHIP_P7(bp) && bp->pdev->devfn > 1) {
...
static int bnxt_hwrm_port_qstats_ext(struct bnxt *bp, u8 flags)
{
	...
	if (!rc) {
		...
		if (BNXT_FW_MAJ(bp) < 220 && !BNXT_CHIP_P7(bp) &&
...
int __bnxt_setup_vnic_p5(struct bnxt *bp, struct bnxt_vnic_info *vnic)
{
	...
	if (bp->ktls_info && bp->eswitch_mode != DEVLINK_ESWITCH_MODE_SWITCHDEV) {
...
static void bnxt_init_napi(struct bnxt *bp)
{
	...
	if (BNXT_CHIP_TYPE_NITRO_A0(bp)) {
		...
		___netif_napi_add(bp->dev, &bnapi->napi, bnxt_poll_nitroa0);
		napi_hash_add(&bnapi->napi);
	}
}
...
static void bnxt_timer(unsigned long data)
{
	...
	if (((bp->fw_cap & BNXT_FW_CAP_LPBK_STATS) || BNXT_LINK_IS_UP(bp)) &&
	    bp->stats_coal_ticks) {
		     ...
		     int tx_xdp)
{
	...
	if (!rc && pci_msix_can_alloc_dyn(bp->pdev)) {
		...
		if (hwr.cp > bp->total_irqs) {
			int total_msix = bnxt_add_msix(bp, hwr.cp);
...
static int bnxt_map_db_bar(struct bnxt *bp)
{
	...
	if ((bp->flags & BNXT_FLAG_CHIP_P5_PLUS) &&
	    bp->tx_push_mode > BNXT_PUSH_MODE_LEGACY) {
		if (bp->tx_push_mode == BNXT_PUSH_MODE_PPP)
			bp->db_size_nc = PAGE_SIZE;
		else if (BNXT_PF(bp))
			bp->db_size_nc = DB_PF_OFFSET_P5 + PAGE_SIZE;
		else
			bp->db_size_nc = DB_VF_OFFSET_P5 + PAGE_SIZE;
		if (bp->db_size <= bp->db_size_nc) {
			bp->tx_push_mode = BNXT_PUSH_MODE_NONE;
			bp->db_size_nc = bp->db_size;
		} else {
			bp->db_base_wc =
...
static bool bnxt_hwlag_fw_reset_get(struct bnxt *bp)
{
	...
	dev = netdev_master_upper_dev_get(bp->dev);
	netdev_for_each_lower_dev(dev, netdev, iter) {
		if (!bnxt_is_member_port(bp, netdev))
			continue;
		temp_bp = ((struct bnxt *)netdev_priv(netdev));
		if (test_bit(BNXT_STATE_IN_FW_RESET, &temp_bp->state))
			return true;
	}
	return false;
}
...
static void bnxt_hwrm_get_fw_lag_id(struct bnxt *bp)
{
	struct hwrm_func_qcfg_output *resp;
	struct hwrm_func_qcfg_input *req;
	int rc;
...
static void bnxt_hndl_bonding_info(struct bnxt *bp, void *ptr)
{
	struct net_device *netdev = netdev_notifier_info_to_dev(ptr);
	...
	if (e_pid == bp->pf.port_id && binfo->primary &&
	    binfo->fw_lag_id == BNXT_INVALID_LAG_ID)
		bnxt_hwrm_lag_update(bp);
}
...
static void bnxt_set_primary(struct bnxt *bp, void *ptr)
{
	...
	if (!primary_bi) {
		...
		binfo->fw_lag_id = primary_bi->fw_lag_id;
		binfo->member_port_map |= primary_bi->member_port_map;
		binfo->bond_active = primary_bi->bond_active;
	}
}
...
static int bnxt_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (BNXT_PF(bp)) {
		...
		wq_name = kasprintf(GFP_KERNEL, "fw_reset_pf_wq-%s", dev_name(&pdev->dev));
		if (!wq_name) {
			rc = -ENOMEM;
			goto init_err_pci_clean;
		}
		bp->fw_reset_pf_wq = create_singlethread_workqueue(wq_name);
		kfree(wq_name);
		if (!bp->fw_reset_pf_wq) {
					       ...
					       pci_channel_state_t state)
{
	...
	if (ptp_cfg)
		spin_lock_bh(&ptp_cfg->ptp_lock);
	if (test_and_set_bit(BNXT_STATE_IN_FW_RESET, &bp->state)) {
		netdev_err(bp->dev, "Firmware reset already in progress\n");
		abort = true;
	}
	if (ptp_cfg)
		spin_unlock_bh(&ptp_cfg->ptp_lock);
