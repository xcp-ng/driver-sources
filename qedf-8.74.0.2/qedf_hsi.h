...
struct fcoe_cmdqe_control
{
	...
	u8 cmdType;
#define FCOE_CMDQE_CONTROL_ABTSREQCMD_MASK  0x1 /* true for ABTS request cmdqe. used in Target mode */
...
struct fcoe_err_report_entry
{
	...
	__le32 tx_buf_off /* Buffer offset the beginning of the Sequence last transmitted */;
	__le32 rx_buf_off /* Buffer offset from the beginning of the Sequence last received */;
...
{
	struct scsi_bd bd_info /* BD information: Physical address and opaque data */;
...
{
	struct scsi_bd bd_info /* BD information: Physical address and opaque data */;
	__le32 buf_off /* Buffer offset the beginning of the Sequence last transmitted */;
	__le16 rx_id /* RX_ID of the associated task */;
	__le16 reserved1;
};
...
union fcoe_cqe_info
{
	...
	struct fcoe_cqe_target_info target_info /* Target completion information */;
	struct fcoe_err_report_entry err_info /* Error completion information */;
	struct fcoe_abts_info abts_info /* ABTS completion information */;
	struct fcoe_cqe_midpath_info midpath_info /* Middle path completion information */;
	struct fcoe_unsolic_info unsolic_info /* Unsolicited packet completion information */;
	struct fcoe_warning_report_entry warn_info /* Warning completion information (Rec Tov expiration) */;
};
...
{
	__le32 cqe_data;
#define FCOE_CQE_TASK_ID_MASK    0xFFFF /* The task identifier (OX_ID) to be completed */
#define FCOE_CQE_TASK_ID_SHIFT   0
#define FCOE_CQE_CQE_TYPE_MASK   0xF /* The CQE type: 0x0 Indicating on a pending work request completion. 0x1 - Indicating on an unsolicited event notification. use enum fcoe_cqe_type  (use enum fcoe_cqe_type) */
...
enum fcoe_cqe_type
{
	FCOE_GOOD_COMPLETION_CQE_TYPE /* solicited response on a R/W or middle-path SQE */,
	...
	FCOE_LOCAL_COMP_CQE_TYPE /* Task was completed wight after sending a pkt to the target */,
	MAX_FCOE_CQE_TYPE
};
...
enum fcoe_fp_error_warning_code
{
	...
	FCOE_ERROR_CODE_ABTS_REPLY_UNEXPECTED /* ABTSrsp pckt arrived unexpected */,
...
#define FCOE_RESPQE_PARAM_MASK            0xFFFFFF /* PARAM that is located in the FCP_RSP FC header */
#define FCOE_RESPQE_PARAM_SHIFT           0
#define FCOE_RESPQE_TARGET_AUTO_RSP_MASK  0xFF /* Indication whther its Target-auto-rsp mode or not */
...
{
	FCOE_ERROR_CODE_SLOW_PATH_TOO_MANY_FUNCS /* Error codes for Error Reporting in slow path flows */,
...
{
	FCOE_TASK_TX_STATE_NORMAL /* Initiate state after driver has initialized the task */,
	FCOE_TASK_TX_STATE_UNSOLICITED_COMPLETED /* Updated by TX path after complete transmitting unsolicited packet */,
	FCOE_TASK_TX_STATE_CLEAN_REQ /* Updated by TX path after start processing the task requesting the cleanup/abort operation */,
	FCOE_TASK_TX_STATE_ABTS /* Updated by TX path during abort procedure */,
	FCOE_TASK_TX_STATE_EXCLEANUP /* Updated by TX path during exchange cleanup procedure */,
	FCOE_TASK_TX_STATE_EXCLEANUP_TARGET_WRITE_CONT /* Updated by TX path during exchange cleanup continuation task procedure */,
	FCOE_TASK_TX_STATE_EXCLEANUP_TARGET_WRITE /* Updated by TX path during exchange cleanup first xfer procedure */,
	FCOE_TASK_TX_STATE_EXCLEANUP_TARGET_READ_OR_RSP /* Updated by TX path during exchange cleanup read task in Target */,
	FCOE_TASK_TX_STATE_EXCLEANUP_TARGET_WRITE_LAST_CYCLE /* Updated by TX path during target exchange cleanup procedure */,
	FCOE_TASK_TX_STATE_SEQRECOVERY /* Updated by TX path during sequence recovery procedure */,
