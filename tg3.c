 ...
 * Copyright (C) 2005-2016 Broadcom Corporation.  All Rights Reserved.
 * Copyright (C) 2016-2020 Broadcom Limited.  All Rights Reserved.
 * The term "Broadcom" refers to Broadcom Limited and/or its subsidiaries.
 * Portions Copyright (C) VMware, Inc. 2007-2010. All Rights Reserved.
 *
 * Firmware is:
 *	Derived from proprietary unpublished source code,
 *	Copyright (C) 2000-2016 Broadcom Corporation. All Rights Reserved.
 *	Copyright (C) 2016-2020 Broadcom Limited. All Rights Reserved.
 * 	The term "Broadcom" refers to Broadcom Limited and/or its subsidiaries.
 *
...
#define TG3_REVISION		"j"
#define DRV_MODULE_VERSION	\
	__stringify(TG3_MAJ_NUM) "." __stringify(TG3_MIN_NUM)\
	TG3_REVISION
#define DRV_MODULE_RELDATE	"Jan 16, 2023"
...
static int tg3_phy_reset(struct tg3 *tp)
{
	...
	/*
	*Customer specific changes, driver needs to invert our
	*Serdes PHY signal detect to meet their hardware design
	*/
#define INVERT_SIG_DET 0x8
	if (tp->phy_flags & TG3_PHYFLG_INVERT_PHY_SIG){
...
#ifndef BCM_HAS_PTPTIME64
static int tg3_ptp_gettime(struct ptp_clock_info *ptp, struct timespec *ts)
#else
static int tg3_ptp_gettime64(struct ptp_clock_info *ptp, struct timespec64 *ts)
#endif
{
	u64 ns;
	u32 remainder;
...
#else
static int tg3_ptp_settime64(struct ptp_clock_info *ptp,
...
#else
	.gettime64	= tg3_ptp_gettime64,
	.settime64	= tg3_ptp_settime64,
#endif
...
static int tg3_rx(struct tg3_napi *tnapi, int budget)
{
	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		skb_record_rx_queue(skb, (tg3_flag(tp, ENABLE_RSS) ? (tnapi->index - 1) : 0));
...
static int tg3_poll_msix(struct napi_struct *napi, int budget)
{
	...
	while (1) {
		...
		if (likely(sblk->idx[0].tx_consumer == tnapi->tx_cons &&
			...
			napi_complete_(tp->dev, napi, work_done);
			/* Reenable interrupts. */
			tw32_mailbox(tnapi->int_mbox, tnapi->last_tag << 24);
...
tx_recovery:
	...
	napi_complete_recovery(tp->dev, napi);
	tg3_reset_task_schedule(tp);
	return work_done;
}
...
{
#ifdef BCM_HAS_NETIF_TRANS_UPDATE
...
static bool tg3_tso_bug_gso_check(struct tg3_napi *tnapi, struct sk_buff *skb)
{
	 ...
	 * as gso_segs can be more then current ring size
	 */
...
#if defined(BCM_HAS_VLAN_TX_TAG_PRESENT)
	if (vlan_tx_tag_present(skb)) {
#else
	if(skb_vlan_tag_present(skb)) {
#endif
		base_flags |= TXD_FLAG_VLAN;
#if defined(BCM_HAS_VLAN_TX_TAG_GET)
...
static int tg3_reset_hw(struct tg3 *tp, bool reset_phy)
{
	...
	/* On one of the AMD platform, MRRS is restricted to 4000.
	 * Hence, driver is setting MRRS to 2048 instead of default
	 * 4096.
	 */
	if (tg3_asic_rev(tp) == ASIC_REV_5762 &&
...
#ifdef BCM_HAS_TIMER_SETUP
	timer_setup(&tp->timer, tg3_timer, 0);
#elif defined BCM_HAS_SETUP_TIMER
...
#ifndef BCM_HAS_ETHTOOL_LINK_KSETTINGS
...
#else /* BCM_HAS_ETHTOOL_LINK_KSETTINGS */
...
#endif /* BCM_HAS_ETHTOOL_LINK_KSETTINGS */
...
NICE
*/
#ifdef BCM_HAS_NDO_SIOC_PRIV
static int
tg3_siocdevprivate(struct net_device *dev, struct ifreq *ifr,
		   void __user *udata, int cmd)
#else
static int
tg3_priv_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
#endif
{
	struct tg3 *tp = netdev_priv(dev);
	struct nice_req *nrq;
...
#ifndef BCM_HAS_NDO_SIOC_PRIV
	case SIOCNICE:
		return tg3_priv_ioctl(dev, ifr, cmd);
#endif
	default:
		/* do nothing */
		break;
...
static struct ethtool_ops tg3_ethtool_ops = {
	...
	.set_eee		= tg3_set_eee,
#endif
#ifdef BCM_HAS_ETHTOOL_LINK_KSETTINGS
...
#ifndef BCM_HAS_STATS64_WITH_VOID
...
#ifdef BCM_HAS_NDO_ETH_IOCTL
	.ndo_eth_ioctl		= tg3_ioctl,
#else
	.ndo_do_ioctl		= tg3_ioctl,
#endif
#ifdef	BCM_HAS_NDO_SIOC_PRIV
	.ndo_siocdevprivate	= tg3_siocdevprivate,
#endif
	.ndo_tx_timeout		= tg3_tx_timeout,
#ifdef BCM_HAS_CHANGE_MTU_RH74
...
static void _tg3_check_invert_phy_signal(struct tg3 *tp)
{
	u32 conf_val;
	if((tg3_asic_rev(tp) == ASIC_REV_5720) &&
		(tp->phy_flags & TG3_PHYFLG_ANY_SERDES))
	{
		/*Extended Configuration starts at 0x278 NVM offset.
		*Here Bit 17 is used to inidcate if  we need to Invert
		*PHY Signal Detect (5720 only).
		*/
		tg3_nvram_read(tp, (0x278 + 4 * tp->pci_fn), &conf_val);
		if(conf_val & 0x20000)/*check 17th bit*/
		{
			tp->phy_flags |= TG3_PHYFLG_INVERT_PHY_SIG;
		}
	}
					...
					const struct pci_device_id *ent)
{
	...
	_tg3_check_invert_phy_signal(tp);
