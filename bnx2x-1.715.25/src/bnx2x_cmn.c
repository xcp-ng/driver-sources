...
_UP_STATIC void bnx2x_add_all_napi_cnic(struct bnx2x *bp)
{
	...
	for_each_rx_queue_cnic(bp, i) {
		...
		netif_napi_add_weight(bp->dev, &bnx2x_fp(bp, i, napi),
	...
	*/
	...
	if (unlikely(skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP)) {
		if (!(bp->flags & TX_TIMESTAMPING_EN)) {
			...
			bp->ptp_tx_start++;
			pr_notice_once("The device supports only a single outstanding packet to timestamp, this packet will not be timestamped\n");
		} else {
