...
_UP_STATIC int bnx2x_vfpf_teardown_queue(struct bnx2x *bp, int qidx);
...
static int bnx2x_send_msg2pf(struct bnx2x *bp, u8 *done, dma_addr_t msg_mapping)
{
	...
	if (bp->old_bulletin.valid_bitmap & 1 << CHANNEL_DOWN) {
		...
		*done = PFVF_STATUS_SUCCESS;
		return -EINVAL;
	} else if (!bp->vf2pf_valid) {
		DP(BNX2X_MSG_IOV, "VF closed yet no indication of channel down. Aborting message\n");
		*done = PFVF_STATUS_SUCCESS;
		return -EINVAL;
	}
...
int bnx2x_vfpf_config_mac(struct bnx2x *bp, u8 const *addr, u8 vf_qid, bool set)
{
				      ...
				      struct bnx2x_vf_mbx *mbx, int vfop_status)
{
	...
	bnx2x_esx_vf_request_all_vmk_gvlans(bp, vf);
#endif
...
static int bnx2x_vf_mbx_qfilters(struct bnx2x *bp, struct bnx2x_virtf *vf)
{
	...
	if (msg->flags & VFPF_SET_Q_FILTERS_MAC_VLAN_CHANGED) {
		...
		if (fl) {
			...
			rc = bnx2x_esx_set_vlan_passthru_config(bp, vf, msg);
			if (rc)
				goto op_err;
#endif
	...
	if (msg->flags & VFPF_SET_Q_FILTERS_RX_MASK_CHANGED) {
		...
		if (msg->rx_mask & VFPF_RX_MASK_ACCEPT_ANY_VLAN)
			bp->esx.vf[vf->index].any_vlan_req = 1;
		else
			bp->esx.vf[vf->index].any_vlan_req = 0;
#endif
...
#ifndef __VMKLNX__ /* BNX2X_UPSTREAM */
extern uint allow_vf_mac_change_mode;
				      ...
				      struct vfpf_set_q_filters_tlv *filters)
{
	...
	if (bulletin->valid_bitmap & 1 << MAC_ADDR_VALID) {
		...
		if (filter &&
			...
			/* ...but not if allow_vf_mac_change_mode is set.
			 * In this case, we allow VFs to change their MAC
			 * address under all conditions.
			 */
			if (allow_vf_mac_change_mode) {
				DP(BNX2X_MSG_IOV, "VF[%d] overriding MAC by set_vf_mac ndo\n",
				   vf->abs_vfid);
				bulletin->valid_bitmap &=
							~(1 << MAC_ADDR_VALID);
				memcpy(bulletin->mac, filter->mac, ETH_ALEN);
				DP(BNX2X_MSG_IOV, "ndo mac " BNX2X_MAC_FMT "\n",
				   BNX2X_MAC_PRN_LIST(bulletin->mac));

				DP(BNX2X_MSG_IOV, "req mac " BNX2X_MAC_FMT "\n",
				   BNX2X_MAC_PRN_LIST(filter->mac));
			} else {
				  ...
				  struct bnx2x_vf_mbx *mbx)
{
	...
	if (vf->state == VF_LOST) {
		 ...
		 * in case of error receovery. VFs are supposed
		 * to be timedout on waiting for PF response.
		 */
		DP(BNX2X_MSG_IOV, "VF 0x%x lost, not handling the request\n",
		   vf->abs_vfid);
		storm_memset_vf_mbx_ack(bp, vf->abs_vfid);

		/* make sure ack is written */
		mmiowb();
		return;
	}
...
int bnx2x_post_vf_bulletin(struct bnx2x *bp, int vf)
{
	...
	DP(BNX2X_MSG_IOV,
	   ...
	   BNX2X_MAC_FMT "vlan %d\n", bulletin->version, bulletin->valid_bitmap,
	   BNX2X_MAC_PRN_LIST(bulletin->mac), bulletin->vlan);
#endif
