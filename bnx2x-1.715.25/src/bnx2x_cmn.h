...
#define BNX2X_VLAN_PCP_SHIFT	13
...
int __bnx2x_setup_tc(struct net_device *dev, enum tc_setup_type type,
		     void *type_data);
#else
#ifdef _HAS_NDO_SETUP_TC_HANDLE /* BNX2X_UPSTREAM */
...
static inline int bnx2x_func_start(struct bnx2x *bp)
{
	...
	if (IS_VLAN_QINQ(bp))
		start_params->mf_mode = MULTI_FUNCTION_SD;
...
#else
	/* Enable Geneve RSS */
	if (bp->udp_tunnel_ports[BNX2X_UDP_PORT_GENEVE].dst_port)
		start_params->inner_rss = 1;
#endif
	...
	if (IS_VLAN_FILTER(bp)) {
		start_params->tx_vlan_filtering_enable = 1;
		start_params->tx_vlan_filtering_use_pvid = 1;
		start_params->sd_vlan_tag = BP_PVID(bp) |
			((u16)bp->vlan_mode_info.pcp << BNX2X_VLAN_PCP_SHIFT);
	} else if (IS_VLAN_QINQ(bp)) {
		start_params->tx_vlan_filtering_use_pvid = 1;
		start_params->sd_vlan_tag = BP_PVID(bp);
		start_params->sd_vlan_force_pri_val = bp->vlan_mode_info.pcp;
		start_params->sd_vlan_force_pri = !!bp->vlan_mode_info.pcp;
	}
...
#if defined(__VMKLNX__)  /* ! BNX2X_UPSTREAM */
	/*
	 * If offset is SGE_PAGE_SIZE, then it is guaranteed that
	 * the whole page is already unmapped and freed.
	 */
	if (pool->offset == SGE_PAGE_SIZE) {
		pool->page = NULL;
		return;
	}
#endif
					       ...
					       bnx2x_obj_type obj_type)
{
	...
	/* Configure classification DBs - designated vlans */
	if (!CHIP_IS_E1x(bp))
		bnx2x_init_vlan_obj(bp, &bnx2x_sp_obj(bp, fp).pvlan_obj,
				    fp->cl_id, fp->cid, BP_FUNC(bp),
				    bnx2x_sp(bp, pvlan_rdata),
				    bnx2x_sp_mapping(bp, pvlan_rdata),
				    BNX2X_FILTER_PVLAN_PENDING,
				    &bp->sp_state, BNX2X_OBJ_TYPE_RX_TX,
				    &bp->vlans_pool);
				     ...
				     struct bnx2x_fastpath *fp)
{
	...
	if (IS_MF_STORAGE_ONLY(bp))
		size = 0;
#endif
	txdata->tx_ring_size = IS_FCOE_FP(fp) ? MAX_TX_AVAIL : size;
...
#if !defined(__VMKLNX__) /* BNX2X_UPSTREAM */
static inline void bnx2x_start_recovery(struct bnx2x *bp)
{
	if (BNX2X_CAN_START_RECOVERY(bp)) {
		/* Disable HW inerrupts */
