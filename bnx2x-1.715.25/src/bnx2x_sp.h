...
union bnx2x_mcast_config_data {
	u8 *mac;
	u8 bin; /* used in a RESTORE/SET flows */
};
...
enum {
	...
	BNX2X_F_UPDATE_TUNNEL_INNER_CLSS_VXLAN_INNER_VNI,
	BNX2X_F_UPDATE_VLAN_FILTERING_PVID_CHNG,
};
...
#define BNX2X_VFS_VLAN(bp) (GET_NUM_VFS_PER_PATH(bp) * VF_VLAN_CREDIT_CNT)
...
#define PF_VLAN_CREDIT_E2(bp, func_num)					 \
	((MAX_VLAN_CREDIT_E2 - 1 - BNX2X_VFS_VLAN(bp)) / \
	 func_num + GET_NUM_VFS_PER_PF(bp) * VF_VLAN_CREDIT_CNT)

#define BNX2X_PF_VLAN_CREDIT_VLAN_FILTERING				256
