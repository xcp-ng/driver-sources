...
lpfc_mbx_cmpl_resume_rpi(struct lpfc_hba *phba, LPFC_MBOXQ_t *mboxq)
{
	...
	if (cmd == ELS_CMD_ADISC) {
		...
		lpfc_els_rsp_acc(vport, ELS_CMD_PLOGI, elsiocb, ndlp, NULL);
	}
	...
	if (get_job_ulpstatus(phba, cmdiocb) == 0 &&
		...
		if (vport->phba->sli_rev == LPFC_SLI_REV4) {
			...
			if (elsiocb) {
				...
				/* Save the ELS cmd.  Don't resume an
				 * unregistered RPI - unnecessary mailbox. Just
				 * send the ACC when the RPI is not registered.
				 */
				elsiocb->drvrTimeout = cmd;
				if (test_bit(NLP_RPI_REGISTERED,
					     &ndlp->nlp_flag)) {
			  ...
			  void *arg, uint32_t evt)
{
	...
	if (npr && npr->prliType == PRLI_FCP_TYPE) {
		lpfc_printf_vlog(vport, KERN_INFO,
				 ...
				 "6028 FCP NPR PRLI Cmpl Init %d Tgt %d "
				 "EIP %d AccCode x%x\n",
				 npr->initiatorFunc, npr->targetFunc,
