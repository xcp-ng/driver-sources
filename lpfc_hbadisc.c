 ...
 * in a single event.  This routine provides some protection
...
lpfc_work_list_done(struct lpfc_hba *phba)
{
	...
	while (!list_empty(&phba->work_list)) {
		...
		case LPFC_EVT_REAUTH:
			ndlp = (struct lpfc_nodelist *)(evtp->evt_arg1);
			if (!hba_pci_err) {
				lpfc_auth_timeout_handler(vport, ndlp);
				free_evt = 0;
			}
			/* decrement the node reference count held for
			 * this queued work
...
lpfc_mbx_cmpl_read_sparam(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb)
{
	...
	memcpy((uint8_t *)sp, (uint8_t *)mp->virt, sizeof(struct serv_parm));
...
/**
 * lpfc_issue_gidft - issue a GID_FT for each FC4 Type supported by the driver
 * @vport: The virtual port for which this call is being executed.
 *
 * This routine will issue a GID_FT for each FC4 Type supported by the driver.
 * ALL GID_FTs must complete before discovery is started.
 *
 * Return value :
 *   0 - Failure to issue a GID_FT
 *   n - Count of GID_FTs issued outstanding
 **/
uint32_t
...
lpfc_register_remote_port(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp)
{
	...
	if (test_bit(NLP_EXTERNAL_DIF, &ndlp->nlp_flag))
		lpfc_edsm_validate_target(vport, ndlp);
...
#if (IS_ENABLED(CONFIG_NVME_FC) || IS_ENABLED(CONFIG_NVME_TARGET_FC))
...
lpfc_unregister_remote_port(struct lpfc_nodelist *ndlp)
{
	...
	/*
	 * Cleanup all External DIF paths that match this ndlp.
	 * They will need to be revalidated if the NPort comes back.
	 */
	if (test_bit(NLP_EXTERNAL_DIF, &ndlp->nlp_flag)) {
		   ...
		   int state)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_NODE,
			 "0904 NPort state transition x%px x%06x x%lx: "
			 "%s -> %s\n",
	...
	uint32_t did)
{
	...
	INIT_LIST_HEAD(&ndlp->reauth_evt.evt_listp);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	setup_timer(&ndlp->dhc_cfg.nlp_reauthfunc, lpfc_auth_reauth,
		    (unsigned long)ndlp);
#else
	timer_setup(&ndlp->dhc_cfg.nlp_reauthfunc, lpfc_auth_reauth, 0);
#endif
...
lpfc_cleanup_node(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp)
{
	...
	list_del_init(&ndlp->reauth_evt.evt_listp);
...
/* Find a node with matching did for a given vport with host_lock held */
struct lpfc_nodelist *
...
/*
 * Find a node with matching did and get a ref on the ndlp with host_lock held.
 * Intended for contexts outside the lpfc_worker_thread where there is a risk
 * of ndlp being freed after it was looked up.
 */
struct lpfc_nodelist *
lpfc_refnode_did(struct lpfc_vport *vport, uint32_t did)
{
	struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
	struct lpfc_nodelist *ndlp;
	unsigned long iflags;
...
restart_disc:
		...
		lpfc_init_link(phba, initlinkmbox, phba->cfg_topology,
			       phba->cfg_link_speed);
		initlinkmbox->u.mb.un.init_link.lipsr_al_pa = cpu_to_le32(0);
...
/*
 * Find a node with matching WWPN and get a ref on the ndlp with host_lock held.
 * Intended for contexts outside the lpfc_worker_thread where there is a risk
 * of ndlp being freed after it was looked up.
 */
struct lpfc_nodelist *
lpfc_refnode_wwpn(struct lpfc_vport *vport, struct lpfc_name *wwpn)
{
	struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
	struct lpfc_nodelist *ndlp;
