...
#define VNIC_RES_INTR_CTRL2_STRIDE 16
	...
	struct vnic_dev_bar *bar, unsigned int num_bars)
{
	...
	while ((type = ioread8(&r->type)) != RES_TYPE_EOL) {
		...
		case RES_TYPE_RDMA2_DATA_WQ:
		case RES_TYPE_RDMA2_REG_WQ:
		case RES_TYPE_RDMA2_CQ:
		...
		case RES_TYPE_ADMIN_CQ:
			...
			break;
		case RES_TYPE_INTR_CTRL2:
			len = count * VNIC_RES_INTR_CTRL2_STRIDE;
			break;
		case RES_TYPE_DPKT:
		case RES_TYPE_MEM:
		...
		if (len + bar_offset > bar[bar_num].len) {
			pr_err("vNIC BAR%u resource %d out-of-bounds, offset 0x%x + size 0x%x > bar len 0x%lx\n",
				bar_num, type, bar_offset,
				len,
				bar[bar_num].len);
			return -EINVAL;
		}
	...
	unsigned int index)
{
	...
	case RES_TYPE_INTR_CTRL2:
		return (char __iomem *)vdev->res[type].vaddr +
			index * VNIC_RES_INTR_CTRL2_STRIDE;
	default:
		return (char __iomem *)vdev->res[type].vaddr;
	}
}
	...
	unsigned int desc_count, unsigned int desc_size)
{
	...
	unsigned int count_align = VNIC_DESC_COUNT_ALIGN;
	unsigned int desc_align = VNIC_DESC_SIZE_ALIGN;
...
/* returns true if cmd is capable
 */
...
/* returns true if RWE TSO capability is present. Bitmap of valid MSS multipliers returned
 */
int vnic_rwe_tso_info(struct vnic_dev *vdev, u64 *rwe_tso_bitmap)
{
	u64 a0 = (u32)CMD_RWE_TSO_OP;
	int wait = 1000;
	int err;

	*rwe_tso_bitmap = 0;

	err = vnic_dev_cmd(vdev, CMD_CAPABILITY, &a0, rwe_tso_bitmap, wait);
...
int vnic_dev_close_wait(struct vnic_dev *vdev, int wait)
{
	u64 a0 = 0, a1 = 0;
...
{
	return vnic_dev_close_wait(vdev, 1000);
}
...
{
	u64 a0 = (u32)arg, a1 = 0;
	/* Using 3000 instead of the default value 1000 because this devcmd
	 * takes longer to execute when SRIOV is enabled.
	 */
	int wait = 3000;
	int err;
...
int vnic_dev_add_addr_vlan(struct vnic_dev *vdev, u8 *addr, u16 vid, u16 flags)
{
	int wait = 1000;
	u64 args[3];
	int err;
	...
	err = vnic_dev_cmd_args(vdev, CMD_ADDR_VLAN_ADD, args,
				ARRAY_SIZE(args), wait);
	if (err)
		pr_err("Can't add addr [%pM/vlan_%u], %d\n", addr, vid, err);

	return err;
}

int vnic_dev_del_addr_vlan(struct vnic_dev *vdev, u8 *addr, u16 vid, u16 flags)
{
	int wait = 1000;
	u64 args[3];
	int err;
	...
	err = vnic_dev_cmd_args(vdev, CMD_ADDR_VLAN_DEL, args,
				ARRAY_SIZE(args), wait);
	if (err)
		pr_err("Can't del addr [%pM/vlan_%u], %d\n", addr, vid, err);
...
int vnic_dev_intr_coal_timer_info(struct vnic_dev *vdev)
{
	...
	if ((err == ERR_ECMDUNKNOWN) ||
		...
		pr_warn("Using default conversion factor for "
...
struct vnic_dev *vnic_dev_register(struct vnic_dev *vdev,
	...
	unsigned int num_bars)
{
	pr_warn("%s: Deprecated API: no op\n", __func__);
...
int vnic_devcmd_init(struct vnic_dev *vdev, int fallback)
{
	...
	if (p)
		...
		pr_warn("DEVCMD2 resource not found, fall back to devcmd\n");
...
void *vnic_queue_error_fill(struct qerror_s *qerr,
			    enum queue_error_type qet_type,
			    u16 len, enum queue_error_qtype qtype, u16 qid,
			    u16 err_status, u16 ndesc)
{

	struct qerror_data_s *qdata;
	struct qerror_tlv_s *qtlv;
	u_int8_t *p;

	qerr->qe_ntlvs = 0;
	p = (u_int8_t *)&qerr->qe_tlvs;
	qerr->qe_ntlvs++;

	qtlv = (struct qerror_tlv_s *)p;
	qtlv->qet_type = qet_type;
	qtlv->qet_len = len;
	p += sizeof(*qtlv);
	qdata = (struct qerror_data_s *) p;
	qdata->u.qed_v0.qed_qtype = qtype;
	qdata->u.qed_v0.qed_id = qid;
	qdata->u.qed_v0.qed_error = err_status;
	qdata->u.qed_v0.qed_ndescs = ndesc;
...
int vnic_dev_log_queue_error(struct vnic_dev *vdev, u64 buf, u32 buf_size)
{
	u64 a0, a1;
	int wait = 1000;
	...
	return vnic_dev_cmd(vdev, CMD_QUEUE_ERROR, &a0, &a1, wait);
}

static inline
void vnic_encode_vf_op(int vf_id, enum vnic_devcmd_get_set_op op, u64 *data)
{
	*data = (u64)(vf_id & 0xFFFF) | ((u64)op << 32);
}

int vnic_dev_spoofchk_op(struct vnic_dev *vdev, int vf_id,
			 enum vnic_devcmd_get_set_op op, bool *enable)
{
	int wait = 1000, ret;
	u64 a0, a1 = 0;

	vnic_encode_vf_op(vf_id, op, &a0);

	if (op == VNIC_DEVCMD_OP_SET)
		a1 = (u64)(*enable);

	ret = vnic_dev_cmd(vdev, CMD_SPOOF_CHK_OP, &a0, &a1, wait);

	if ((ret == 0) && (op == VNIC_DEVCMD_OP_GET))
		*enable = !!(a1);

	return ret;
}

int vnic_dev_local_fwd_op(struct vnic_dev *vdev, enum vnic_devcmd_get_set_op op,
			  bool *enable)
{
	int wait = 1000;
	u64 a0, a1;
	...
	vnic_encode_vf_op(SRIOV_PF_IDX, op, &a0);
	...
	err = vnic_dev_cmd(vdev, CMD_SRIOV_LOCAL_FWD_OP, &a0, &a1, wait);
	if (err == 0 && op == VNIC_DEVCMD_OP_GET)
		*enable = !!a1;

	return err;
}

int vnic_dev_vf_allowed_list_op(struct vnic_dev *vdev, int vf_id,
				...
				u64 *allowed_bitmap, int size)
{
	...
	/* The CMD_VF_ALLOWED_LIST_OP accepts and returns a 64*4 bits size
	 * bitmap
	 */
	if (size < 4)
		return -EINVAL;

	vnic_encode_vf_op(vf_id, op, &args[0]);

	if (op == VNIC_DEVCMD_OP_SET) {
		args[1] = allowed_bitmap[0];
		args[2] = allowed_bitmap[1];
		args[3] = allowed_bitmap[2];
		args[4] = allowed_bitmap[3];
	} else {
		args[1] = 0;
		args[2] = 0;
		args[3] = 0;
	...
	ret = vnic_dev_cmd_args(vdev, CMD_VF_ALLOWED_LIST_OP, args,
				ARRAY_SIZE(args), wait);

	if ((op == VNIC_DEVCMD_OP_GET) && (ret == 0)) {
		allowed_bitmap[0] = args[1];
		allowed_bitmap[1] = args[2];
		allowed_bitmap[2] = args[3];
		allowed_bitmap[3] = args[4];
	}

	return ret;
}

int vnic_dev_vf_access_vlan_op(struct vnic_dev *vdev,
			       enum vnic_devcmd_get_set_op op,
			       u16 vf_id, u16 *vlan_id)
{
	...
	if (op == VNIC_DEVCMD_OP_SET) {
		if (vlan_id == NULL) {
			pr_err("%s - vlan_id ptr is NULL\n", __func__);
			return -EINVAL;
		}

		args[1] = (u64)(*vlan_id);

		/* Setting vlan_id to 0 means 'removing' the accees vlan
		 * config'. If that's the case, we ask for a 'reset back to
		 * trunk mode' with args[2].
		 */
		if (*vlan_id == 0)
			args[2] = 1;
		else
	...
	err = vnic_dev_cmd_args(vdev, CMD_VF_ACCESS_VLAN_OP,
				args, ARRAY_SIZE(args), wait);
	if (err == 0 && op == VNIC_DEVCMD_OP_GET) {
		/* When the vlan is not set, the GET op returns 0xFFFF
		 */
		*vlan_id = (u16) (args[1] & 0xFFFF);
	}

	return err;
}

int vnic_dev_link_status_notify_mode(struct vnic_dev *vdev,
				     enum vnic_devcmd_get_set_op op,
				     u16 vf_id, bool *pf_only)
{
	int wait = 1000;
	u64 args[6];
	int err;
	...
	vnic_encode_vf_op(vf_id, op, &args[0]);
	args[1] = OPT_LINK_NOTIFY_PF_ONLY;
	...
	err = vnic_dev_cmd_args(vdev, CMD_SRIOV_OPTION_OP, args,
				ARRAY_SIZE(args), wait);
	if (err == 0 && op == VNIC_DEVCMD_OP_GET)
		*pf_only = args[2] ? true : false;

	return err;
}

int vnic_dev_qp_type_set(struct vnic_dev *vdev, u32 qp_type, u32 enable)
{
	u64 a0, a1;
	int wait = 1000;
...
int vnic_dev_sriov_stats(struct vnic_dev *vdev, struct vnic_sriov_stats **stats)
{
	...
	if (!vdev->sriov_stats) {
		vdev->sriov_stats =
			pci_alloc_consistent(vdev->pdev,
					     sizeof(struct vnic_sriov_stats),
					     &vdev->sriov_stats_pa);
		if (!vdev->sriov_stats)
			return -ENOMEM;
	...
	memset(vdev->sriov_stats, 0, sizeof(struct vnic_sriov_stats));
	a0 = (u64) vdev->sriov_stats_pa;
	a1 = (u64) sizeof(struct vnic_sriov_stats);
...
int vnic_has_admin_res(struct vnic_dev *vdev)
{
	if ((vnic_dev_get_res_count(vdev, RES_TYPE_ADMIN_WQ) == 1) &&
	    (vnic_dev_get_res_count(vdev, RES_TYPE_ADMIN_RQ) == 1) &&
	    (vnic_dev_get_res_count(vdev, RES_TYPE_ADMIN_CQ) >= 1))
		return 1;
	return 0;
}

int vnic_dev_rwe_tso_enable(struct vnic_dev *vdev, u32 mtu, u32 multiplier, u32 enable_bits)
{
	int wait = 1000;
	u64 args[4];

	args[0] = VNIC_RWE_TSO_OP_SET;
	args[1] = mtu + ETH_HLEN;	// true MSS value + ethernet header
	args[2] = multiplier;		// The RWE TSO multiplier
	args[3] = enable_bits;          // enable for IPv4 and/or IPv6

	return vnic_dev_cmd_args(vdev, CMD_RWE_TSO_OP, args, ARRAY_SIZE(args), wait);
}

int vnic_dev_rwe_tso_disable(struct vnic_dev *vdev)
{
	int wait = 1000;
	u64 args[4];

	args[0] = VNIC_RWE_TSO_OP_SET;
	args[3] = 0;                 // set all enable bits to 0 to disable
