...
static const char *const bnxt_db_error_qstats[] = {
	"express_db_dropped_misc_error",
	"express_db_dropped_rq_overflow",
	"express_db_dropped_sq overflow",
	"rx_db_drop_invalid_rq_state",
	"tx_db_drop_invalid_qp_state",
	"tx_db_drop_format_error",
};
...
static const struct {
	...
	BNXT_RX_STATS_EXT_ENTRY(rx_fec_symbol_err),
};
...
#define BNXT_PFC_WD_STATS_PRI_ENTRY(counter, n)				\
	{ BNXT_PFC_WD_STATS_OFFSET(pfc_wd_##counter) + (n),		\
	  __stringify(pfc_wd_##counter##_p##n) }

#define BNXT_PFC_WD_STATS_PRI_GROUP(n)					\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_detected, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_reverted, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_pkts, n),			\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_bytes, n),			\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_pkts_drp, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(storms_bytes_drp, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(last_storm_pkts, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(last_storm_bytes, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(last_storm_pkts_drp, n),		\
	BNXT_PFC_WD_STATS_PRI_ENTRY(last_storm_bytes_drp, n)

static const struct {
	long off;
	char str[ETH_GSTRING_LEN];
} bnxt_pfc_wd_stats_arr[] = {
	BNXT_PFC_WD_STATS_PRI_GROUP(0),
	BNXT_PFC_WD_STATS_PRI_GROUP(1),
	BNXT_PFC_WD_STATS_PRI_GROUP(2),
	BNXT_PFC_WD_STATS_PRI_GROUP(3),
	BNXT_PFC_WD_STATS_PRI_GROUP(4),
	BNXT_PFC_WD_STATS_PRI_GROUP(5),
	BNXT_PFC_WD_STATS_PRI_GROUP(6),
	BNXT_PFC_WD_STATS_PRI_GROUP(7),
};
...
#define BNXT_NUM_DB_ERROR_QSTATS	ARRAY_SIZE(bnxt_db_error_qstats)
...
#define BNXT_NUM_PFC_WD_STATS	ARRAY_SIZE(bnxt_pfc_wd_stats_arr)
...
static int bnxt_get_num_stats(struct bnxt *bp)
{
	...
	if (bp->stats_cap & BNXT_STATS_CAP_DB_ERROR)
		num_stats += BNXT_NUM_DB_ERROR_QSTATS;
	if (bp->quic_info)
	...
	if ((bp->stats_cap & BNXT_STATS_CAP_PFC_WD) &&
		...
		num_stats += BNXT_NUM_PFC_WD_STATS;
...
static int bnxt_get_db_error_qstats(struct bnxt *bp, u64 *buf, u32 idx)
{
	struct hwrm_stat_db_error_qstats_output *resp;
	struct hwrm_stat_db_error_qstats_input *req;
	int rc;

	rc = hwrm_req_init(bp, req, HWRM_STAT_DB_ERROR_QSTATS);
	if (rc)
		return rc;
	...
	if (rc)
		goto hwrm_db_error_qstats_exit;

	buf[idx++] = le32_to_cpu(resp->express_db_dropped_misc_error);
	buf[idx++] = le32_to_cpu(resp->express_db_dropped_rq_overflow);
	buf[idx++] = le32_to_cpu(resp->express_db_dropped_sq_overflow);
	buf[idx++] = le32_to_cpu(resp->rx_db_drop_invalid_rq_state);
	buf[idx++] = le32_to_cpu(resp->tx_db_drop_invalid_qp_state);
	buf[idx++] = le32_to_cpu(resp->tx_db_drop_format_error);

hwrm_db_error_qstats_exit:
...
skip_ring_stats:
	...
	if (bp->stats_cap & BNXT_STATS_CAP_DB_ERROR) {
		int rc = bnxt_get_db_error_qstats(bp, buf, j);

		if (rc)
			netdev_warn(dev,
				    "err code %d trying to get db_error_qstats\n",
				    rc);

		j += BNXT_NUM_DB_ERROR_QSTATS;
	}
	if (bp->quic_info) {
	...
	if (bp->stats_cap & BNXT_STATS_CAP_PORT_EXT) {
		...
		if (bp->pri2cos_valid) {
			...
			for (i = 0; i < 8; i++, j++) {
				u8 cos_idx = bp->tx_pri2cos_idx[i];
				long n;
...
skip_gen_stats:
	...
	if (bp->stats_cap & BNXT_STATS_CAP_PFC_WD) {
		__le64 *hw = bp->pfc_wd_stats.hw_stats;

		for (i = 0; hw && i < BNXT_NUM_PFC_WD_STATS; i++, j++)
			buf[j] = le64_to_cpu(hw[bnxt_pfc_wd_stats_arr[i].off]);
	}
...
skip_tpa_stats:
			...
			for (j = 0; j < BNXT_NUM_XSK_STATS; j++) {
				str = bnxt_xsk_stats[j];
				ethtool_sprintf(&buf, "[%d]: %s", i, str);
			}
		}
		...
		if (bp->ktls_info) {
			for (i = 0; i < BNXT_NUM_KTLS_STATS; i++)
				ethtool_puts(&buf, bnxt_ktls_stats[i]);
		}
		if (bp->quic_info) {
			for (i = 0; i < BNXT_NUM_QUIC_STATS; i++)
				ethtool_puts(&buf, bnxt_quic_stats[i]);
		}
		if (bp->stats_cap & BNXT_STATS_CAP_DB_ERROR) {
			for (i = 0; i < BNXT_NUM_DB_ERROR_QSTATS; i++) {
				strscpy(buf, bnxt_db_error_qstats[i], ETH_GSTRING_LEN);
				buf += ETH_GSTRING_LEN;
			}
		}
		if (bp->stats_cap & BNXT_STATS_CAP_PORT)
...
skip_gen_stats:
		...
		if (bp->stats_cap & BNXT_STATS_CAP_PFC_WD) {
			if (bp->pfc_wd_stats.hw_stats) {
				for (i = 0; i < BNXT_NUM_PFC_WD_STATS; i++) {
					str = bnxt_pfc_wd_stats_arr[i].str;
					ethtool_puts(&buf, str);
				}
			}
		}
...
#endif
	struct bnxt *bp = netdev_priv(dev);
	u8 __maybe_unused hds_config_mod;
...
#ifdef HAVE_ETHTOOL_RSS_CONTEXT
	if (rxfh->rss_context) {
...
#if !defined(BNXT_RSSCTX_DISABLED) && defined(HAVE_NEW_RSSCTX_INTERFACE)
...
#if !defined(HAVE_NEW_RSSCTX_INTERFACE) && defined(HAVE_ETHTOOL_RSS_CONTEXT)
	if (rxfh->rss_context)
...
static void *__bnxt_hwrm_pcie_qstats(struct bnxt *bp, struct hwrm_pcie_qstats_input *req)
{
...
static int bnxt_get_regs_len(struct net_device *dev)
{
	...
	len = BNXT_PXP_REG_LEN + bp->pcie_stat_len;

	if (bp->fw_cap_ext & BNXT_FW_CAP_MBUF_DATA)
		len = BNXT_MBUF_REG_START + BNXT_MBUF_REG_LEN;

	return len;
}

struct bnxt_reg_desc {
	u8 size;
	u16 count;
};

#define BNXT_PCIE_REG_SIZE(reg)		\
	sizeof_field(struct pcie_ctx_hw_stats_v2, pcie_##reg)

struct bnxt_reg_desc bnxt_pcie_reg[] = {
	{ BNXT_PCIE_REG_SIZE(pl_signal_integrity), 9 },
	{ BNXT_PCIE_REG_SIZE(ltssm_histogram[0]), 4 },
	{ BNXT_PCIE_REG_SIZE(recovery_histogram), 1 },
	{ BNXT_PCIE_REG_SIZE(tl_credit_nph_histogram[0]), 94 },
	{ BNXT_PCIE_REG_SIZE(posted_packet_count), 5 },
	{ BNXT_PCIE_REG_SIZE(rd_latency_histogram[0]), 14 },
};

static void bnxt_write_val(void *dst, void *src, u8 size)
{
	switch (size) {
	case 1:
		*(u8 *)dst = *(u8 *)src;
		break;
	case 2:
		*(u16 *)dst = le16_to_cpu(*(__le16 *)src);
		break;
	case 4:
		*(u32 *)dst = le32_to_cpu(*(__le32 *)src);
		break;
	case 8:
		*(u64 *)dst = le64_to_cpu(*(__le64 *)src);
		break;
	}
}

static void bnxt_copy_regs(u8 *src, u8 *dst, struct bnxt_reg_desc *desc,
			   int num_desc, int max_len)
{
	int i, j, off = 0;

	for (i = 0; i < num_desc; i++) {
		const struct bnxt_reg_desc *d = &desc[i];
		u8 size = d->size;

		for (j = 0; j < d->count && off < max_len; j++) {
			bnxt_write_val(dst + off, src + off, size);
			off += size;
		}
	}
}

#define BNXT_RX_MBUF_REG_SIZE(reg)		\
	sizeof_field(struct queue_adptv_qos_rx_info, rx_##reg)

struct bnxt_reg_desc bnxt_rx_mbuf_reg[] = {
	{ BNXT_RX_MBUF_REG_SIZE(shared_pool_cells), 2 },
	{ BNXT_RX_MBUF_REG_SIZE(cell_size), 4 },
	{ BNXT_RX_MBUF_REG_SIZE(reserved_cells_cos[0]), 40 },
	{ BNXT_RX_MBUF_REG_SIZE(shared_pool_index_cos[0]), 16 },
	{ BNXT_RX_MBUF_REG_SIZE(cos_allocate[0]), 8 },
};

#define BNXT_TX_MBUF_REG_SIZE(reg)		\
	sizeof_field(struct queue_adptv_qos_tx_info, tx_##reg)

struct bnxt_reg_desc bnxt_tx_mbuf_reg[] = {
	{ BNXT_TX_MBUF_REG_SIZE(shared_pool_cells), 2 },
	{ BNXT_TX_MBUF_REG_SIZE(cell_size), 4 },
	{ BNXT_TX_MBUF_REG_SIZE(reserved_cells_cos[0]), 24 },
	{ BNXT_TX_MBUF_REG_SIZE(shared_pool_index_cos[0]), 8 },
	{ BNXT_TX_MBUF_REG_SIZE(cos_allocate[0]), 8 },
};

static void bnxt_hwrm_rx_mbuf_data(struct bnxt *bp, u8 *reg_buf)
{
	struct hwrm_queue_adptv_qos_rx_qcfg_output *resp;
	struct hwrm_queue_adptv_qos_rx_qcfg_input *req;
	struct queue_adptv_qos_rx_info *rx_mbuf_reg;
	dma_addr_t rx_mbuf_reg_addr;
	int rc;

	if (hwrm_req_init(bp, req, HWRM_QUEUE_ADPTV_QOS_RX_QCFG))
		return;

	resp = hwrm_req_hold(bp, req);
	rx_mbuf_reg = hwrm_req_dma_slice(bp, req, sizeof(*rx_mbuf_reg),
					 &rx_mbuf_reg_addr);
	if (!rx_mbuf_reg)
		goto out;

	req->rx_qcfg_size = cpu_to_le16(sizeof(*rx_mbuf_reg));
	req->port_id = (u8)bp->pf.port_id;
	req->rx_qcfg_host_addr = cpu_to_le64(rx_mbuf_reg_addr);

	rc = hwrm_req_send(bp, req);
	if (!rc)
		bnxt_copy_regs((u8 *)rx_mbuf_reg, reg_buf, bnxt_rx_mbuf_reg,
			       ARRAY_SIZE(bnxt_rx_mbuf_reg),
			       le16_to_cpu(resp->rx_qcfg_size));
out:
	hwrm_req_drop(bp, req);
}

static void bnxt_hwrm_tx_mbuf_data(struct bnxt *bp, u8 *reg_buf)
{
	struct hwrm_queue_adptv_qos_tx_qcfg_output *resp;
	struct hwrm_queue_adptv_qos_tx_qcfg_input *req;
	struct queue_adptv_qos_rx_info *tx_mbuf_reg;
	dma_addr_t tx_mbuf_reg_addr;
	int rc;

	if (hwrm_req_init(bp, req, HWRM_QUEUE_ADPTV_QOS_TX_QCFG))
		return;

	resp = hwrm_req_hold(bp, req);
	tx_mbuf_reg = hwrm_req_dma_slice(bp, req, sizeof(*tx_mbuf_reg),
					 &tx_mbuf_reg_addr);
	if (!tx_mbuf_reg)
		goto out;

	req->tx_qcfg_size = cpu_to_le16(sizeof(*tx_mbuf_reg));
	req->port_id = (u8)bp->pf.port_id;
	req->tx_qcfg_host_addr = cpu_to_le64(tx_mbuf_reg_addr);

	rc = hwrm_req_send(bp, req);
	if (!rc)
		bnxt_copy_regs((u8 *)tx_mbuf_reg, reg_buf, bnxt_tx_mbuf_reg,
			       ARRAY_SIZE(bnxt_tx_mbuf_reg),
			       le16_to_cpu(resp->tx_qcfg_size));
out:
	hwrm_req_drop(bp, req);
}
			  ...
			  void *_p)
{
	...
	if (!(bp->stats_cap & BNXT_STATS_CAP_PCIE))
		return;
	...
	if (src) {
		...
		bnxt_copy_regs(src, dst, bnxt_pcie_reg,
			       ARRAY_SIZE(bnxt_pcie_reg), len);
	}
	hwrm_req_drop(bp, req);

	if (bp->fw_cap_ext & BNXT_FW_CAP_MBUF_DATA) {
		regs->version = 4;
		bnxt_hwrm_rx_mbuf_data(bp,
				       (u8 *)(_p + BNXT_RX_MBUF_REG_OFFSET));
		bnxt_hwrm_tx_mbuf_data(bp,
				       (u8 *)(_p + BNXT_TX_MBUF_REG_OFFSET));
	}
}
			       ...
			       struct ethtool_fec_stats *fec_stats)
{
	...
	if (BNXT_VF(bp) || !(bp->stats_cap & BNXT_STATS_CAP_PORT_EXT))
		return;
				 ...
				 struct ethtool_pause_stats *epstat)
{
	...
	if (BNXT_VF(bp) || !(bp->stats_cap & BNXT_STATS_CAP_PORT))
		return;
...
#ifdef HAVE_ETHTOOL_A_LINKSTATE_EXT_STATE
				   ...
				   struct ethtool_link_ext_state_info *info)
{
	...
	link_info = &bp->link_info;
	switch (link_info->link_down_reason) {
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_CABLE_REMOVED:
		info->link_ext_state = ETHTOOL_LINK_EXT_STATE_NO_CABLE;
		return 0;
	case PORT_PHY_QCFG_RESP_LINK_DOWN_REASON_MODULE_FAULT:
...
#if defined(HAVE_SET_MODULE_EEPROM_BY_PAGE)
...
static int bnxt_run_loopback(struct bnxt *bp)
{
	...
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS)
		cpr = rxr->rx_cpr;
	pkt_size = min(bp->dev->mtu + ETH_HLEN, max_t(u32,
						      BNXT_DEFAULT_RX_COPYBREAK,
						      bp->rx_copybreak));
...
static void bnxt_hwrm_pcie_qstats(struct bnxt *bp)
{
	...
	if (__bnxt_hwrm_pcie_qstats(bp, req))
		bp->pcie_stat_len = min_t(u16, le16_to_cpu(resp->pcie_stat_size),
...
#ifdef HAVE_CAP_RSS_CTX_SUPPORTED
