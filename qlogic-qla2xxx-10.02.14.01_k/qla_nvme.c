...
static int qla_nvme_ls_reject_iocb(struct scsi_qla_host *vha,
			struct qla_qpair *qp, struct qla_nvme_lsrjt_pt_arg *a,
...
static void qla_nvme_release_fcp_cmd_kref(struct kref *kref)
{
	...
	if (sp->type == SRB_NVME_LS) {
		ktime_get_real_ts64(&sp->cmd_to_ml);
		qla_get_nvme_cmd_latency(sp);
	}
#endif
...
static void qla_nvme_release_ls_cmd_kref(struct kref *kref)
{
	...
	fd->done(fd, sp->comp_status);
}

static void qla_nvme_ls_complete(struct work_struct *work)
{
	srb_t *sp =
		container_of(work, srb_t, ls_work);
...
static void qla_nvme_sp_ls_done(srb_t *sp, int res)
{
	...
	sp->comp_status = res;
	INIT_WORK(&sp->ls_work, qla_nvme_ls_complete);
	schedule_work(&sp->ls_work);
}
...
static void qla_nvme_release_lsrsp_cmd_kref(struct kref *kref)
{
	...
	ql_srb_trace_ext(ql_dbg_disc, sp->vha, sp->fcport,
			 "exchange_address=0x%x ox_id=0x%x",
			 nvme->u.nvme.exchange_address,
			 nvme->u.nvme.ox_id);
	if (!uctx) {
		qla2x00_rel_sp(sp);
		return;
...
{
	srb_t *sp = container_of(work, srb_t, abort_work);
	...
	ql_dbg(ql_dbg_io, fcport->vha, 0xffff,
	       "%s called for sp=%px, hndl=%x on fcport=%px desc=%px deleted=%d\n",
    ...
    struct nvme_fc_remote_port *rport, struct nvmefc_ls_req *fd)
{
	...
	INIT_WORK(&sp->abort_work, qla_nvme_abort_work);
	schedule_work(&sp->abort_work);
}
    ...
    struct nvmefc_fcp_req *fd)
{
	...
	/* If there is peer congestion, select the slow queue */
	...
	if (fd->io_dir == NVMEFC_FCP_WRITE)
		sp->dir = DMA_TO_DEVICE;
	else if (fd->io_dir == NVMEFC_FCP_READ)
		sp->dir = DMA_FROM_DEVICE;
	...
	if (IS_SCM_CAPABLE(ha)) {
		/* Throttle I/O commands only */
		if (fd->sqid) {
			throttle_down = qla2xxx_throttle_req(sp, ha, fcport, fd->io_dir);
			if (throttle_down == true) {
				return -EBUSY;
			}
		}
		qla2xxx_update_sfc_ios(sp, ha, fcport, fd->payload_length);
	}
	...
	if (rval != QLA_SUCCESS) {
		...
		qla2xxx_atomic64_sub(&ha->sfc.perf.scmr_bytes_per_period,
				   fd->payload_length);
...
static struct nvme_fc_port_template qla_nvme_fc_transport = {
	...
	.lsrqst_priv_sz = sizeof(srb_t),
	.fcprqst_priv_sz = sizeof(srb_t),
	NVME_FC_TEMPLATE_MAP_QUEUES
};
...
/*
 * Function qla2xxx_process_purls_pkt
 *
 * Description:
 * This API will Pass-up Unsolicited Received FC-NVMe Link Service pkt
 * to nvme_fc_rcv_ls_req(). LLDD need to provide memory for response
 * buffer, which will be used to reference the exchange corresponding
...
qla2xxx_process_purls_pkt(struct scsi_qla_host *vha, struct purex_item *item)
{
	...
	ret = qla_nvme_fc_rcv_ls_req(fcport->nvme_remote_port, &uctx->lsrsp,
				     &item->iocb, item->size);
	if (ret) {
