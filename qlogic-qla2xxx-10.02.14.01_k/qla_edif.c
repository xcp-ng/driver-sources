...
static struct auth_msg_code_name {
	u8 auth_msg_code;
	const char *str;
} auth_msg_code_str[] = {
	...
	{AUTH_Done,            	"AUTH_Done"},
	{DHCHAP_Challenge,     	"DHCHAP_Challenge"},
	{DHCHAP_Reply,         	"DHCHAP_Reply"},
	{DHCHAP_Success,       	"DHCHAP_Success"},
	{FCAP_Request,         	"FCAP_Request"},
	{FCAP_Acknowledge,     	"FCAP_Acknowledge"},
	{FCAP_Confirm,         	"FCAP_Confirm"},
	{FCPAP_Init,           	"FCPAP_Init"},
	{FCPAP_Accept,         	"FCPAP_Accept"},
	{FCPAP_Complete,       	"FCPAP_Complete"},
	{IKE_SA_Init ,         	"IKE_SA_Init "},
	{IKE_Auth ,            	"IKE_Auth "},
	{IKE_Create_Child_SA,  	"IKE_Create_Child_SA"},
	{IKE_Informational,    	"IKE_Informational"},
	{FCEAP_Request,        	"FCEAP_Request"},
	{FCEAP_Response,       	"FCEAP_Response"},
	{FCEAP_Success,        	"FCEAP_Success"},
	{FCEAP_Failure,        	"FCEAP_Failure"},
};

static const char* msg_code_to_str(u8 auth_msg_code)
{
	int i;
	struct auth_msg_code_name *e;

	for (i = 0; i < ARRAY_SIZE(auth_msg_code_str); i++) {
		e = auth_msg_code_str + i;
		if (auth_msg_code == e->auth_msg_code)
			return e->str;
	}
	return "unknown";
}

static void qla_edif_print_auth_hdr(struct scsi_qla_host *vha, u8 *wwpn,
	u32 src_pid, u32 dest_pid, u8 *ptr, u32 len, u32 xchgaddr)
{
	auth_els_header_t *h = (auth_els_header_t *)ptr;
	u8 lwwpn[8]= {0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff};

	if (h->els_code != ELS_AUTH_ELS)
		return;

	ql_dbg(ql_dbg_edif, vha, 0x1315,
	    "auth hdr: %8phC s:%06x d:%06x xchgaddr=%08x msg code=%s length=0x%x|0x%x transaction id=0x%x\n",
	    wwpn ? wwpn: lwwpn, src_pid, dest_pid, xchgaddr,
	    msg_code_to_str(h->message_code), len, be32_to_cpu(h->message_length),
	    be32_to_cpu(h->transaction_identifier));
...
static void qla2x00_sa_replace_iocb_timeout(qla_timer_arg_t t)
{
	struct edif_list_entry *edif_entry = qla_from_timer(edif_entry, t, timer);
...
static int qla_bsg_check(scsi_qla_host_t *vha, bsg_job_t *bsg_job,
fc_port_t *fcport)
{
	...
	if (!vha->hw->flags.edif_enabled) {
		ql_dbg(ql_dbg_edif, vha, 0x1304,
		    "%s edif not enabled\n", __func__);
		goto done;
	}
	if (DBELL_INACTIVE(vha)) {
		ql_dbg(ql_dbg_edif, vha, 0x1305,
...
qla_edif_app_check(scsi_qla_host_t *vha, struct app_id appid)
{
	...
	if (appid.app_vid != EDIF_APP_ID) {
		ql_dbg(ql_dbg_edif, vha, 0x1307, "%s app id not ok (%x)",
		    __func__, appid.app_vid);
		return false;
	}

	if (appid.version != EDIF_VERSION1) {
		ql_dbg(ql_dbg_edif, vha, 0x1308, "%s app version is not ok (%x)",
		...
		uint16_t sa_index)
{
	...
	ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1309,
	    "%s: entry\n", __func__);
...
void qla2x00_release_all_sadb(struct scsi_qla_host *vha, struct fc_port *fcport)
{
	...
	ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1312,
	    "%s: Starting...\n", __func__);
...
qla_edif_app_start(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x1314, "%s app_vid=%x app_start_flags %x\n",
	...
	if (N2N_TOPO(vha->hw)) {
		...
		list_for_each_entry_safe(fcport, tf, &vha->vp_fcports, list) {
			ql_dbg(ql_dbg_edif, vha, 0x1316,
			    "%s: sess %p %8phC lid %#04x s_id %06x logout %d\n",
			    __func__, fcport, fcport->port_name,
			    fcport->loop_id, fcport->d_id.b24,
			    fcport->logout_on_delete);

			ql_dbg(ql_dbg_edif, vha, 0x1317,
	...
	if (vha->pur_cinfo.enode_flags != ENODE_ACTIVE) {
		...
		ql_dbg(ql_dbg_edif, vha, 0x1319, "%s enode already active\n",
		     __func__);
	}
...
qla_edif_app_stop(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x131b, "%s Stopping APP: app_vid=%x\n",
...
qla_edif_app_authok(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	case PL_TYPE_DID:
		...
		if (!fcport)
			ql_dbg(ql_dbg_edif, vha, 0x1322,
			    "%s d_id lookup failed: %x\n", __func__,
			    portid.b24);
		break;
	default:
		ql_dbg(ql_dbg_edif, vha, 0x1323,
		    "%s undefined type: %x\n", __func__,
		    appplogiok.type);
		break;
	}
	...
	fcport->edif.auth_success_cnt++;
	fcport->edif.priv_auth_state = QLA_AUTH_SUCCESS;

	if (!(fcport->edif.rx_sa_set && fcport->edif.tx_sa_set)) {
		ql_dbg(ql_dbg_edif, vha, 0x1326,
		    "%s: wwpn %8phC Both SA indexes has not been SET TX %d, RX %d.\n",
		    __func__, fcport->port_name, fcport->edif.tx_sa_set,
		    fcport->edif.rx_sa_set);
		SET_DID_STATUS(bsg_reply->result, DID_OK);
...
qla_edif_app_authfail(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x1329, "%s app auth fail\n", __func__);
	...
	case PL_TYPE_DID:
		fcport = qla2x00_find_fcport_by_pid(vha, &portid);
		if (!fcport)
			ql_dbg(ql_dbg_edif, vha, 0x132a,
			    "%s d_id lookup failed: %x\n", __func__,
			    portid.b24);
		SET_DID_STATUS(bsg_reply->result, DID_OK);
		break;
	default:
		ql_dbg(ql_dbg_edif, vha, 0x132b,
		    "%s undefined type: %x\n", __func__,
		    appplogifail.type);
		bsg_job->reply_len = sizeof(struct fc_bsg_reply);
		SET_DID_STATUS(bsg_reply->result, DID_ERROR);
...
qla_edif_app_getfcinfo(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x132e, "%s app get fcinfo\n", __func__);
...
qla_edif_app_getstats(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	if (app_req.num_ports == 0) {
		ql_dbg(ql_dbg_async, vha, 0x13a9,
...
static int32_t
qla_edif_ack(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	struct fc_port *fcport;
	...
	ql_dbg(ql_dbg_edif, vha, 0x1331,
	    "%s: %06x event_code %x\n",
	    __func__, ack.port_id.b24, ack.event_code);

	/* silent unaligned access warning */
	portid.b.domain = ack.port_id.b.domain;
	portid.b.area   = ack.port_id.b.area;
	portid.b.al_pa  = ack.port_id.b.al_pa;

	fcport = qla2x00_find_fcport_by_pid(vha, &portid);
	SET_DID_STATUS(bsg_reply->result, DID_OK);

	if (!fcport) {
		ql_dbg(ql_dbg_edif, vha, 0x1332,
...
static int qla_edif_consume_dbell(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	struct fc_bsg_reply	*bsg_reply = bsg_job->reply;
...
static void __qla_edif_dbell_bsg_done(scsi_qla_host_t *vha, bsg_job_t *bsg_job,
	u32 delay)
{
	struct fc_bsg_reply *bsg_reply = bsg_job->reply;
	uint32_t result;
	...
	result = bsg_reply->result;
	reply_payload_rcv_len = bsg_reply->reply_payload_rcv_len;

	bsg_job_done(bsg_job, result, reply_payload_rcv_len);
}

static void qla_edif_dbell_bsg_done(scsi_qla_host_t *vha)
{
	unsigned long flags;
	bsg_job_t *prev_bsg_job = NULL;
...
static int
qla_edif_dbell_bsg(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	unsigned long flags;
...
qla_edif_app_mgmt(bsg_job_t *bsg_job)
{
	...
	ql_dbg(level, vha, 0x13ae, "%s vnd subcmd=%x\n",
	    __func__, vnd_sc);
	...
	case QL_VND_SC_READ_DBELL:
		...
		break;
	default:
		ql_dbg(ql_dbg_edif, vha, 0x1334, "%s unknown cmd=%x\n",
		    __func__,
	...
	int dir)
{
	...
	if (!sa_ctl) {
		/* couldn't get space */
		ql_dbg(ql_dbg_edif, fcport->vha, 0x1335,
	...
	*sap = *sa_frame;
	...
	sa_ctl->flags = 0;
	sa_ctl->state = 0L;
	ql_dbg(ql_dbg_edif, fcport->vha, 0x1336,
	...
	struct qla_sa_update_frame *sa_frame)
{
	...
	if (sa_index == RX_DELETE_NO_EDIF_SA_INDEX) {
		/* process rx delete */
		ql_dbg(ql_dbg_edif, fcport->vha, 0x1337,
	...
	if (!sa_ctl) {
		ql_dbg(ql_dbg_edif, fcport->vha, 0x133a,
	...
	ql_dbg(ql_dbg_edif, fcport->vha, 0x133b,
...
qla24xx_sadb_update(bsg_job_t *bsg_job)
{
	...
	ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x133c,
	    "%s entered, vha: 0x%px\n", __func__, vha);
	...
	if (!vha->flags.online) {
		ql_log(ql_log_warn, vha, 0x13a0, "Host is not online\n");
		rval = -EIO;
		SET_DID_STATUS(bsg_reply->result, DID_ERROR);
		goto done;
	}
	...
	if (DBELL_INACTIVE(vha)) {
		ql_log(ql_log_warn, vha, 0x13a1, "App not started\n");
		rval = -EIO;
		SET_DID_STATUS(bsg_reply->result, DID_ERROR);
		goto done;
	}
	...
	if (!found) {
		ql_dbg(ql_dbg_edif, vha, 0x133d, "Failed to find port= %06x\n",
...
qla_enode_add(scsi_qla_host_t *vha, struct enode *ptr)
{
	...
	ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1354,
	...
	struct qla_els_pt_arg *a)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x1356,
	    "Sending ELS reject ox_id %04x s:%06x -> d:%06x\n",
	    a->ox_id, a->sid.b24, a->did.b24);
	ql_dump_buffer(ql_dbg_edif + ql_dbg_verbose, vha, 0x1357,
...
static void qla_edb_clear(scsi_qla_host_t *vha, port_id_t portid)
{
	...
	list_for_each_entry_safe(e, tmp, &vha->e_dbell.head, list) {
		...
		case VND_CMD_AUTH_STATE_SAUPDATE_COMPL:
			...
			continue;
		default:
			ql_log(ql_log_warn, vha, 0x13a6,
			       "%s unknown node type: %x\n", __func__, e->ntype);
			sid.b24 = 0;
			break;
		}
		if (sid.b24 == portid.b24) {
			ql_dbg(ql_dbg_edif, vha, 0x135a,
	...
	uint32_t data, uint32_t data2, fc_port_t	*sfcport)
{
	...
	case VND_CMD_AUTH_STATE_NEEDED:
		...
		fcport->edif.priv_auth_state = QLA_AUTH_NEEDED;
		break;
	case VND_CMD_AUTH_STATE_SESSION_SHUTDOWN:
		edbnode->u.plogi_did.b24 = fcport->d_id.b24;
		fcport->edif.priv_auth_state = QLA_AUTH_TERMINATE;
		break;
	case VND_CMD_AUTH_STATE_ELS_RCVD:
		edbnode->u.els_sid.b24 = fcport->d_id.b24;
		if (fcport->disc_state == DSC_LOGIN_AUTH_PEND)
			fcport->edif.priv_auth_state = QLA_AUTH_PENDING;
		break;
	case VND_CMD_AUTH_STATE_SAUPDATE_COMPL:
	...
	if (edbnode) {
		...
		spin_lock_irqsave(&vha->e_dbell.db_lock, flags);
		list_add_tail(&edbnode->list, &vha->e_dbell.head);
		spin_unlock_irqrestore(&vha->e_dbell.db_lock, flags);

		ql_dbg(ql_dbg_edif, vha, 0x1365,
	...
	if (!vha->vp_idx && N2N_TOPO(ha) && ha->flags.n2n_fw_acc_sec) {
		if (DBELL_INACTIVE(vha) &&
			...
			if (ha->edif_post_stop_cnt_down == 0) {
				ql_dbg(ql_dbg_async, vha, 0x139b,
...
qla24xx_issue_sa_replace_iocb(scsi_qla_host_t *vha, struct qla_work_evt *e)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x1366,
	    "%s: starting,  sa_ctl: %px\n", __func__, sa_ctl);

	if (!sa_ctl) {
		ql_dbg(ql_dbg_edif, vha, 0x1367,
...
void qla24xx_auth_els(scsi_qla_host_t *vha, void **pkt, struct rsp_que **rsp)
{
	...
	if (!vha->hw->flags.edif_enabled) {
		...
		ql_dbg(ql_dbg_edif, vha, 0x1372, "%s edif not enabled\n",
		    __func__);
		qla_els_reject_iocb(vha, (*rsp)->qpair, &a);
		__qla_consume_iocb(vha, pkt, rsp);
		return;
	...
	if (!host) {
		ql_log(ql_log_fatal, vha, 0x13ac,
	...
	if (DBELL_INACTIVE(vha)) {
		ql_dbg(ql_dbg_edif, host, 0x1374, "%s e_dbell.db_flags =%x %06x\n",
	...
	qla_edif_print_auth_hdr(host, fcport->port_name, purex->pur_info.pur_sid.b24,
	    purex->pur_info.pur_did.b24, purex->msgp, purex->pur_info.pur_bytes_rcvd,
	    purex->pur_info.pur_rx_xchg_address);
...
static uint16_t  qla_edif_get_sa_index_from_freepool(fc_port_t *fcport, int dir)
{
	...
	ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1376,
	    "%s: entry\n", __func__);
	...
	struct sa_update_28xx *pkt)
{
	...
	case 0:
		action = "UPDATE RX key";
		break;
	case 1:
		action = "DELETE RX key";
		break;
	case 2:
		action = "UPDATE TX key";
		break;
	case 3:
		action = "DELETE TX key";
		break;
	}
	...
	if (pkt->u.comp_sts)
		ql_dbg(ql_dbg_edif, vha, 0x137f,
		    "%8phC %s failed status=%x old_sa_info=%x new_sa_info=%x lid %d, index=0x%x "
		    "pkt_flags %xh hdl=%x\n",
		    sp->fcport->port_name, action, pkt->u.comp_sts, pkt->old_sa_info, pkt->new_sa_info,
		    nport_handle, pkt->sa_index, pkt->flags, sp->handle);
	else
		ql_dbg(ql_dbg_edif, vha, 0x1380,
		    "%8phC %s success old_sa_info=%x new_sa_info=%x lid %d, index=0x%x "
		    "pkt_flags %xh hdl=%x\n",
	...
	if ((pkt->flags & (SA_FLAG_INVALIDATE | SA_FLAG_TX)) ==  SA_FLAG_INVALIDATE) {
		...
		if (edif_entry) {
			ql_dbg(ql_dbg_edif, vha, 0x1381,
			    "%s: removing edif_entry %px, new sa_index: 0x%x\n",
			    __func__, edif_entry, pkt->sa_index);
			qla_edif_list_delete_sa_index(sp->fcport, edif_entry);
			del_timer(&edif_entry->timer);

			ql_dbg(ql_dbg_edif, vha, 0x1382,
	...
	if (sp->flags & SRB_EDIF_CLEANUP_DELETE) {
		 ...
		 */
		ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1384,
	...
	if (pkt->flags & SA_FLAG_INVALIDATE) {
		...
		if (sa_ctl &&
			...
			ql_dbg(ql_dbg_edif + ql_dbg_verbose, vha, 0x1386,
			    "%s: freeing sa_ctl for index %d\n",
			    __func__, sa_ctl->index);
			qla_edif_free_sa_ctl(sp->fcport, sa_ctl, sa_ctl->index);
		} else {
			ql_dbg(ql_dbg_edif, vha, 0x1387,
			    "%s: sa_ctl NOT freed, sa_ctl: %px\n",
			    __func__, sa_ctl);
		}
		ql_dbg(ql_dbg_edif, vha, 0x1388,
		    "%s: freeing sa_index %d, nph: 0x%x\n",
	 ...
	 */
		...
		case CS_PORT_EDIF_LOGOUT:
			qla_schedule_sess_for_deletion(sp->fcport);
			break;
		default:
			break;
		}
...
qla28xx_start_scsi_edif(srb_t *sp)
{
	...
	if (qla_get_buf(vha, sp->qpair, &sp->u.scmd.buf_dsc)) {
		ql_log(ql_log_fatal, vha, 0x13b1,
		    "Failed to allocate buf for fcp_cmnd for cmd=%px.\n", cmd);
		goto queuing_error;
	}
	...
	if (cmd->cmd_len > 16) {
		...
		if ((cmd->cmd_len % 4) != 0) {
			 ...
			 * multiple of 4
			 */
			ql_log(ql_log_warn, vha, 0x13b2,
			    "scsi cmd len %d not multiple of 4 for cmd=%px.\n",
			    cmd->cmd_len, cmd);
		...
		struct qla_sa_update_frame *sa_frame)
{
	...
	sa_index = qla_edif_get_sa_index_from_freepool(fcport, dir);
	if (sa_index == INVALID_EDIF_SA_INDEX) {
		ql_dbg(ql_dbg_edif, fcport->vha, 0x1390,
		    "%s: empty freepool!!\n", __func__);
		return INVALID_EDIF_SA_INDEX;
	}
	...
	spin_unlock_irqrestore(&ha->sadb_lock, flags);
	ql_dbg(ql_dbg_edif, fcport->vha, 0x1391,
...
int qla_edif_sadb_build_free_pool(struct qla_hw_data *ha)
{
	...
	if (!ha->edif_tx_sa_id_map) {
		ql_log_pci(ql_log_fatal, ha->pdev, 0x13a7,
	...
	if (!ha->edif_rx_sa_id_map) {
		...
		ql_log_pci(ql_log_fatal, ha->pdev, 0x13a8,
...
int qla_edif_process_els(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	...
	if (!vha->flags.online) {
		ql_log(ql_log_warn, vha, 0x13a2, "Host not online.\n");
		SET_DID_STATUS(bsg_reply->result, DID_BAD_TARGET);
		rval = -EIO;
		goto done;
	}
...
retry:
	...
	case QLA_SUCCESS:
		...
		if (p->e.sub_cmd == SEND_ELS)
			qla_edif_print_auth_hdr(vha, fcport->port_name, vha->d_id.b24,
			    fcport->d_id.b24, sp->remap.req.buf, sp->remap.req.len, 0xffffffff);
		else
			ql_dbg(ql_dbg_edif, vha, 0x1399,
