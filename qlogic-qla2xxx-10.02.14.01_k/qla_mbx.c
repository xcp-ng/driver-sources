...
static struct mb_cmd_name {
	...
	{MBC_MPI_PASSTHROUGH,		"MpiPT"},
	{MBC_GET_RNID_PARAMS,		"GetParm"},
};
    ...
    uint32_t risc_code_size)
{
	...
	if (rval != QLA_SUCCESS) {
		...
		vha->hw_err_cnt++;
		if (LOCKDOWN_ERROR(vha->hw, mcp->mb[1]))
			rval = QLA_FLASH_LOCKDOWN;
	} else {
...
static void qla_print_mb(scsi_qla_host_t *vha, u16 command)
{
	uint16_t mb[32], *p, i;
	struct qla_hw_data *ha = vha->hw;
	device_reg_t *reg = ha->iobase;;

	p = &reg->isp24.mailbox0;

	for (i = 0; i < 32; i++, p++)
		mb[i] = rd_reg_word(p);

	ql_log(ql_log_warn, vha, 0xd03b,
	    "Cmd=%x completion.\n", command);

	ql_log(ql_log_warn, vha, 0xd03d,
	    "mb[ 0- 7]=[%04x %04x %04x %04x | %04x %04x %04x %04x] \n",
	    mb[0], mb[1], mb[2], mb[3], mb[4], mb[5], mb[6], mb[7]);

	ql_log(ql_log_warn, vha, 0xd03f,
	    "mb[ 8-15]=[%04x %04x %04x %04x | %04x %04x %04x %04x] \n",
	    mb[8], mb[9] , mb[10], mb[11], mb[12], mb[13], mb[14], mb[15]);

	ql_log(ql_log_warn, vha, 0xd040,
	    "mb[16-23]=[%04x %04x %04x %04x | %04x %04x %04x %04x] \n",
	    mb[16], mb[17], mb[18], mb[19], mb[20], mb[21], mb[22], mb[23]);

	ql_log(ql_log_warn, vha, 0xd044,
	    "mb[24-31]=[%04x %04x %04x %04x | %04x %04x %04x %04x] \n",
...
again:
	...
	if (rval != QLA_SUCCESS) {
		if (IS_QLA28XX(ha) && mcp->mb[0] == MBS_COMMAND_ERROR &&
		    mcp->mb[1] == 0x27 && retry) {
			ql_dbg(ql_dbg_async, vha, 0x1026,
			    "Exe FW: retry=%d force semaphore[%d].\n",
			    retry, semaphore);
...
qla2x00_get_fw_version(scsi_qla_host_t *vha)
{
	...
	if (IS_FWI2_CAPABLE(ha)) {
		...
		if (ha->fw_attributes_ext[0] & FW_ATTR_EXT0_SCM_SUPPORTED)
			ql_dbg(ql_dbg_mbx + ql_dbg_verbose, vha, 0x1191,
			    "%s: Firmware supports SCM 0x%x\n",
	...
	if (IS_QLA27XX(ha) || IS_QLA28XX(ha)) {
		...
		if(ha->flags.scm_supported_a &&
			...
			if (ha->sf_init_cb) {
				ha->sf_init_cb->flags |= cpu_to_le16(SCM_SUPPORT);
				/* Virtual Lane support */
				if (ha->flags.scm_supported_vl == 1) {
					ha->sf_init_cb->flags |= cpu_to_le16(SCM_PUN_SUPPORT);
					ha->sf_init_cb->flags |= cpu_to_le16(SCM_VL_SUPPORT);
					ql_log(ql_log_info, vha, 0x11a2,
						    "Driver supports Virtual Lane handling \n");
				}
				if (ql2xcontrol_edc_rdf) {
					ha->sf_init_cb->flags |= cpu_to_le16(SCM_DRIVER_CTRL_ELS);
					ql_log(ql_log_info, vha, 0xffff,
						    "Driver supports sending RDF and EDC ELS \n");
				}
			}
		}
	...
	if (rval != QLA_SUCCESS) {
		...
		if (IS_QLA27XX(vha->hw) || IS_QLA28XX(vha->hw)) {
			...
			if (mcp->mb[7] & SCM_RDF_ACC_RECEIVED) {
				...
				vha->hw->scm.scm_fabric_connection_flags |=
	...
	struct vp_rpt_id_entry_24xx *rptid_entry)
{
	...
	if (rptid_entry->format == 0) {
		...
		if ((ha->flags.flogi_acc_enabled == 1) &&
		   (rptid_entry->vp_idx == 0) &&
		   (rptid_entry->vp_status == 0 ||
		    rptid_entry->vp_status == 2)) {
			if (ha->flags.flogi_acc_pl_in_cont_iocb == 0) {
				ha->flags.flogi_acc_pl_in_cont_iocb = 1;
				ha->flogi_acc_pld_remaining =
				    rptid_entry->u.f1.flogi_acc_payload_size;
				ha->flogi_acc_curr_offset = 0;
				ql_dbg(ql_dbg_async, vha, 0x5075,
				    "FLOGI ACC payload size %x\n",
				    ha->flogi_acc_pld_remaining);
			} else {
				ql_dbg(ql_log_warn, vha, 0x5075,
				    "Working on previous FLOGI ACC payload\n");
			}
		}
		...
		case TOPO_F:
			...
			if (QLA_DRV_SEND_ELS(ha) && rptid_entry->vp_idx == 0) {
...
qla25xx_set_els_cmds_supported(scsi_qla_host_t *vha)
{
	...
	/* List of Purex ELS */
	if (ql2xrdpenable) {
		cmd_opcode[active_cnt] = ELS_COMMAND_RDP;
		active_cnt++;
	}
	if (ha->flags.scm_supported_f) {
		cmd_opcode[active_cnt] = ELS_COMMAND_FPIN;
		active_cnt++;
	}
	if (QLA_DRV_SEND_ELS(ha)) {
		cmd_opcode[active_cnt] = ELS_COMMAND_RDF;
		active_cnt++;
		cmd_opcode[active_cnt] = ELS_COMMAND_EDC;
		active_cnt++;
	}
	if (ha->flags.edif_enabled) {
...
static bool is_cache_valid(scsi_qla_host_t *vha, uint16_t dev)
{
	if (dev == 0xa0) {
		if (vha->hw->sfp_cache.qla_sfp_a0_cache_valid) {
			return true;
		}
	}
	else if (dev == 0xa2) {
		if ((vha->hw->sfp_cache.qla_sfp_a2_cache_valid) &&
		    (time_after_eq(vha->hw->sfp_cache.qla_a2_expires_after,
		     jiffies)))
			return true;
	}
...
static bool is_sfpdata_cachable(uint16_t dev, uint16_t off, uint16_t len)
{
	if (dev == 0xa0) {
		if ((off + len) <= QLA_A0_CACHED_OFFSETS)
			return true;
	}
	else if (dev == 0xa2) {
		if ((off + len) <= QLA_A2_CACHED_OFFSETS)
			return true;
	}
...
static int
qla2x00_read_sfp_mbx(scsi_qla_host_t *vha, dma_addr_t sfp_dma, uint8_t *sfp,
		...
		uint16_t dev, uint16_t off, uint16_t len, uint16_t opt)
{
	...
	ql_dbg(ql_dbg_mbx + ql_dbg_verbose, vha, 0x10e8,
		"Entered %s dev = 0x%x off 0x%x len 0x%x\n", __func__, dev, off, len);

	if (!IS_FWI2_CAPABLE(ha))
		return QLA_FUNCTION_FAILED;

	if (is_sfpdata_cachable(dev, off, len)) {
		if (!(is_cache_valid(vha, dev))) {
			ql_dbg(ql_dbg_mbx + ql_dbg_verbose, vha, 0x10e8,
				"Need to recache for dev = 0x%x off 0x%x len 0x%x\n", dev, off, len);
			lsfp = dma_alloc_coherent(&vha->hw->pdev->dev,
			    QLA_MAX_OFFSETS_PER_MBX, &lsfp_dma, GFP_KERNEL);
			if (!lsfp) {
				ql_dbg_pci(ql_dbg_mbx, vha->hw->pdev, 0x011d,
				    "Unable to allocate memory for SFP cache\n");
				return QLA_FUNCTION_FAILED;
			}
			if (dev == 0xa0) {
				memset(lsfp, 0, QLA_MAX_OFFSETS_PER_MBX);
				rval = qla2x00_read_sfp_mbx(vha, lsfp_dma, lsfp, dev, 0,
							    QLA_MAX_OFFSETS_PER_MBX, opt);
				if (rval != QLA_SUCCESS) {
					dma_free_coherent(&vha->hw->pdev->dev, QLA_MAX_OFFSETS_PER_MBX,
							  lsfp, lsfp_dma);
					return rval;
				}
				memcpy(&vha->hw->sfp_cache.data_A0[0], lsfp,
				       QLA_MAX_OFFSETS_PER_MBX);

				memset(lsfp, 0, QLA_MAX_OFFSETS_PER_MBX);
				rval = qla2x00_read_sfp_mbx(vha, lsfp_dma, lsfp, dev, 64,
							    (QLA_A0_CACHED_OFFSETS - QLA_MAX_OFFSETS_PER_MBX), opt);
				if (rval != QLA_SUCCESS) {
					dma_free_coherent(&vha->hw->pdev->dev, QLA_MAX_OFFSETS_PER_MBX,
							  lsfp, lsfp_dma);
					return rval;
				}
				memcpy(&vha->hw->sfp_cache.data_A0[64], lsfp,
				       (QLA_A0_CACHED_OFFSETS - QLA_MAX_OFFSETS_PER_MBX));
				vha->hw->sfp_cache.qla_sfp_a0_cache_valid = 1;
			}
			else if (dev == 0xa2) {
				memset(lsfp, 0, QLA_MAX_OFFSETS_PER_MBX);
				rval = qla2x00_read_sfp_mbx(vha, lsfp_dma, lsfp, dev, 0,
				...
				memcpy(&vha->hw->sfp_cache.data_A2[0], lsfp,
				       QLA_MAX_OFFSETS_PER_MBX);

				memset(lsfp, 0, QLA_MAX_OFFSETS_PER_MBX);
				rval = qla2x00_read_sfp_mbx(vha, lsfp_dma, lsfp, dev, 64,
							    (QLA_A2_CACHED_OFFSETS - QLA_MAX_OFFSETS_PER_MBX), opt);
				if (rval != QLA_SUCCESS) {
					dma_free_coherent(&vha->hw->pdev->dev, QLA_MAX_OFFSETS_PER_MBX,
							  lsfp, lsfp_dma);
					return rval;
				}
				memcpy(&vha->hw->sfp_cache.data_A2[64], lsfp,
				       (QLA_A2_CACHED_OFFSETS - QLA_MAX_OFFSETS_PER_MBX));

				vha->hw->sfp_cache.qla_sfp_a2_cache_valid = 1;
				if (ql2xcache_valid_time < MIN_CACHE_VALID_TIME)
					ql2xcache_valid_time = MIN_CACHE_VALID_TIME;
				else if (ql2xcache_valid_time > MAX_CACHE_VALID_TIME)
					ql2xcache_valid_time = MAX_CACHE_VALID_TIME;
				vha->hw->sfp_cache.qla_a2_expires_after = jiffies +
					(ql2xcache_valid_time* HZ);
			}
			dma_free_coherent(&vha->hw->pdev->dev, QLA_MAX_OFFSETS_PER_MBX,
					  lsfp, lsfp_dma);
		}
		if (dev == 0xa2) {
			memcpy(sfp, &vha->hw->sfp_cache.data_A2[off], len);
			rval = QLA_SUCCESS;
		}
		else if (dev == 0xa0) {
			memcpy(sfp, &vha->hw->sfp_cache.data_A0[off], len);
			rval = QLA_SUCCESS;
		}
	}
	else {
		rval = qla2x00_read_sfp_mbx(vha, sfp_dma, sfp, dev, off, len, opt);
	}
...
/* Check if the priority for a VL, falls within the range
 * specificed.
 * Note: This could change based on switch and firmware recommendations
 */

static bool qla2xxx_chk_prio_range(uint8_t vl, uint8_t prio)
{
	bool ret = false;
...
int
qla2xxx_set_vl(fc_port_t *fcport, uint8_t vl)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	if  (!IS_QLA28XX(vha->hw))
		return QLA_FUNCTION_FAILED;

	if (atomic_read(&fcport->state) != FCS_ONLINE) {
		ql_log(ql_log_warn, vha, 0x1016,
			"fc port is not online, state:%d\n",atomic_read(&fcport->state));
		return QLA_FUNCTION_FAILED;
	}

	mcp->mb[0] = MBC_PORT_PARAMS;
	mcp->mb[1] = fcport->loop_id;
	mcp->mb[2] = BIT_0|BIT_8;
	/* Set the priority (low) in MB 5
	 * If the priority value is not available
	 * in the FPIN, use the default priority.
	 * This could happen when the congestion is cleared owing
	 * to time-period expiry
	 */
	if (qla2xxx_chk_prio_range(vl, fcport->vl.prio_lo))
		mcp->mb[5] = fcport->vl.prio_lo;
	else {
		ql_log(ql_log_info, vha, 0x1017,
		    "Prio: %d not in range for VL: %d, Using default \n",fcport->vl.prio_lo,
		    vl);
		if (vl == VL_NORMAL)
			mcp->mb[5] = VL_NORMAL_DEF_PRIO;
		else if (vl == VL_SLOW)
			mcp->mb[5] = ha->flogi_acc.rx_vl[VL_SLOW].prio_lo;
	}

	switch (vl) {
		case VL_SLOW:
			mcp->mb[2] |= SCM_MARK_DEVICE_SLOW;
			/* TODO: We are NOT changing the target speed, as this
			 * causes a significant drop in throughput. Could be
			 * changed in future
			 * //mcp->mb[3] = 0;
			 */
			mcp->mb[3] = fcport->fp_speed & 0x3F;
			ql_log(ql_log_info, vha, 0x0203,
			    "USCM: Moving %8phN to slow VL, prio:%d\n",
			    fcport->port_name, mcp->mb[5]);
			break;
		case VL_NORMAL:
			mcp->mb[2] |= SCM_MARK_DEVICE_NORMAL;
			mcp->mb[3] = fcport->fp_speed & 0x3F;
			ql_log(ql_log_info, vha, 0x0203,
			    "USCM: Moving %8phN to normal VL, prio:%d\n",
			    fcport->port_name, mcp->mb[5]);
			break;
		case VL_FAST:
			mcp->mb[2] |= SCM_MARK_DEVICE_FAST;
			mcp->mb[3] = fcport->fp_speed & 0x3F;
			ql_log(ql_log_info, vha, 0x0203,
			    "USCM: Moving %8phN  to fast VL\n", fcport->port_name);
			break;
	}

	mcp->mb[9] = vha->vp_idx;
	mcp->in_mb = MBX_2|MBX_1|MBX_0;
	mcp->out_mb = MBX_9|MBX_5|MBX_3|MBX_2|MBX_1|MBX_0;
	mcp->tov = 30;
	mcp->flags = 0;
	rval = qla2x00_mailbox_command(vha, mcp);

	if (rval != QLA_SUCCESS) {
		ql_log(ql_log_info, vha, 0x1016,
		    "Mbx Failed=%x mb[0]=%x, mb[1]=%x.\n", rval,
		    mcp->mb[0], mcp->mb[1]);
	} else {
...
static void qla2x00_async_mb_sp_done(srb_t *sp, int res)
{
	...
	ql_dbg(ql_dbg_mbx, sp->vha, 0x113f, "MB:%s hndl %x completed\n",
	    sp->name, sp->handle);
...
int qla_get_features(scsi_qla_host_t *vha, dma_addr_t dma, u16 buf_len)
{
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	int rval;
	...
	if (buf_len < FW_FEATURES_SIZE || !dma) {
		ql_dbg(ql_dbg_mbx, vha, 0xffff, "%s %d\n", __func__,__LINE__);
		return QLA_FUNCTION_FAILED;
	}
	...
	memset(&mc, 0, sizeof(mc));
	mcp->mb[0] = MBC_GET_RNID_PARAMS;
	mcp->mb[1] = RNID_TYPE_GET_FEATURES << 8;
	mcp->mb[2] = MSW(LSD(dma));
	mcp->mb[3] = LSW(LSD(dma));
	mcp->mb[6] = MSW(MSD(dma));
	mcp->mb[7] = LSW(MSD(dma));

	/* send mb via iocb */
	rval = qla24xx_send_mb_cmd(vha, mcp);

	return rval;
}

int qla_mpipt_get_status(scsi_qla_host_t *vha, u16 page, u16 *ret_buf, u16 buf_len)
{
	struct qla_hw_data *ha = vha->hw;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	mcp->mb[1] = MPIPT_SUBCMD_GET_STATUS;
	mcp->mb[2] = MPIPT_REQ_V1;
	mcp->mb[3] = page;

	/* send mb via iocb */
	rval = qla24xx_send_mb_cmd(vha, &mc);
	if (rval) {
		ql_dbg(ql_dbg_mbx, vha, 0x7071, "Failed %s %x (mb=%x,%x,%x)\n",
		    __func__, rval, mcp->mb[0], mcp->mb[1], mcp->mb[2]);
	} else {
		ql_dbg(ql_dbg_mbx, vha, 0x7070,
		    "%s, page %x mb=%x,%x,%x,%x.\n",
		    __func__, page,mcp->mb[2], mcp->mb[3], mcp->mb[4], mcp->mb[5]);
		memcpy(ret_buf, &mcp->mb[2], 8);
	}
...
int
qla27xx_update_sfp_fw_mb(scsi_qla_host_t *vha, mbx_cmd_t *mcp)
{
	int rval;
	dma_addr_t dma;
	uint32_t len;
	...
	ql_dbg(ql_dbg_mbx + ql_dbg_verbose, vha, 0xf0b3,
			"Entered %s.\n", __func__);

	if (!IS_QLA27XX(ha) && !IS_QLA28XX(ha)) {
		ql_dbg(ql_dbg_mbx, vha, 0xf0aa, "%s %d\n", __func__, __LINE__);
		return QLA_FUNCTION_FAILED;
	}

	dma = ha->sfp_fw_update_gdb.dma_addr;
	len = ha->sfp_fw_update_gdb.len;

	memset(mcp, 0, sizeof(*mcp));

	mcp->mb[0] = MBC_UPDATE_SFP_FW;
	mcp->mb[1] = ha->sfp_fw_update_gdb.options;
	...
	mcp->out_mb =
		MBX_7|MBX_6|MBX_5|MBX_4|MBX_3|MBX_2|MBX_1|MBX_0;
	mcp->in_mb = MBX_4|MBX_3|MBX_2|MBX_1|MBX_0;
	mcp->tov = MBX_SFP_FW_UPDATE_TOV_SECONDS;
	mcp->flags = 0;
	/* Copy sfp binary buffer */
	ql_dbg(ql_dbg_mbx + ql_dbg_verbose, vha, 0xf0ab, "SFP FW BIN:\n");
	ql_dump_buffer(ql_dbg_mbx + ql_dbg_verbose, vha, 0xf0ac,
			(char *)vha->hw->sfp_fw_update_gdb.buffer, len);

	rval = qla2x00_mailbox_command(vha, mcp);

	if (rval != QLA_SUCCESS)
		ql_log(ql_log_info, vha, 0xf0ac,
			"%s(%ld): Failed rval %#x %#x %#x %#x %#x %#x\n",
			__func__, vha->host_no, rval, mcp->mb[0],
			mcp->mb[1], mcp->mb[2], mcp->mb[3], mcp->mb[4]);
	else
		ql_log(ql_log_info, vha, 0xf0ad,
			"%s(%ld): Success rval %#x %#x %#x %#x %#x %#x\n",
			__func__, vha->host_no, rval, mcp->mb[0],
			mcp->mb[1], mcp->mb[2], mcp->mb[3], mcp->mb[4]);
