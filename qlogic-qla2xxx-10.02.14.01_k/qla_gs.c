...
qla2x00_rff_id(scsi_qla_host_t *vha, u8 type)
{
	...
	return qla_async_rffid(vha, &vha->d_id, INITIATOR_FC4F, type);
}
...
void qla24xx_sp_unmap(scsi_qla_host_t *vha, srb_t *sp)
{
	...
	case SRB_ELS_RDF:
	case SRB_ELS_EDC:
		if (sp->u.iocb_cmd.u.drv_els.dma_addr.cmd_buf) {
			dma_free_coherent(&vha->hw->pdev->dev,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.cmd_len,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.cmd_buf,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.cmd_dma);
			sp->u.iocb_cmd.u.drv_els.dma_addr.cmd_buf = NULL;
		}
		if (sp->u.iocb_cmd.u.drv_els.dma_addr.rsp_buf) {
			dma_free_coherent(&vha->hw->pdev->dev,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.rsp_len,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.rsp_buf,
			    sp->u.iocb_cmd.u.drv_els.dma_addr.rsp_dma);
			sp->u.iocb_cmd.u.drv_els.dma_addr.rsp_buf = NULL;
		}
