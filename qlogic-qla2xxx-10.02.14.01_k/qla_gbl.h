...
extern int qla24xx_post_prli_work(struct scsi_qla_host *, fc_port_t *);
...
int qla_edif_process_els(scsi_qla_host_t *vha, bsg_job_t *bsgjob);
...
extern u32 ql2xcache_valid_time;
extern int ql2xrspq_follow_inptr;
extern int ql2xrspq_follow_inptr_legacy;
extern int ql2x_scmr_drop_pct_low_wm;
extern int ql2x_scmr_flow_ctl_tgt;
extern int ql2x_scmr_flow_ctl_host;
extern int ql2x_scmr_throttle_mode;
extern int ql2x_scmr_profile;
extern int ql2x_scmr_use_slow_queue;
extern int ql2xcontrol_edc_rdf;
extern u64 ql2xdebug;
extern int ql2xvirtuallane;
...
struct edif_sa_ctl *qla_edif_find_sa_ctl_by_index(fc_port_t *fcport,
						  int index, int dir);
void qla2xxx_update_scm_fcport(scsi_qla_host_t *vha);
bool qla2xxx_throttle_req(srb_t *sp, struct qla_hw_data *ha, fc_port_t *fcport, uint8_t dir);
void qla2xxx_scmr_clear_throttle(struct qla_scmr_flow_control *sfc);
void qla2xxx_scmr_clear_congn(struct qla_scmr_flow_control *sfc);
void qla2xxx_scmr_manage_qdepth(srb_t *sp, struct fc_port *fcport, bool inc);
void qla2xxx_update_sfc_ios(srb_t *sp, struct qla_hw_data *ha, fc_port_t *fcport,
			    int new);
int qla2xxx_scm_get_features(struct scsi_qla_host *vha);
int qla2xxx_scm_send_rdf_els(scsi_qla_host_t *vha);
int qla2xxx_scm_send_edc_els(scsi_qla_host_t *vha);
void qla2xxx_send_uscm_els(scsi_qla_host_t *vha);
void qla2xx_scm_process_purex_edc(struct scsi_qla_host *vha,
			struct purex_item *item);
void qla2xxx_scm_process_purex_rdf(struct scsi_qla_host *vha,
			struct purex_item *item);
void qla2xxx_scm_alloc_rdf_payload(struct scsi_qla_host *vha);
void qla2xxx_scm_free_rdf_payload(struct scsi_qla_host *vha);
void qla2xxx_clear_scm_stats(struct scsi_qla_host *vha);
void qla2xxx_clear_scmr_stats(struct scsi_qla_host *vha);
void qla2xxx_scmr_init_deltas(struct qla_scmr_flow_control *sfc);
void qla_scm_clear_previous_event(struct scsi_qla_host *vha);
void qla_scm_clear_session(fc_port_t *fcport);
void qla_scm_clear_host(struct scsi_qla_host *vha);
bool qla_scm_chk_throttle_cmd_opcode(srb_t *sp);
int qla2xxx_set_vl(fc_port_t *fcport, uint8_t vl);
void qla_scm_host_clear_vl_state(struct scsi_qla_host *vha);
void qla_scm_tgt_clear_vl_state(fc_port_t *fcport);
uint8_t qla_get_throttling_state(struct qla_scmr_flow_control *sfc);
bool qla2xxx_switch_vl(struct qla_scmr_flow_control *sfc, uint8_t vl);
void qla27xx_process_purex_fpin(struct scsi_qla_host *vha,
				struct purex_item *item);
void qla_scm_clear_all_tgt_sess(struct scsi_qla_host *vha);
...
extern void qla2x00_timer(qla_timer_arg_t);
extern void qla2xxx_perf_timer(qla_timer_arg_t);
...
extern void qla27xx_sfp_fw_update_work_fn(struct work_struct *work);
extern int qla27xx_update_sfp_fw_mb(scsi_qla_host_t *vha, mbx_cmd_t *mcp);
extern void
qla27xx_update_sfp_fw_update_gdb(struct qla_hw_data *ha, mbx_cmd_t *mcp);

int qla2xxx_set_scm_params(fc_port_t *fcport, bool congested);
...
int qla_mpipt_get_features(scsi_qla_host_t *, u16 *, u16);
int qla_get_features(scsi_qla_host_t *, dma_addr_t, u16);
int qla_mpipt_get_status(scsi_qla_host_t *, u16 , u16 *, u16);
...
extern int qla2x00_mailbox_passthru(bsg_job_t *bsg_job);
int qla2x00_sys_ld_info(bsg_job_t *bsg_job);
...
int qla_mailbox_passthru(scsi_qla_host_t *vha, uint16_t *mbx_in,
			 uint16_t *mbx_out);
int qla2x00_sys_ld_info(bsg_job_t *bsg_job);
void qla_mpi_scan(scsi_qla_host_t *);
...
extern void ql_scm_dump_buffer(uint level, scsi_qla_host_t *vha,
	uint id, void *buf, uint size);
/*
...
extern void qla2x00_sp_timeout(qla_timer_arg_t timer_arg);
...
extern int qla24xx_bsg_request(bsg_job_t *);
extern int qla24xx_bsg_timeout(bsg_job_t *);
...
extern int qla2x00_get_idma_speed(scsi_qla_host_t *, uint16_t,
	uint16_t *, uint16_t *);
extern int qla24xx_sadb_update(bsg_job_t *bsg_job);
...
extern void qla_schedule_sess_for_deletion(struct fc_port *);
...
int32_t qla_edif_app_mgmt(bsg_job_t *bsg_job);
...
#define QLA2XX_LINK_DOWN		BIT_5
#define QLA2XX_TGT_SEND_LOGO		BIT_16
...
int qla2xxx_get_tgt_stats(struct Scsi_Host *shost, u32 flags,
	struct fc_rport *rport, struct ql_vnd_tgt_stats_resp *data, int num_entry);
int qla2xxx_disable_port(struct Scsi_Host *shost);
int qla2xxx_enable_port(struct Scsi_Host *shost);

uint64_t qla2x00_get_num_tgts(scsi_qla_host_t *vha, u32 stat_type);
...
void qla_logo_completion_handler(fc_port_t *, int);
#endif /* _QLA_GBL_H */
