 ...
 * Copyright (C) 2018-2024 Broadcom. All Rights Reserved. The term *
...
static struct dh_group dh_group_array[5] = {
	{
		DH_GROUP_NULL, 0,
		{
...
{
	if (!test_and_clear_bit(NLP_AUTH_TMO, &ndlp->nlp_flag))
		return;
...
{
	if (test_bit(NLP_AUTH_TMO, &ndlp->nlp_flag))
		lpfc_auth_cancel_tmr(ndlp);

	mod_timer(&ndlp->dhc_cfg.nlp_reauthfunc,
		  jiffies + msecs_to_jiffies(value));
	set_bit(NLP_AUTH_TMO, &ndlp->nlp_flag);
}

/**
 * lpfc_auth_start_reauth_tmr - start re-authentication timer
 * @ndlp: pointer to a node-list data structure.
 *
...
lpfc_auth_start_reauth_tmr(struct lpfc_nodelist *ndlp)
{
	...
	msecs = (ndlp->dhc_cfg.reauth_interval * 60) * MSEC_PER_SEC;
	ndlp->dhc_cfg.next_reauth = jiffies + (msecs_to_jiffies(msecs));
	msecs = (msecs * 9) / 10; /* overcompensate for timer inaccuracy */
		   ...
		   struct lpfc_name *rwwpn)
{
	...
	if (!memcmp(&vport->fc_portname, lwwpn->u.wwn, 8)) {
		if (!memcmp(wwpn.u.wwn, rwwpn->u.wwn, 8))
			...
			ndlp = lpfc_refnode_wwpn(vport, rwwpn);
	}
	return ndlp;
}
...
lpfc_auth_cmpl_rd_object(struct lpfc_hba *phba, LPFC_MBOXQ_t *mboxq)
{
	...
	if (shdr_status || shdr_add_status || mb_sts) {
		...
		if (mboxq->ctx_u.dmabuf_list == &phba->lpfc_auth_active_cfg_list)
			lpfc_auth_free_active_cfg_list(phba);
	}
 ...
 * @hash_id: the hash algorithm to be used
 * @val1: Pointer to first parameter
 * @val1_len: Size of first value
 * @val2: Pointer to second parameter
 * @val2_len: Size of second value
 * @val3: Pointer to third parameter
 * @val3_len: Size of third value
 *
 * Return value
 *   Pointer to hash
 **/
			 ...
			 struct lpfc_iocbq *rspiocb)
{
	...
	if (ulp_status == IOSTAT_LS_RJT &&
	    ndlp && ndlp->dhc_cfg.auth_mode == LPFC_AUTH_MODE_ACTIVE) {
...
lpfc_auth_issue_negotiate(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp)
{
	...
	elsiocb->cmd_cmpl = lpfc_auth_cmpl_negotiate;
 ...
 * @dh_grp_id: Diffie-Hellman group identifier
 * @hash_id: specifies hash algorithm to be used
 * @hash_value: response value
 * @dh_value: dh parameter
 * @dh_val_len: dh parameter size
 *
 * Return code
 *   0 -
 *   1 -
 ...
 * @vport: pointer to a host virtual N_Port data structure.
 * @ndlp: pointer to a node-list data structure.
 * @hash_value: hash value to be incorporated in payload.
 *
 * Return code
 *   0 -
 *   1 -
			   ...
			   struct lpfc_nodelist *ndlp)
{
	...
	for (i = 0; i < be32_to_cpu(negotiate->num_protocol); i++) {
		...
		/* skip to next protocol parameter descriptor */
		     ...
		     struct lpfc_nodelist *ndlp)
{
	...
	size = cmdiocb->wcqe_cmpl.total_data_placed;
	pcmd = kzalloc(size, GFP_KERNEL);
	if (!pcmd)
		goto out;

	size1 = cmdiocb->wqe.xmit_els_rsp.bde.tus.f.bdeSize;
	memcpy(pcmd, cmdiocb->cmd_dmabuf->virt, size1);

	if (cmdiocb->wcqe_cmpl.word3 == 2) {
		size2 = cmdiocb->unsol_rcv_len;
		memcpy(pcmd + size1, cmdiocb->bpl_dmabuf->virt, size2);
	}
...
#endif
{
	...
	if (time_is_after_jiffies(ndlp->dhc_cfg.next_reauth)) {
		/* Setup remaining reauth delay */
		mod_timer(&ndlp->dhc_cfg.nlp_reauthfunc,
			  ndlp->dhc_cfg.next_reauth);
		return;
	}
