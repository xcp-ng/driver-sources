...
qla24xx_walk_and_build_prot_sglist(struct qla_hw_data *ha, srb_t *sp,
	struct dsd64 *cur_dsd, uint16_t tot_dsds)
{
...
{
	qla_timer_setup(&sp->u.iocb_cmd.timer, qla2x00_sp_timeout, 0, sp);
...
qla2x00_start_sp(srb_t *sp)
{
	...
	case SRB_ELS_RDF:
	case SRB_ELS_EDC:
		qla_els_pt_iocb(sp->vha, pkt,  &sp->u.iocb_cmd.u.drv_els.els_pt_arg);
		((struct els_entry_24xx *)pkt)->handle = sp->handle;
		break;
	case SRB_MARKER:
	...
	for (i = 0; i < more_dsd_lists; i++) {
		...
		if (!dsd_ptr) {
			ql_log(ql_log_fatal, vha, 0x3029,
			    "Failed to allocate memory for dsd_dma for cmd=%px.\n", cmd);
			goto queuing_error;
		}
		...
		if (!dsd_ptr->dsd_addr) {
			...
			ql_log(ql_log_fatal, vha, 0x302a,
			    "Failed to allocate memory for dsd_addr for cmd=%px.\n", cmd);
			goto queuing_error;
		}
		list_add_tail(&dsd_ptr->list, &qpair->dsd_list);
		qpair->dsd_avail++;
