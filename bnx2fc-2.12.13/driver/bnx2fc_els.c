...
static void bnx2fc_srr_compl(struct bnx2fc_els_cb_arg *cb_arg)
{
	...
	/* Signal that SRR request has been completed */
	clear_bit(BNX2FC_FLAG_SRR_SENT, &orig_io_req->req_flags);
...
void bnx2fc_flogi_resp(struct fc_seq *seq, struct fc_frame *fp,
			      void *arg)
{
	...
	if (!is_zero_ether_addr(granted_mac)) {
		memcpy(fcoe_mac, granted_mac, ETH_ALEN);
	...
	/*
	 * Complete flogi_compl in case some link_down work is waiting for
	 * the FLOGI to complete before sending ADISC requests out.
	 */
	complete(&interface->flogi_compl);
