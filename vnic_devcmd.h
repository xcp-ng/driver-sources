...
/***** Replaced by CMD_INIT *****/
	...
	/*
	 * spoof check OP.
	 * in:     (u32) a0.high32 = op_get (0), op_set (1)
	 * in:     (u16) a0.low32 = index of VF or SRIOV_PF_IDX
	 * in/out: (u32) a1 = enable (1), disable (0)
	 */
	CMD_SPOOF_CHK_OP = _CMDC(_CMD_DIR_RW, _CMD_VTYPE_ENET, 92),

	/*
	 * SRIOV local forwarding OP within PF domain.
	 * in:     (u32) a0.high32 = op_get (0), op_set (1)
	 * in:     (u16) a0.low32 = SRIOV_PF_IDX
	 * in/out: (u32) a1 = enable (1), disable (0)
	 */
	CMD_SRIOV_LOCAL_FWD_OP = _CMDC(_CMD_DIR_RW, _CMD_VTYPE_ENET, 93),

	/*
	 * SRIOV VF allowed devcmd list OP
	 * in:     (u32) a0.high32 = op_get (0), op_set (1)
	 * in:     (u16) a0.low32 = index of VF or SRIOV_PF_IDX
	 * inout:  (u64) a1 - a4 = bitmap of allowed devcmds, one bit per devcmd
	 */
	CMD_VF_ALLOWED_LIST_OP = _CMDC(_CMD_DIR_RW, _CMD_VTYPE_ENET, 94),

	/*
	 * SRIOV set VF access vlan OP
	 * in:     (u32) a0.high32 = op_get (0), op_set (1)
	 * in:     (u16) a0.low32 = index of VF (SRIOV_PF_IDX not support)
	 * inout:  (u16) a1 = access-vlan
	 * in      (u16) a2 = nonzero restore back to the default trunk
	 *		      (op_set only)
	 */
	CMD_VF_ACCESS_VLAN_OP = _CMDC(_CMD_DIR_RW, _CMD_VTYPE_ALL, 95),

	/*
	 *·SRIOV·option·OPs·to·get/set·sriov_option
	 *·in:·····(u32)·a0.high32·=·op_get·(0),·op_set(1)
	 *·in:·····(u16)·a0.low32·=·index·of·VF
	 *·inout:··(u32)·a1.high32·=·CMD_XXX_XXX·(related)·or·0
	 *·inout:··(u32)·a1.low32·=·option·(defined·as·sriov_option)
	 *·inout:··(u64)·a2·=·option·dependent
	 *·inout:··(u64)·a3·=·option·dependent
	 *·inout:··(u64)·a4·=·option·dependent
	 *·inout:··(u64)·a5·=·option·dependent
	 */
	CMD_SRIOV_OPTION_OP = _CMDC(_CMD_DIR_RW, _CMD_VTYPE_ENET, 96),
	...
	/*
	 * RWE TSO Carve OPs
	 * in:	  (u32) a0 = op_get (0), op_set(1)
	 * inout: (u32) a1 = MTU
	 * inout: (u32) a2 = carve multiplier
	 * inout: (u32) a3 = bit 0: IPv4 enable, bit 1: IPv6 enable
	 *
	 * Capability query:
	 * out: (u32) a0 = errno, 0:valid cmd
	 * out: (u64) a1 = carve bitmap
	 */
	CMD_RWE_TSO_OP = _CMDC(_CMD_DIR_WRITE, _CMD_VTYPE_ENET, 99),
};

#define CMD_DEVCMD_LAST	(_CMD_N(CMD_SRIOV_STATS_GET) + 1)

#define SRIOV_PF_IDX	((u16) 0xFFFF)

enum vnic_devcmd_get_set_op {
	VNIC_DEVCMD_OP_GET,
	VNIC_DEVCMD_OP_SET
};

/* options for CMD_SRIOV_OPTION_OP
 */
enum sriov_option {
	OPT_PRECONF,			/* a2.low32: 1 = enable,
					 *	     0 = disable
					 */
	OPT_PACKET_FILTER_BITS_IGNORED,	/* a2.low32: 1 = ignore VF packet
					 *		 filter bits
					 */
	OPT_VF_MAC_RESTRICTED,		/* a2: 1:= set, 0 = clear
					 * a3: 6 bytes of restricted mac
					 *     address for VF
					 */
	OPT_LINK_NOTIFY_PF_ONLY,	/* a2: 1 = set, 0 = clear
					 * Default mode is !PF_ONLY
					 */
};

/* CMD_QP_TYPE_SET: (wq/rq)·queue·pair·type
 */
#define QP_TYPE_ADMIN	0
#define QP_TYPE_DATA	1
#define QP_TYPE_MAX	2

struct vnic_sriov_stats {
	u32 ver;
	u8 sriov_vlan_membership_cap;	/* sriov support vlan-membership */
	u8 sriov_vlan_membership_enabled; /* Default is disabled (0) */
	u8 sriov_rss_vf_full_cap;	/* sriov VFs support full rss */
	u8 sriov_host_rx_stats;		/* host does rx stats */

	/* IGx/EGx classifier TCAM
	 */
	u32 ig_classifier0_tcam_cfg;	/* IG0 TCAM config entries */
	u32 ig_classifier0_tcam_free;	/* IG0 TCAM free count */
	u32 eg_classifier0_tcam_cfg;	/* EG0 TCAM config entries */
	u32 eg_classifier0_tcam_free;	/* EG0 TCAM free count */

	u32 ig_classifier1_tcam_cfg;	/* IG1 TCAM config entries */
	u32 ig_classifier1_tcam_free;	/* IG1 TCAM free count */
	u32 eg_classifier1_tcam_cfg;	/* EG1 TCAM config entries */
	u32 eg_classifier1_tcam_free;	/* EG1 TCAM free count */

	/* IGx/EGx flow table entries
	 */
	u32 sriov_ig_flow_table_cfg;	/* sriov IG FTE config */
	u32 sriov_ig_flow_table_free;	/* sriov IG FTE free */
	u32 sriov_eg_flow_table_cfg;	/* sriov EG FTE config */
	u32 sriov_eg_flow_table_free;	/* sriov EG FTE free */

	u8 admin_qp_ready[32];		/* admin_qp ready bits (256) */
	u16 vf_index;			/* VF index or SRIOV_PF_IDX */
	u16 reserved1;
	u32 reserved2[256 - 23];
};
...
/*
 * Defines for special case performance enhancement. If the RWE TSO firmware
 * feature is enabled and the MTU is within the LOW-HIGH range, TSO offload is
 * affected. The mutiplier 6 works for MTU valuse up to 1568.
 */
#define VNIC_RWE_TSO_MSS_MULTIPLIER 6
#define VNIC_RWE_TSO_MSS_LOW        1000
#define VNIC_RWE_TSO_MSS_HIGH       1560
#define VNIC_RWE_TSO_OP_GET         0
#define VNIC_RWE_TSO_OP_SET         1
#define VNIC_RWE_TSO_IPV4           (1 << 0)  // enable bit for IPv4
#define VNIC_RWE_TSO_IPV6           (1 << 1)  // enable bit for IPv6
