...
//		clear_bit(QEDF_CMD_OUTSTANDING, &io_req->flags);
		...
		/* Do abort on REC timeout of REC command
		 * and retry REC once.
		 */

		if (op == ELS_REC) {
			init_completion(&io_req->abts_done);
			qedf_initiate_abts(io_req, false);
		} else
			qedf_initiate_cleanup(io_req, true);
	...
	case QEDF_SEQ_CLEANUP:
		QEDF_ERR(&(qedf->dbg_ctx), "Sequence cleanup timeout, "
		    "xid=0x%x.\n", io_req->xid);
		if (io_req->retry_seq_cleanup) {
			qedf_initiate_seq_cleanup(io_req,
				io_req->cb_arg->offset, io_req->cb_arg->r_ctl);
			io_req->retry_seq_cleanup = 0;
			/* Put reference back taken during seq cleanup */
			kref_put(&io_req->refcount, qedf_release_cmd);
			break;
		}
		qedf_initiate_cleanup(io_req, true);
...
struct qedf_ioreq *qedf_alloc_cmd(struct qedf_rport *fcport, u8 cmd_type)
{
	...
	io_req->tm_flags = 0;
	io_req->event = QEDF_IOREQ_EV_NONE;
...
#if defined(NR_HW_QUEUES) && !defined(USE_BLK_MQ) && defined(CMD_SCSI_REQ)
...
#elif defined(NR_HW_QUEUES) && defined(USE_BLK_MQ) && defined(CMD_SCSI_REQ)
...
#if defined(BLK_DEV_SPECIAL) && defined(CMD_SCSI_REQ)
	...
	struct qedf_ioreq *io_req)
{
	...
	if (fcport->dev_type == QEDF_RPORT_TYPE_TAPE) {
			...
			if (!test_bit(QEDF_CMD_SRR_SENT, &io_req->flags)) {
				io_req->rx_buf_off =
				    cqe->cqe_info.warn_info.buf_off;
				io_req->tx_buf_off =
				    cqe->cqe_info.warn_info.buf_off;
				io_req->rx_id = cqe->cqe_info.warn_info.rx_id;
				io_req->retry_rec = 1;
...
#define FCOE_ERROR_CODE_DATA_OOO_RO 0x40000// BIT_18
#define FCOE_ERROR_CODE_DATA_SOFI3_SEQ_ACTIVE_SET 0x200000//BIT_21
#define FCOE_ERROR_CODE_DATA_SOFN_SEQ_ACTIVE_RESET 0x400000 //BIT_22
#define FCOE_ERROR_CODE_COMMON_INCORRECT_SEQ_CNT 0x40 //BIT_38 - high

#define FCOE_ERROR_CODE_FCP_RSP_INVALID_PAYLOAD_SIZE 0x1000 // BIT_12
#define FCOE_ERROR_CODE_FCP_RSP_OPENED_SEQ 0x4000 // BIT_14
	...
	struct qedf_ioreq *io_req)
{
	...
	err_warn_bitmap_hi = le32_to_cpu(cqe->cqe_info.err_info.err_warn_bitmap_hi);
	err_warn_bitmap_lo = le32_to_cpu(cqe->cqe_info.err_info.err_warn_bitmap_lo);

	QEDF_ERR(&(fcport->qedf->dbg_ctx),
	    "Error detection CQE, xid=0x%x err_warn_bitmap=%08x:%08x tx_buff_off=%08x rx_buff_off=%08x, rx_id=%04x\n",
	    io_req->xid, err_warn_bitmap_hi, err_warn_bitmap_lo,
        ...
        if (test_bit(QEDF_RPORT_IN_TARGET_RESET, &fcport->flags) ||
            (test_bit(QEDF_RPORT_IN_LUN_RESET, &fcport->flags) &&
            io_req->sc_cmd->device->lun == (u64)fcport->lun_reset_lun)) {
                QEDF_ERR(&qedf->dbg_ctx,
	...
	/* if its TAPE and TARGET supports RETRY */
	if ((fcport->dev_type == QEDF_RPORT_TYPE_TAPE) &&
		(fcport->rdata->flags & FC_RP_FLAGS_RETRY)) {

		if ((err_warn_bitmap_lo & FCOE_ERROR_CODE_DATA_OOO_RO) ||
		    (err_warn_bitmap_lo & FCOE_ERROR_CODE_DATA_SOFI3_SEQ_ACTIVE_SET) ||
		    (err_warn_bitmap_lo & FCOE_ERROR_CODE_DATA_SOFN_SEQ_ACTIVE_RESET) ||
		    (err_warn_bitmap_hi & FCOE_ERROR_CODE_COMMON_INCORRECT_SEQ_CNT)) {
			io_req->chk_4_seq_int = 1;
			/* Send REC */
			io_req->retry_seq_cleanup = 1;
			qedf_send_rec(io_req);
			return;
		} else if ((err_warn_bitmap_lo & FCOE_ERROR_CODE_FCP_RSP_INVALID_PAYLOAD_SIZE) ||
		           (err_warn_bitmap_lo & FCOE_ERROR_CODE_FCP_RSP_OPENED_SEQ)) {
			io_req->chk_4_seq_int = 0;
			/* Send REC */
			io_req->retry_seq_cleanup = 1;
			qedf_send_rec(io_req);
...
void qedf_update_bdq_prod_idx(struct qedf_ctx *qedf)
{
	unsigned long flags;
	uint16_t tmp;
