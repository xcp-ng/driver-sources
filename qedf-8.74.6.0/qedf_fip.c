	...
	struct sk_buff *skb)
{
	...
	if (atomic_read(&qedf->link_state) == QEDF_LINK_DOWN) {
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC, "Dropping VLAN"
			"response as link is down.\n");
		return;
	}
...
#else
void qedf_fip_recv(struct qedf_ctx *qedf, struct sk_buff *skb, u16 vlan)
#endif
{
	struct ethhdr *eth_hdr;
	struct fip_header *fiph;
...
#ifdef MODERN_VLAN
	__vlan_hwaccel_get_tag(skb, &vlan);
#endif

	if (test_bit(QEDF_UNLOADING, &qedf->flags)) {
		QEDF_ERR(&qedf->dbg_ctx, "Ignore FIP frame, driver getting unload.\n");
		return;
	}


	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_LL2, "FIP frame received: "
		    "skb=%px fiph=%px source=%pM destn=%pM op=%x sub=%x vlan=%04x", skb, fiph,
	    eth_hdr->h_source, eth_hdr->h_dest, op, sub, vlan);
	if (qedf_dump_frames)
		print_hex_dump(KERN_WARNING, "fip ", DUMP_PREFIX_OFFSET, 16, 1,
		    skb->data, skb->len, false);
	...
	if (!ether_addr_equal(eth_hdr->h_dest, qedf->mac) &&
		...
		QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
			"Dropping FIP type 0x%x pkt due to destination MAC "
			"mismatch dest_mac=%pM ctlr.dest_addr=%pM "
			"data_src_addr=%pM.\n", op, eth_hdr->h_dest,
			qedf->mac, qedf->data_src_addr);
		kfree_skb(skb);
		return;
	}
	...
	if (op == FIP_OP_VLAN) {
#ifdef DEFINE_FIP_SC_VL_REP
		if (sub == FIP_SC_VL_REP) {
#else
		if (sub == FIP_SC_VL_NOTE) {
#endif
		...
		while (rlen >= sizeof(*desc)) {
			...
			case FIP_DT_VN_ID:
				...
				QEDF_ERR(&qedf->dbg_ctx,
				    "CVL vx_port fd_fc_id=%x fd_mac=%pM fd_wwpn=%016llx.\n",
				    ntoh24(vp->fd_fc_id), vp->fd_mac,
				    get_unaligned_be64(&vp->fd_wwpn));
				/* Check for vx_port wwpn OR Check vx_port fabric ID OR Check vx_port MAC */
				if ((get_unaligned_be64(&vp->fd_wwpn) == qedf->wwpn) ||
				     (ntoh24(vp->fd_fc_id) == qedf->lport->port_id) ||
				     (ether_addr_equal(vp->fd_mac, qedf->data_src_addr))) {
