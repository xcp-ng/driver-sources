...
static void bnx2x_get_ringparam(struct net_device *dev, struct ethtool_ringparam *ering,
...
#ifdef _HAS_ETHTOOL_KERNEL_ERING /* QEDE_UPSTREAM */
static int bnx2x_set_ringparam(struct net_device *dev, struct ethtool_ringparam *ering,
			       struct kernel_ethtool_ringparam *kernel_ering,
			       struct netlink_ext_ack *ext_ack)
#else
...
#ifdef _HAS_RSS_HASH_FUNCS /* BNX2X_UPSTREAM */
#ifndef _HAS_ETHTOOL_RXFH_PARAM /* QEDE_UPSTREAM */
	if (hfunc)
...
#endif
{
	 ...
	 */
#ifdef _HAS_ETHTOOL_RXFH_PARAM /* QEDE_UPSTREAM */
	if (rxfh->key || (rxfh->hfunc != ETH_RSS_HASH_NO_CHANGE && rxfh->hfunc != ETH_RSS_HASH_TOP))
#else
	if (key || (hfunc != ETH_RSS_HASH_NO_CHANGE && hfunc != ETH_RSS_HASH_TOP))
#endif
		return -EOPNOTSUPP;

#ifdef _HAS_ETHTOOL_RXFH_PARAM
	if (!rxfh->indir)
#else
	if (!indir)
#endif
