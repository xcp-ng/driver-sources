...
#define DRV_MODULE_VERSION	"2.2.6j"
#define DRV_MODULE_RELDATE	"2024"
...
#ifndef _DEFINE_SKB_FRAG_SET_PAGE		
...
bnx2_setup_wol(struct bnx2 *bp)
{
	...
	if (bp->wol) {
		...
		bnx2_set_mac_addr(bp, (void *)bp->dev->dev_addr, 0);
...
{
#ifdef BNX2_NEW_NAPI
#if (LINUX_VERSION_CODE >= 0x030f00)
	int i, total_vecs;
#else
	int i, total_vecs, rc;
#endif
...
#ifdef HAVE_KERNEL_ETHTOOL_COALESCE
...
#ifdef HAVE_ETHTOOL_KERNEL_ERING /* UPSTREAM */
static void
...
bnx2_init_board(struct pci_dev *pdev, struct net_device *dev)
{
	...
	if (dma_set_mask_and_coherent(&bp->pdev->dev, dma_mask) == 0) {
#if defined(__VMKLNX__) && (VMWARE_ESX_DDK_VERSION >= 50000)
                if (BNX2_CHIP(bp) == BNX2_CHIP_5708)
		...
		dev->features |= NETIF_F_HIGHDMA;
#endif
	} else if ((rc = dma_set_mask_and_coherent(&bp->pdev->dev, DMA_BIT_MASK(32))) != 0) {
...
#else
#ifdef HAVE_NDO_EXT_CHANGE_MTU /* ! BNX2_UPSTREAM */
...
bnx2_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	memcpy((void *)dev->dev_addr, bp->mac_addr, ETH_ALEN);
#ifdef ETHTOOL_GPERMADDR
