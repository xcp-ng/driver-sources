...
#ifndef _DEFINE_MMIOWB_SPIN_LOCK_
	mmiowb();
#endif
}
			   ...
			   struct iscsi_task *task)
{
	...
	if (test_bit(SK_F_IPV6, &bnx2i_conn->ep->cm_sk->flags)) {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Login sent to %pI6 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid)
	} else {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Login sent to %pI4 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid)
	}
			    ...
			    struct iscsi_task *task)
{
	...
	if (test_bit(SK_F_IPV6, &bnx2i_conn->ep->cm_sk->flags)) {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Logout sent to %pI6 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid);
	} else {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Logout sent to %pI4 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid);
	}
...
int bnx2i_send_conn_destroy(struct bnx2i_hba *hba, struct bnx2i_endpoint *ep)
{
	...
	BNX2I_DBG_LOG(hba, "Destroy connection sent for ep_iscsi_id=%u.\n",
	    ep->ep_iscsi_cid);
					 ...
					 struct bnx2i_endpoint *ep)
{
	...
	BNX2I_DBG_LOG(hba, "Offload request sent for ep_iscsi_cid=%u.\n",
	    ep->ep_iscsi_cid);
				    ...
				    struct cqe *cqe)
{
	...
	if (test_bit(SK_F_IPV6, &bnx2i_conn->ep->cm_sk->flags)) {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Login complete to %pI6 on "
		    "ep_iscsi_id=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid)
	} else {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Login complete to %pI4 on "
		    "ep_iscsi_id=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid)
	}
				     ...
				     struct cqe *cqe)
{
	...
	if (test_bit(SK_F_IPV6, &bnx2i_conn->ep->cm_sk->flags)) {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Logout complete from %pI6 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid);
	} else {
		BNX2I_DBG_LOG(bnx2i_conn->hba, "Logout complete from %pI4 on "
		    "ep_iscsi_cid=%u.\n", bnx2i_conn->ep->cm_sk->dst_ip,
		    bnx2i_conn->ep->ep_iscsi_cid)
	}
					    ...
					    struct iscsi_kcqe *conn_destroy)
{
	...
	if (conn_destroy->completion_status) {
		...
		BNX2I_DBG_LOG(hba, "Destroy connection complete for "
		    "ep_iscsi_cid=%u.\n", ep->ep_iscsi_cid);
	}
				    ...
				    struct iscsi_kcqe *ofld_kcqe)
{
	...
	if (ofld_kcqe->completion_status) {
		...
		BNX2I_DBG_LOG(hba, "Offload complete for ep_iscsi_cid=%u",
		    ep->ep_iscsi_cid);
	}
	wake_up_interruptible(&ep->ofld_wait);
}
