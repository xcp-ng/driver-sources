...
lpfc_config_link(struct lpfc_hba * phba, LPFC_MBOXQ_t * pmb)
{
	...
	if (phba->cfg_cr_delay && (phba->sli_rev < LPFC_SLI_REV4)) {
		mb->un.cfg_lnk_s3.cr_ci_cr_delay |=
			(CONFIG_LINK_CR | CONFIG_LINK_CI |
			 (phba->cfg_cr_delay << CONFIG_LINK_CR_DELAY_SHIFT &
			  CONFIG_LINK_CR_DELAY_MASK));
		mb->un.cfg_lnk_s3.cr_count = phba->cfg_cr_count;
	}

	mb->un.cfg_lnk.myId = cpu_to_le32(vport->fc_myDID);
	mb->un.cfg_lnk.edtov = cpu_to_le32(phba->fc_edtov);
	mb->un.cfg_lnk.lptov = cpu_to_le32(phba->fc_arbtov);
	mb->un.cfg_lnk.ratov = cpu_to_le32(phba->fc_ratov);
	mb->un.cfg_lnk.rttov = cpu_to_le32(phba->fc_rttov);
	mb->un.cfg_lnk.altov = cpu_to_le32(phba->fc_altov);
	if (phba->bbcredit_support && phba->cfg_enable_bbcr) {
		bbscn = bf_get(lpfc_bbscn_def, &phba->sli4_hba.bbscn_params);
		bbscn = bbscn << CONFIG_LINK_BBSCN_SHIFT & CONFIG_LINK_BBSCN_MASK;
		mb->un.cfg_lnk.word10 = cpu_to_le32(CONFIG_LINK_CSCN | bbscn);
	}

	if (phba->cfg_ack0 && (phba->sli_rev < LPFC_SLI_REV4))
		mb->un.cfg_lnk_s3.word11 |= cpu_to_le32(CONFIG_LINK_ACK0_ENABLE);
	       ...
	       LPFC_MBOXQ_t * pmb, uint32_t topology, uint32_t linkspeed)
{
	...
	case FLAGS_TOPOLOGY_MODE_LOOP_PT:
		mb->un.init_link.link_flags =
			cpu_to_le16(FLAGS_TOPOLOGY_MODE_LOOP | FLAGS_TOPOLOGY_FAILOVER);
		break;
	case FLAGS_TOPOLOGY_MODE_PT_PT:
		mb->un.init_link.link_flags = cpu_to_le16(FLAGS_TOPOLOGY_MODE_PT_PT);
		break;
	case FLAGS_TOPOLOGY_MODE_LOOP:
		mb->un.init_link.link_flags = cpu_to_le16(FLAGS_TOPOLOGY_MODE_LOOP);
		break;
	case FLAGS_TOPOLOGY_MODE_PT_LOOP:
		mb->un.init_link.link_flags =
			cpu_to_le16(FLAGS_TOPOLOGY_MODE_PT_PT | FLAGS_TOPOLOGY_FAILOVER);
		break;
	case FLAGS_LOCAL_LB:
		mb->un.init_link.link_flags = cpu_to_le16(FLAGS_LOCAL_LB);
		break;
	}
	...
	if (phba->sli_rev == LPFC_SLI_REV3 && !phba->cfg_fcp_wait_abts_rsp)
		mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_IMED_ABORT);
	...
	if (vpd->rev.feaLevelHigh >= 0x02){
		...
		case LPFC_USER_LINK_SPEED_1G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_1G);
			break;
		case LPFC_USER_LINK_SPEED_2G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_2G);
			break;
		case LPFC_USER_LINK_SPEED_4G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_4G);
			break;
		case LPFC_USER_LINK_SPEED_8G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_8G);
			break;
		case LPFC_USER_LINK_SPEED_10G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_10G);
			break;
		case LPFC_USER_LINK_SPEED_16G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_16G);
			break;
		case LPFC_USER_LINK_SPEED_32G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_32G);
			break;
		case LPFC_USER_LINK_SPEED_64G:
			mb->un.init_link.link_flags |= cpu_to_le16(FLAGS_LINK_SPEED);
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_64G);
			break;
		case LPFC_USER_LINK_SPEED_AUTO:
		default:
			mb->un.init_link.link_speed = cpu_to_le32(LINK_SPEED_AUTO);
			break;
		}
	...
	mb->mbxOwner = OWN_HOST;
	mb->un.init_link.fabric_al_pa = phba->fc_pref_ALPA;
...
lpfc_read_sparam(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb, int vpi)
{
	...
	mb->un.rd_sparm.sp64.type_size =
		cpu_to_le32(sizeof(struct serv_parm) & ULP_BDE64_SIZE_MASK);
	mb->un.rd_sparm.sp64.type_size |= cpu_to_le32(ULP_BDE64_TYPE_BDE_64);
	mb->un.rd_sparm.sp64.addr_high = cpu_to_le32(putPaddrHigh(mp->phys));
	mb->un.rd_sparm.sp64.addr_low = cpu_to_le32(putPaddrLow(mp->phys));
	if (phba->sli_rev >= LPFC_SLI_REV3)
		mb->un.rd_sparm.vpi = cpu_to_le16(phba->vpi_ids[vpi]);
...
lpfc_config_ring(struct lpfc_hba * phba, int ring, LPFC_MBOXQ_t * pmb)
{
	...
	mb->un.cfg_ring.ring = ring;
	mb->un.cfg_ring.buddy_rn = CONFIG_RING_RN;

	psli = &phba->sli;
	pring = &psli->sli3_ring[ring];
	mb->un.cfg_ring.num_mask = pring->num_mask;
	mb->mbxCommand = MBX_CONFIG_RING;
	mb->mbxOwner = OWN_HOST;

	/* Is this ring configured for a specific profile */
	if (pring->prt[0].profile) {
		mb->un.cfg_ring.profile = pring->prt[0].profile;
		goto done;
	}

	/* Otherwise we setup specific rctl / type masks for this ring */
	for (i = 0; i < pring->num_mask; i++) {
		mb->un.cfg_ring.rr_regs[i].rval = pring->prt[i].rctl;
		if (mb->un.cfg_ring.rr_regs[i].rval != FC_RCTL_ELS_REQ)
			mb->un.cfg_ring.rr_regs[i].rmask = 0xff;
		else
			mb->un.cfg_ring.rr_regs[i].rmask = 0xfe;
		mb->un.cfg_ring.rr_regs[i].tval = pring->prt[i].type;
		mb->un.cfg_ring.rr_regs[i].tmask = 0xff;
	}
