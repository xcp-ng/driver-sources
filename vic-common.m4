...
dnl
dnl Simple function to check multiple headers to AC DEFINE a value to 1 if
dnl any of them are found.
dnl
dnl $1 space separated list of headers to check
dnl $2 macro to AC DEFINE to 1 or 0
dnl $3 addional includes
dnl
AC_DEFUN([VIC_AC_CHECK_HEADERS_DEFINE_ANY],[
    $2=0
    VIC_AC_CHECK_HEADER([$1], [$2=1], [], [$3])
    AC_DEFINE_UNQUOTED([$2], [$$2], [Whether any of $1 exists or not])
])
...
AC_DEFUN([VIC_LINUX_DISTRO],[
    ...
    if test -r $os_release_file; then
        ...
        if test -n "`echo $NAME | grep 'Red Hat Enterprise Linux'`"; then
            ...
            RHEL_EXTRA="_`echo $KNAME | sed -E 's;(.*)\.el.*;\1;' | tr - _`"
            ...
            if test -n "`echo $KNAME | grep 'uek'`"; then
                ...
                RHEL_EXTRA="uek_`echo $KNAME | sed -E 's;(.*)\.el.*;\1;' | tr - _`"
                DISTRO_VER=${LINUX_DISTRO}${RHEL_EXTRA}
            else
                RHEL_EXTRA="rhck_`echo $KNAME | sed -E 's;(.*)\.el.*;\1;' | tr - _`"
                DISTRO_VER=${LINUX_DISTRO}${RHEL_MAJOR}u${RHEL_MINOR}${RHEL_EXTRA}
            fi
            ...
            LINUX_HEADER_DIR=/usr/src/linux-headers-$KNAME
...
AC_DEFUN([VIC_KCOMPAT_TESTS], [
	...
	VIC_AC_CHECK_HEADERS_DEFINE_ANY([linux/pci-dma-compat.h asm-generic/pci-dma-compat.h],
			[VIC_HAVE_PCI_DMA_COMPAT_H], [#include <linux/pci.h>])
