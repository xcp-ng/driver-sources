...
static int qla27xx_do_sfp_fw_update(bsg_job_t *bsg_job);
static int qla28xx_validate_flash_image(bsg_job_t *bsg_job);
...
static int
qla27xx_read_cached_sfp_data(bsg_job_t *bsg_job)
{
	struct fc_bsg_reply *bsg_reply = bsg_job->reply;
	struct Scsi_Host *host = fc_bsg_to_shost(bsg_job);
	scsi_qla_host_t *vha = shost_priv(host);
	struct qla_sfp_data sfp_data;
	int rval;

	sg_copy_to_buffer(bsg_job->request_payload.sg_list,
	    bsg_job->request_payload.sg_cnt, &sfp_data, sizeof(sfp_data));

	memset(sfp_data.a0, 0, 256);
	memset(sfp_data.a2, 0, 256);

	switch(sfp_data.options) {
		case QLA_READ_SFP_CACHED_DATA:
		case QLA_READ_SFP_CACHED_DATA_SKIP_CACHE:
			if (sfp_data.options == QLA_READ_SFP_CACHED_DATA_SKIP_CACHE) {
				vha->hw->sfp_cache.qla_sfp_a0_cache_valid = 0;
				vha->hw->sfp_cache.qla_sfp_a2_cache_valid = 0;
			}

			rval = qla2x00_read_sfp(vha, 0, sfp_data.a0,
						0xA0, 0, QLA_A0_CACHED_OFFSETS, BIT_1);
			if (rval != QLA_SUCCESS) {
				bsg_reply->reply_data.vendor_reply.vendor_rsp[0] = EXT_STATUS_ERR;
				goto done;
			}
			rval = qla2x00_read_sfp(vha, 0, sfp_data.a2,
						0xA2, 0, QLA_A2_CACHED_OFFSETS, BIT_1);
			if (rval != QLA_SUCCESS) {
				bsg_reply->reply_data.vendor_reply.vendor_rsp[0] = EXT_STATUS_ERR;
				goto done;
			}
			break;
		case QLA_READ_SFP_COMPL_DATA:
		case QLA_READ_SFP_COMPL_DATA_SKIP_CACHE:
			if (sfp_data.options == QLA_READ_SFP_COMPL_DATA_SKIP_CACHE) {
				vha->hw->sfp_cache.qla_sfp_a0_cache_valid = 0;
				vha->hw->sfp_cache.qla_sfp_a2_cache_valid = 0;
			}

			rval = qla2x00_read_sfp_dev(vha, NULL, 0);
			if (rval != QLA_SUCCESS) {
				bsg_reply->reply_data.vendor_reply.vendor_rsp[0] = EXT_STATUS_ERR;
				goto done;
			}
			memcpy(sfp_data.a0, (uint8_t*)vha->hw->sfp_data, 256);
			memcpy(sfp_data.a2, ((uint8_t*)vha->hw->sfp_data)+256, 256);
			break;
		default:

			ql_log(ql_log_warn, vha, 0x70db,
			    "Unsupported option provided in BSG call.\n");
			bsg_reply->reply_data.vendor_reply.vendor_rsp[0] = EXT_STATUS_ERR;
			goto done;
	}
	...
	sg_copy_from_buffer(bsg_job->reply_payload.sg_list,
		bsg_job->reply_payload.sg_cnt, &sfp_data, sizeof(sfp_data));
	bsg_reply->reply_payload_rcv_len = sizeof(sfp_data);
...
qla2x00_get_drv_attr(bsg_job_t *bsg_job)
{
	...
	drv_attr.ext_attributes |= QLA_RPORT_INFO_SUPPORT;
	drv_attr.ext_attributes |= QLA_DRV_EXT_ATTR_SFP_FW_LOAD_SUPPORT;
	drv_attr.ext_attributes |= QLA_DRV_MAINTENANCE_MODE_SUPPORT;
	drv_attr.ext_attributes |= QLA_CACHED_SFP_DATA_SUPPORT;
	drv_attr.ext_attributes |= QLA_IMG_SET_VALID_SUPPORT;

	if (vha->hw->flags.maint_mode)
		drv_attr.status_flags |= QLA_STATUS_FLAG_DRV_MAINT_MODE;
...
qla2x00_get_port_scm_v2(bsg_job_t *bsg_job)
{
	...
	scm_stats->li_uncorrectable_fec_count = ha->scm.li_uncorrectable_fec_count;
...
qla2x00_get_target_scm_v2(bsg_job_t *bsg_job)
{
	...
	scm_stats->li_uncorrectable_fec_count = fcport->scm.li_uncorrectable_fec_count;
...
qla2x00_bidi_scm_mgmt(bsg_job_t *bsg_job)
{
	...
	if (scm_config->controls & QLA_APPLY_SCMR_PROFILE) {
		...
		qla_scm_update_profile(vha, scm_config->profile.scmr_profile);
		ha->sfc.profile.scmr_control_flags = scm_config->profile.scmr_control_flags;
...
qla2x00_get_host_stats(bsg_job_t *bsg_job)
{
	...
	tgt_num = qla2x00_get_num_tgts(vha, tmp_stat_type);
...
/* 2 entries for  QLA2XX_TGT_SEND_LOGO + QLA2XX_TGT_SHT_LNK_DOWN */
#define TGT_STAT_ENTRIES 2
...
qla2x00_get_tgt_stats(bsg_job_t *bsg_job)
{
	...
	if (req_data_len != sizeof(*req_data)) {
		ql_log(ql_log_warn, vha, 0x0000, "req_data_len invalid.\n");
		return -EIO;
	}
	...
	response_len = sizeof(struct ql_vnd_tgt_stats_resp) +
		sizeof(struct ql_vnd_stat_entry) * TGT_STAT_ENTRIES;
...
static int qla_read_rport_info(bsg_job_t *bsg_job)
{
	struct fc_bsg_reply *bsg_reply = bsg_job->reply;
	scsi_qla_host_t *vha = shost_priv(fc_bsg_to_shost(bsg_job));
	struct fc_port *fcport;
	struct qla_rport_info_reply *reply = NULL;
	struct qla_rport_info *elem = NULL;
	u32 size;
	u16 port_total, cnt, i;
	int rc = 0;

	if (!bsg_job->reply || !bsg_job->reply_len ||
	    (bsg_job->reply_len < (sizeof(*reply) + sizeof(*elem)))) {
		ql_dbg(ql_dbg_user, vha, 0x7039,
		    "%s: user fail to provide return buffer.\n", __func__);
		rc = -EINVAL;
		goto done;
	}
	...
	/* cnt should be at least 1 here */
	cnt = (bsg_job->reply_payload.payload_len - sizeof(*reply))/ sizeof(*elem);

	if (cnt > port_total)
		cnt = port_total;

	size = sizeof(*reply) + (cnt * sizeof(*elem));
	reply = kzalloc(size, GFP_KERNEL);
	if (!reply) {
		ql_dbg(ql_dbg_user, vha, 0x7074,
		    "%s: alloc failed.\n", __func__);
		rc = -ENOMEM;
		goto done;
	}
	...
	list_for_each_entry(fcport, &vha->vp_fcports, list) {
		...
		elem->nport_id = fcport->d_id;

		elem->initiator_mode = (fcport->port_type & (FCT_INITIATOR|FCT_NVME_INITIATOR)) ? 1 : 0;
		elem->target_mode    = (fcport->port_type & (FCT_TARGET|FCT_NVME_TARGET)) ? 1 : 0;
		elem->type_fcp       = (fcport->port_type & (FCT_INITIATOR|FCT_TARGET)) ? 1 : 0;
		elem->type_nvme      = (fcport->port_type & (FCT_NVME_TARGET|FCT_NVME_INITIATOR)) ? 1 : 0;
		elem->online         = (atomic_read(&fcport->state) == FCS_ONLINE) ? 1 : 0;
		memcpy(elem->wwpn, fcport->port_name, 8);
		memcpy(elem->wwnn, fcport->node_name, 8);

		if (fcport->flags & FCF_FCSP_DEVICE) {
			elem->secure_device = 1;
			elem->auth_fail_cnt = fcport->edif.auth_fail_cnt;
			elem->auth_success_cnt = fcport->edif.auth_success_cnt;
			elem->auth_state = fcport->edif.priv_auth_state;
		}
	...
	reply->port_count = i;
	reply->port_total = port_total;
	reply->version = 1;

do_respond:
	bsg_reply->reply_payload_rcv_len = sg_copy_from_buffer(bsg_job->reply_payload.sg_list,
		bsg_job->reply_payload.sg_cnt, reply, size);
	bsg_reply->reply_data.vendor_reply.vendor_rsp[0] = EXT_STATUS_OK;
	bsg_reply->result = DID_OK << 16;
	bsg_job->reply_len = sizeof(struct fc_bsg_reply);

	ql_dbg(ql_dbg_user, vha, 0x7075,
	    "%s: port_count %d, port_total %d size %d payload_len %d.\n",
	    __func__, reply->port_count, reply->port_total, size, bsg_job->reply_payload.payload_len);
...
qla2x00_process_vendor_specific(struct scsi_qla_host *vha, bsg_job_t *bsg_job)
{
	...
	case QL_VND_READ_SFP:
		return qla27xx_read_cached_sfp_data(bsg_job);
	...
	case QL_VND_GET_RPORT_INFO:
		return qla_read_rport_info(bsg_job);

	case QL_VND_SFP_FW_LOAD:
		return qla27xx_do_sfp_fw_update(bsg_job);
...
static bool qla_let_bsg_through(scsi_qla_host_t *vha, bsg_job_t *bsg_job)
{
	struct fc_bsg_request *bsg_request = bsg_job->request;
	bool rc = false;
...
qla24xx_bsg_request(bsg_job_t *bsg_job)
{
	...
	if (qla_let_bsg_through(vha, bsg_job))
		goto skip_chip_chk;
...
static bool qla_bsg_found(struct qla_qpair *qpair, bsg_job_t *bsg_job)
{
	...
	if (ha->flags.eeh_busy) {
		...
		ql_dbg(ql_dbg_user, vha, 0x802c,
		    "eeh encounter.  bsg %px sp=%px handle=%x \n",
		    bsg_job, sp, sp->handle);
	} else {
		rval = ha->isp_ops->abort_command(sp);
		ql_dbg(ql_dbg_user, vha, 0x802c,
		    "Aborting bsg %px sp=%px handle=%x rval=%x\n",
		...
		if (!wait_for_completion_timeout(&comp, ratov_j)) {
			ql_log(ql_log_info, vha, 0x7089,
			    "bsg abort timeout.  bsg=%px sp=%px handle %#x .\n",
			    bsg_job, sp, sp->handle);
			...
			ql_dbg(ql_dbg_user, vha, 0x708a,
			    "bsg abort success. bsg %px sp=%px handle=%#x\n",
			    bsg_job, sp, sp->handle);
			do_bsg_done = false;
                }
                break;
		...
		ql_log(ql_log_info, vha, 0x704f,
		    "bsg abort fail.  bsg=%px sp=%px rval=%x.\n",
		    bsg_job, sp, rval);
	...
	*/
	...
	ql_dbg(ql_dbg_user, vha, 0x7051,
	    "%s bsg_job_done : bsg %px result %#x sp %px.\n",
...
qla24xx_bsg_timeout(bsg_job_t *bsg_job)
{
	...
	ql_log(ql_log_info, vha, 0x708b,
	    "%s CMD timeout. bsg ptr %px msgcode %x vendor cmd %x\n",
...
static int
qla28xx_do_validate_flash_image(bsg_job_t *bsg_job, uint16_t *state)
{
	struct fc_bsg_request *bsg_request = bsg_job->request;
	scsi_qla_host_t *vha = shost_priv(fc_bsg_to_shost(bsg_job));
	uint16_t mstate[16];
...
static void qla27xx_update_sfp_fw_load_info(struct qla_hw_data *ha)
{
	ha->fw_load_info.mbx1 = ha->sfp_fw_update_gdb.mbx2f_1;
	ha->fw_load_info.mbx2 = ha->sfp_fw_update_gdb.mbx2f_2;
	ha->fw_load_info.mbx3 = ha->sfp_fw_update_gdb.mbx2f_3;
	ha->fw_load_info.mbx4 = ha->sfp_fw_update_gdb.mbx2f_4;
}

void
qla27xx_update_sfp_fw_update_gdb(struct qla_hw_data *ha, mbx_cmd_t *mcp)
{
	ha->sfp_fw_update_gdb.mbx2f_0 = mcp->mb[0];
	ha->sfp_fw_update_gdb.mbx2f_1 = mcp->mb[1];
	ha->sfp_fw_update_gdb.mbx2f_2 = mcp->mb[2];
	ha->sfp_fw_update_gdb.mbx2f_3 = mcp->mb[3];
	ha->sfp_fw_update_gdb.mbx2f_4 = mcp->mb[4];
}

static void qla27xx_update_sfp_fw_request(struct qla_hw_data *ha)
{
	...
	if ((!qla2x00_reset_active(vha)) &&
		...
		if (ha->isp_ops->abort_isp(vha)) {
			/* abort isp is called if SFP FW update is failed */
			set_bit(ISP_ABORT_NEEDED, &vha->dpc_flags);
			qla2xxx_wake_dpc(vha);
		}
...
void
qla27xx_sfp_fw_update_work_fn(struct work_struct *work)
{
	struct qla_hw_data *ha =
		container_of(work, struct qla_hw_data, sfpfw_update_work);
	qla27xx_update_sfp_fw_request(ha);
}

static int
qla27xx_do_sfp_fw_update_request(bsg_job_t *bsg_job)
{
	struct fc_bsg_request *bsg_request = bsg_job->request;
	struct fc_bsg_reply *bsg_reply = bsg_job->reply;
	scsi_qla_host_t *vha = shost_priv(fc_bsg_to_shost(bsg_job));
	struct qla_hw_data *ha = vha->hw;
	int rval = -EINVAL;
	unsigned int req_data_len = 0, rsp_data_len = 0;
	void *req_data;
	void *rsp_data;
	dma_addr_t req_data_dma;

	rsp_data_len = sizeof(struct qla_sfp_fw_load_info);
	req_data_len = bsg_job->request_payload.payload_len;
	memset(&ha->fw_load_info, 0, rsp_data_len);

	if (atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEFAILED ||
		atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEPASSED) {
		atomic_set(&ha->sfp_fw_update_gdb.fw_update_status, SFPFWUPDATEINACTIVE);
	}

	/* Check global DB in progress before updating SFP FW */
	if (atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEINACTIVE) {
		memset(&ha->sfp_fw_update_gdb, 0, sizeof(ha->sfp_fw_update_gdb));

		req_data = dma_alloc_coherent(&ha->pdev->dev, req_data_len,
				&req_data_dma, GFP_KERNEL);
		if (!req_data) {
			ql_log(ql_log_warn, vha, 0xf0a7,
				"%s dma alloc failed for req_data.\n", __func__);
			rval = -ENOMEM;
			bsg_reply->reply_data.vendor_reply.vendor_rsp[0] =
				EXT_STATUS_NO_MEMORY;
			goto dma_alloc_failed_out;
		}

		atomic_set(&ha->sfp_fw_update_gdb.fw_update_status, SFPFWUPDATEBUSY);
		ha->flags.sfp_fw_update_req = 1;

		/* Copy the request buffer in req_data now */
		sg_copy_to_buffer(bsg_job->request_payload.sg_list,
			bsg_job->request_payload.sg_cnt, req_data, req_data_len);

		ha->sfp_fw_update_gdb.options =
			bsg_request->rqst_data.h_vendor.vendor_cmd[2];
		ha->sfp_fw_update_gdb.buffer = req_data;
		ha->sfp_fw_update_gdb.dma_addr = req_data_dma;
		ha->sfp_fw_update_gdb.len = req_data_len;

		queue_work(ha->wq, &ha->sfpfw_update_work);
		...
		bsg_reply->reply_data.vendor_reply.vendor_rsp[0] =
				EXT_STATUS_SFP_FW_UPDATE_IN_PROGRESS;
		rval = QLA_SUCCESS;

	}

	rsp_data = (void *) &ha->fw_load_info;
	sg_copy_from_buffer(bsg_job->reply_payload.sg_list,
			bsg_job->reply_payload.sg_cnt, rsp_data, rsp_data_len);
dma_alloc_failed_out:
	bsg_reply->reply_payload_rcv_len = rsp_data_len;
	bsg_job->reply_len = sizeof(struct fc_bsg_reply);
	bsg_reply->result = DID_OK << 16;
	bsg_job_done(bsg_job, bsg_reply->result,
			bsg_reply->reply_payload_rcv_len);
...
static void qla27xx_sfp_fw_free_mem(struct qla_hw_data *ha)
{
	...
	req_data = ha->sfp_fw_update_gdb.buffer;
	req_data_dma = ha->sfp_fw_update_gdb.dma_addr;
	req_data_len = ha->sfp_fw_update_gdb.len;

	if (ha->sfp_fw_update_gdb.buffer) {
		dma_free_coherent(&ha->pdev->dev, req_data_len, req_data, req_data_dma);
		ha->sfp_fw_update_gdb.buffer = NULL;
	}
}

static int
qla27xx_get_sfp_fw_update_result(bsg_job_t *bsg_job)
{
	struct fc_bsg_reply *bsg_reply = bsg_job->reply;
	scsi_qla_host_t *vha = shost_priv(fc_bsg_to_shost(bsg_job));
	void *rsp_data = NULL;
	...
	if (atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEPASSED) {
		ql_log(ql_log_info, vha, 0xf0a8,
			"SFP FW Update is successfully completed\n");
		bsg_reply->reply_data.vendor_reply.vendor_rsp[0] =
				EXT_STATUS_OK;
		qla27xx_sfp_fw_free_mem(ha);
		goto out;
	}
	if (atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEFAILED) {
		ql_log(ql_log_warn, vha, 0xf0a9,
			"SFP FW Update is Failed\n");
		bsg_reply->reply_data.vendor_reply.vendor_rsp[0] =
				EXT_STATUS_ERR;
		qla27xx_sfp_fw_free_mem(ha);
		goto out;
	...
	if (atomic_read(&ha->sfp_fw_update_gdb.fw_update_status) == SFPFWUPDATEBUSY) {
		ql_log(ql_log_info, vha, 0xf0aa,
			"SFP FW Update is in progress\n");
		bsg_reply->reply_data.vendor_reply.vendor_rsp[0] =
				EXT_STATUS_SFP_FW_UPDATE_IN_PROGRESS;
	}
...
out:
	qla27xx_update_sfp_fw_load_info(ha);
...
static int qla27xx_do_sfp_fw_update(bsg_job_t *bsg_job)
{
	...
	if (bsg_request->rqst_data.h_vendor.vendor_cmd[1] ==
		QL_VND_SC_SFP_FW_UPDATE) {
		rval = qla27xx_do_sfp_fw_update_request(bsg_job);
	}

	if (bsg_request->rqst_data.h_vendor.vendor_cmd[1] ==
		QL_VND_SC_SFP_FW_RESULT) {
		rval = qla27xx_get_sfp_fw_update_result(bsg_job);
	}
	return rval;
}
