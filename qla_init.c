...
static void qla_free_session_done(struct work_struct *work);

/* SRB Extensions ---------------------------------------------------------- */

void
qla2x00_sp_timeout(qla_timer_arg_t t)
{
	srb_t *sp = qla_from_timer(sp, t, u.iocb_cmd.timer);
    ...
    uint16_t *data)
{
	...
	if (!vha->flags.online || (fcport->flags & FCF_ASYNC_SENT) ||
		...
		ql_log(ql_log_warn, vha, 0xffff,
		    ...
		    __func__, fcport->port_name, vha->flags.online,
		    fcport->flags);
		goto done;
	}
...
done_free_sp:
	...
	/*
	 * async login failed. Could be due to iocb/exchange resource being low.
	 * Set state DELETED for re-login process to start again.
	 */
...
static void qla2x00_async_logout_sp_done(srb_t *sp, int res)
{
	sp->fcport->flags &= ~(FCF_ASYNC_SENT | FCF_ASYNC_ACTIVE);
	sp->fcport->login_gen++;
	qla_logo_completion_handler(sp->fcport, sp->u.iocb_cmd.u.logio.data[0]);
	/* ref: INIT */
	kref_put(&sp->cmd_kref, qla2x00_sp_release);
}
    ...
    uint16_t *data)
{
	...
	qla_logo_completion_handler(fcport, data[0]);
}
...
void qla24xx_handle_adisc_event(scsi_qla_host_t *vha, struct event_arg *ea)
{
	...
	if (ea->data[0] != MBS_COMMAND_COMPLETE) {
		...
		qla_schedule_sess_for_deletion(ea->fcport);
		return;
	}
			...
			for (i = 0; i < n; i++) {
				...
				if (fcport->d_id.b24 == id.b24) {
					...
					if (conflict_fcport) {
						...
						qla_schedule_sess_for_deletion
							(conflict_fcport);
					}
				}
				/*
...
int qla24xx_fcport_handle_login(struct scsi_qla_host *vha, fc_port_t *fcport)
{
	...
	ql_dbg(ql_dbg_disc, vha, 0x20d8,
	    "%s %8phC DS %d LS %d P %d fl %x rscn %d|%d login %d lid %d scan %d fc4type %x\n",
	    __func__, fcport->port_name, fcport->disc_state,
	    fcport->fw_login_state, fcport->login_pause, fcport->flags,
	    fcport->last_rscn_gen, fcport->rscn_gen, fcport->login_gen,
	    fcport->loop_id, fcport->scan_state, fcport->fc4_type);
	...
	struct event_arg *ea)
{
	...
	ql_dbg(ql_dbg_disc, vha, 0x2102,
	    "%s %8phC DS %d LS %d P %d del %d rscn %d|%d login %d|%d fl %x\n",
	    __func__, fcport->port_name, fcport->disc_state,
	    fcport->fw_login_state, fcport->login_pause,
	    fcport->deleted, fcport->last_rscn_gen,
	    fcport->rscn_gen, fcport->last_login_gen,
	    fcport->login_gen, fcport->flags);
...
int qla_tmf_wait(struct tmf_arg *arg)
{
...
__qla2x00_async_tm_cmd(struct tmf_arg *arg)
{
	...
	if (!test_bit(UNLOADING, &vha->dpc_flags) && !IS_QLAFX00(vha->hw)) {
		...
		if (chip_gen == vha->hw->chip_reset && login_gen == fcport->login_gen) {
			...
			ql_log(ql_log_info, vha, 0x803e,
			    "Skip Maker due to disruption. Nexus=%ld:%06x:%llu.\n",
...
void qla_put_tmf(struct tmf_arg *arg)
{
	struct scsi_qla_host *vha = arg->vha;
	struct qla_hw_data *ha = vha->hw;
	unsigned long flags;
...
qla2x00_initialize_adapter(scsi_qla_host_t *vha)
{
	...
	ha->sfp_cache.qla_sfp_a0_cache_valid = 0;
	ha->sfp_cache.qla_sfp_a2_cache_valid = 0;
	ha->sfp_cache.qla_a2_expires_after = 0;
	...
	if (qla2x00_isp_firmware(vha) != QLA_SUCCESS) {
		...
		if (rval) {
			ql_log(ql_log_fatal, vha, 0x004b,
			    "Chip Diag failed(%x).\n", rval);
			return (rval);
		}
	...
	if (rval != QLA_SUCCESS) {
		ql_log(ql_log_fatal, vha, 0x00f7,
		    "Init FW failed(%x).\n", rval);
		return rval;
	}
...
qla_save_mbregs(scsi_qla_host_t *vha)
{
	...
	if ((!IS_QLA27XX(ha) && !IS_QLA28XX(ha)) || vha->flags.init_done)
		return;
		...
		if (!fw_dump) {
			...
			if (ha->fw_dumped || ha->mpi_fw_dumped) {
				...
				if (IS_QLA27XX(ha) || IS_QLA28XX(ha)) {
					...
					ha->mpi_fw_dump = (char *)fw_dump + ha->fwdt[0].dump_size;

					if (ha->mpi_fw_dumped)
						memcpy(ha->mpi_fw_dump, prev_mpi_fw_dump,
						    ha->mpi_fw_dump_len);
...
void __qla_adjust_iocb_limit(struct qla_qpair *qpair)
{
	u8 num_qps;
	u16 limit;
	...
	qpair->fwres.exch_limit = (ha->orig_fw_xcb_count * QLA_IOCB_PCT_LIMIT)/100;
}
...
execute_fw_with_lr:
	...
	if (rval == QLA_SUCCESS) {
		...
		if (rval == QLA_SUCCESS) {
			...
			*/
			if (ql2xallocfwdump && (IS_QLA27XX(ha) || IS_QLA28XX(ha)) &&
			    !vha->flags.init_done)
			...
			if (rval == QLA_SUCCESS) {
				...
				if (rval != QLA_SUCCESS) {
					ql_log(ql_log_fatal, vha, 0x00bf,
					    "Extended Login setup failed(%x).\n", rval);
					goto failed;
				}

				rval = qla2x00_set_exchoffld_buffer(vha);
				if (rval != QLA_SUCCESS) {
					ql_log(ql_log_fatal, vha, 0x0169,
					    "Exchange offload setup failed(%x).\n", rval);
					goto failed;
				}
...
enable_82xx_npiv:
				...
				if (IS_P3P_TYPE(ha)) {
					...
					if (rval != QLA_SUCCESS) {
						ql_log(ql_log_fatal, vha, 0x00c2,
						    "Get FW Version failed(%x).\n", rval);
						goto failed;
					}
				}
			...
			ql_log(ql_log_fatal, vha, 0x00cd,
			    "ISP Firmware failed checksum(%x).\n", rval);
			goto failed;
		}
		...
		ql_log(ql_log_fatal, vha, 0x0091,
		    "Load Risc failed(%x).\n", rval);
		goto failed;
	}
	...
	if (IS_QLA27XX(ha) || IS_QLA28XX(ha))
		...
		if (rval == QLA_SUCCESS) {
			...
			ql_log(ql_log_info, vha, 0x00ce,
			    "Flash Access Control MB cmd failed (%d).\n",
			    rval);
			if (IS_QLA83XX(ha)) {
...
qla2x00_update_fw_options(scsi_qla_host_t *vha)
{
	...
	/* Enable PUREX */
	ha->fw_options[1] |= FO1_ENABLE_PURE_IOCB;
...
qla24xx_update_fw_options(scsi_qla_host_t *vha)
{
	...
	if (ql2xrdpenable || ha->flags.scm_supported_f ||
		...
		ha->fw_options[3] |= ADD_FO3_COPY_FLOGI_ACC_PL;
		ha->flags.flogi_acc_enabled = 1;
	}
...
qla24xx_config_rings(struct scsi_qla_host *vha)
{
	...
	if (ha->set_data_rate) {
		...
		temp = le32_to_cpu(icb->firmware_options_3);
		temp &= ~FWO3_DATA_RATE_MASK;
		temp |= (ha->set_data_rate << FWO3_DATA_RATE_SHIFT);
		icb->firmware_options_3 |= cpu_to_le32(temp);
	}
...
void qla2x00_set_fcport_state(fc_port_t *fcport, int state)
{
	...
	if (state == FCS_ONLINE)
		fcport->online_time = jiffies;
	else if (old_state == FCS_ONLINE)
		fcport->offline_time = jiffies;
...
qla2x00_alloc_fcport(scsi_qla_host_t *vha, gfp_t flags)
{
	...
	fcport->vha = vha;
	fcport->sfc.vha = vha;
	fcport->sfc.mode = ql2x_scmr_throttle_mode;
	memcpy(&fcport->sfc.profile, &vha->hw->sfc.profile,
		sizeof(struct qla_scmr_port_profile));
	fcport->scm.last_event_timestamp = qla_get_real_seconds();
	qla_scmr_set_tgt(&fcport->sfc);
	fcport->sfc.fcport = fcport;
	fcport->sfc.rstats = &fcport->scm.rstats;
	...
	memcpy(fcport->sfc.scmr_down_delta, vha->hw->sfc.scmr_down_delta,
			    sizeof(fcport->sfc.scmr_down_delta));
	memcpy(fcport->sfc.scmr_up_delta, vha->hw->sfc.scmr_up_delta,
			    sizeof(fcport->sfc.scmr_up_delta));
	...
	INIT_WORK(&fcport->free_work, qla_free_session_done);
...
qla2x00_configure_fabric(scsi_qla_host_t *vha)
{
	...
	do {
		...
		/*
		* Check if FW has support for SCM and if driver has to send EDC and RDF
		* Get the features from firmware and use that to build payload
		*/
		if (QLA_DRV_SEND_ELS(ha)) {
			ql_dbg(ql_dbg_scm, vha, 0xffff,
				"Adapter and Firmware support SCM, send EDC and RDF \n");
			if (!qla2xxx_scm_get_features(vha))
				qla2xxx_send_uscm_els(vha);
		}
...
qla2x00_abort_isp_cleanup(scsi_qla_host_t *vha)
{
	...
	loop_state = atomic_read(&vha->loop_state);
	if (loop_state == LOOP_UP || loop_state == LOOP_READY) {
	...
	qla_scm_clear_host(vha);
	qla_scm_clear_all_tgt_sess(vha);
	ha->flags.scm_enabled = 0;
	ha->flags.conn_fabric_cisco_er_rdy = 0;
	ha->flags.conn_fabric_brocade = 0;
	ha->flags.flogi_acc_enabled = 0;
	ha->flags.flogi_acc_pl_in_cont_iocb = 0;
	ha->flags.scm_supported_vl = 0;
	memset(&ha->flogi_acc, 0, sizeof(struct flogi_acc_payld));
		...
		if (!qla2x00_restart_isp(vha)) {
			...
			if (test_bit(ISP_ABORT_RETRY, &vha->dpc_flags)) {
				if (ha->isp_abort_cnt == 0) {
					if (IS_QLA27XX(ha) || IS_QLA28XX(ha)) {
						ql_log(ql_log_fatal, vha, 0x803c,
...
static int
qla27xx_update_sfp_fw(scsi_qla_host_t *vha)
{
	int rval = 0;
	struct qla_hw_data *ha = vha->hw;
	mbx_cmd_t mc;
	...
	rval = qla27xx_update_sfp_fw_mb(vha, mcp);

	if (rval == QLA_SUCCESS) {
		ql_log(ql_log_info, vha, 0xf0ab,
			"%s SFP FW Update successfully completed  rval = %d\n",
			__func__, rval);
		atomic_set(&ha->sfp_fw_update_gdb.fw_update_status,
			   SFPFWUPDATEPASSED);
	} else if (rval == QLA_FUNCTION_TIMEOUT) {
		ql_log(ql_log_info, vha, 0xf0af,
			"%s SFP FW Update timeout rval = %d\n", __func__, rval);
		ha->isp_ops->fw_dump(vha);
		atomic_set(&ha->sfp_fw_update_gdb.fw_update_status,
			   SFPFWUPDATEFAILED);
	} else {
		/* We don't suspend abort_isp() on mbx 0x2f failure */
		rval = QLA_SUCCESS;
		ql_log(ql_log_info, vha, 0xf0af,
			"%s SFP FW Update Failed rval = %d\n", __func__, rval);
		atomic_set(&ha->sfp_fw_update_gdb.fw_update_status,
			   SFPFWUPDATEFAILED);
	}

	qla27xx_update_sfp_fw_update_gdb(ha, mcp);
...
qla2x00_restart_isp(scsi_qla_host_t *vha)
{
	...
	if (ha->flags.sfp_fw_update_req) {
		ha->flags.sfp_fw_update_req = 0;
		status = qla27xx_update_sfp_fw(vha);
		if (status)
			return status;
	}
	...
	while (*seg && rval == QLA_SUCCESS) {
		...
		*srisc_addr = *srisc_addr == 0 ? *seg : *srisc_addr;
		...
		while (risc_size > 0 && rval == QLA_SUCCESS) {
			...
			for (i = 0; i < wlen; i++)
				wcode[i] = swab16((__force u16)fwcode[i]);
...
qla81xx_nvram_config(scsi_qla_host_t *vha)
{
	...
	if (chksum || memcmp("ISP ", nv->id, sizeof(nv->id)) ||
		...
		if (IS_QLA28XX(ha) || IS_QLA27XX(ha)) {
			nv->scmr_throttle_profile = 1; /* Conservative */
			nv->scmr_control_flags = 0; /* Driver default */
		}
		rval = 1;
	}
	...
	/* Reset the flag - in case SCM/VL is disabled via CLI */
	ha->flags.scm_supported_a = 0;
	ha->flags.scm_supported_vl = 0;
	...
	if (IS_QLA28XX(ha) || IS_QLA27XX(ha)) {
		...
		if ((le16_to_cpu(nv->enhanced_features) & BIT_7) == 0) {
			...
			/* Check if VL is enabled in NVRAM or mod param */
			if ((nv->port_features & BIT_2) || (ql2xvirtuallane)) {
				ql_log(ql_log_info, vha, 0x0073,
				    "Virtual Lane Support enabled via NVRAM:%u"
				    " and/or Mod Param:%d\n", (nv->port_features >> 2), ql2xvirtuallane);
				ha->flags.scm_supported_vl = 1;
				icb->firmware_options_3 |= cpu_to_le32(BIT_2);
			}
		}
	}
	...
	/* Set USCM profile */
	ha->sfc.profile.scmr_control_flags = nv->scmr_control_flags;
	if (nv->scmr_control_flags & QLA_USE_NVRAM_CONFIG) {
		/* Use NVRAM values */
		qla_scm_update_profile(vha, nv->scmr_throttle_profile);
		ql_log(ql_log_info, vha, 0x0077,
		    "SCM profile from NVRAM:%s\n",
			qla_get_profile_type(ha->sfc.profile.scmr_profile));
	} else { /* Use module param */
		qla_scm_update_profile(vha, ql2x_scmr_profile);
		ql_log(ql_log_info, vha, 0x0078,
		    "SCM profile from driver:%s\n",
			qla_get_profile_type(ha->sfc.profile.scmr_profile));
	}
	/* We might choose to get these values from NVRAM in the future */
	qla2xxx_scmr_init_deltas(&ha->sfc);
...
uint64_t
qla2x00_get_num_tgts(scsi_qla_host_t *vha, u32 stat_type)
{
	fc_port_t *f, *tf;
	u64 count = 0;
	...
	list_for_each_entry_safe(f, tf, &vha->vp_fcports, list) {
		if (f->port_type != FCT_TARGET)
			continue;
		if (stat_type & QLA2XX_TGT_SHT_LNK_DOWN)
			count++;
		if (stat_type & QLA2XX_TGT_SEND_LOGO)
			count++;
	}
	return count;
}
...
int qla2xxx_reset_stats(struct Scsi_Host *host, u32 flags)
{
	...
	spin_lock_irqsave(&vha->hw->sess_lock, int_flags);
	list_for_each_entry(fcport, &vha->vp_fcports, list) {
		if (flags & QLA2XX_TGT_SHT_LNK_DOWN) {
			fcport->tgt_short_link_down_cnt = 0;
			fcport->tgt_link_down_time = QLA2XX_MAX_LINK_DOWN_TIME;
		}
		if (flags & QLA2XX_TGT_SEND_LOGO)
			fcport->tgt_send_logo_cnt = 0;
	}
	spin_unlock_irqrestore(&vha->hw->sess_lock, int_flags);
			  ...
			  void *data, u64 size)
{
	...
	num_tgt = qla2x00_get_num_tgts(vha, tmp_stat_type);
	tmp_stat_type &= ~(QLA2XX_TGT_SHT_LNK_DOWN|QLA2XX_TGT_SEND_LOGO);
	...
	if (flags & QLA2XX_LINK_DOWN) {
		rsp_data->entry[i].stat_type = QLA2XX_LINK_DOWN;
		rsp_data->entry[i].tgt_num = 0x0;
		rsp_data->entry[i].cnt = vha->link_down_cnt;
		i++;
	}
	...
	list_for_each_entry(fcport, &vha->vp_fcports, list) {
		...
		if (flags & QLA2XX_TGT_SEND_LOGO) {
			rsp_data->entry[i].stat_type = QLA2XX_TGT_SEND_LOGO;
			rsp_data->entry[i].tgt_num = fcport->rport->number;
			rsp_data->entry[i].cnt = fcport->tgt_send_logo_cnt;
			i++;
		}
		if (i >= entry_count)
			break;
...
int qla2xxx_get_tgt_stats(struct Scsi_Host *host, u32 flags,
    struct fc_rport *rport, struct ql_vnd_tgt_stats_resp *tgt_data, int num_entry)
{
	...
	if (flags & QLA2XX_TGT_SEND_LOGO) {
		tgt_data->stats.entry[i].stat_type = QLA2XX_TGT_SEND_LOGO;
		tgt_data->stats.entry[i].tgt_num = rport->number;
		tgt_data->stats.entry[i].cnt = fcport->tgt_send_logo_cnt;
	...
	if (flags & QLA2XX_TGT_SHT_LNK_DOWN) {
		tgt_data->stats.entry[i].stat_type = QLA2XX_TGT_SHT_LNK_DOWN;
		tgt_data->stats.entry[i].tgt_num = rport->number;
		tgt_data->stats.entry[i].cnt = fcport->tgt_short_link_down_cnt;
		tgt_data->stats.entry_count += 1;
		i++;
	}
	if (i >= num_entry)
...
#define QLA_NANO	1000000000L
uint64_t ts_to_ns(struct timespec64 *ts)
{
	uint64_t ns;

	ns = ts->tv_sec;
	ns *= QLA_NANO; /* sec to nanosec */
	ns += ts->tv_nsec;
...
void qla_get_scsi_cmd_latency(srb_t *sp)
{
	struct scsi_qla_host *vha = sp->vha;
	struct scsi_cmnd *cmd = GET_CMD_SP(sp);
	struct timespec64 ts1, ts2, ts3, ts_tot;
	uint8_t *c = cmd->cmnd;
	uint64_t ns_tot, ns1, ns2, ns3;
	int op;
	int type;
	...
	ts_tot = timespec64_sub(sp->cmd_to_ml, sp->q_cmd);
	ts1 = timespec64_sub(sp->cmd_to_req_q, sp->q_cmd);
	ts2 = timespec64_sub(sp->cmd_from_rsp_q, sp->cmd_to_req_q);
	ts3 = timespec64_sub(sp->cmd_to_ml, sp->cmd_from_rsp_q);

	ns_tot = ts_to_ns(&ts_tot);
	ns1 = ts_to_ns(&ts1);
	ns2 = ts_to_ns(&ts2);
	ns3 = ts_to_ns(&ts3);

	/* weed out outliers, use only entries with time < ~4s */
	if (ns_tot >= 0xffffffff || ns1 >= 0xffffffff ||
			ns2 >= 0xffffffff || ns3 >= 0xffffffff)
		vha->qla_stats.latency_outliers++;
	ns_tot = ns_tot < 0xffffffff ? ns_tot : 0;
	ns1 = ns1 < 0xffffffff ? ns1 : 0;
	ns2 = ns2 < 0xffffffff ? ns2 : 0;
	ns3 = ns3 < 0xffffffff ? ns3 : 0;

	vha->latency_counters.qla_tot_cmds[type] += 1;
	vha->latency_counters.qla_time_qcmd_to_ml[type] += ns_tot;
	vha->latency_counters.qla_time_qcmd_to_req_q[type] += ns1;
	vha->latency_counters.qla_time_req_q_to_rsp_q[type] += ns2;
	vha->latency_counters.qla_time_rsq_q_to_ml[type] += ns3;

}

void qla_get_nvme_cmd_latency(srb_t *sp)
{
	struct scsi_qla_host *vha = sp->vha;
	struct timespec64 ts1, ts2, ts3, ts_tot;
	uint64_t ns_tot, ns1, ns2, ns3;
	...
	/* For NVMe use read16 and write16 for READ and WRITE */
	...
	vha->latency_counters.qla_nvme_tot_cmds[type] += 1;
	vha->latency_counters.qla_nvme_qcmd_to_ml[type] += ns_tot;
	vha->latency_counters.qla_nvme_qcmd_to_req_q[type] += ns1;
	vha->latency_counters.qla_nvme_req_q_to_rsp_q[type] += ns2;
	vha->latency_counters.qla_nvme_rsp_q_to_ml[type] += ns3;
...
void qla_schedule_sess_for_deletion(struct fc_port *sess)
{
	unsigned long flags;
	u16 sec;
	...
	qla_scm_clear_session(sess);

	queue_work(sess->vha->hw->wq, &sess->free_work);
}

void qla_send_first_logo(struct scsi_qla_host *vha, qla_port_logo_t *logo)
{
	qla_port_logo_t *tmp;
	unsigned long flags;
	int res;
...
static void qla_free_session_done(struct work_struct *work)
{
	...
	ql_dbg(ql_dbg_disc, vha, 0xf084,
		"%s: sess %px from port %8phC loop_id %#04x"
	...
	if (!IS_SW_RESV_ADDR(sess->d_id)) {
		...
		if (sess->send_els_logo) {
			qla_port_logo_t logo;

			logo.id = sess->d_id;
			logo.cmd_count = 0;
			INIT_LIST_HEAD(&logo.list);
			qla_send_first_logo(vha, &logo);
			msleep(100);
