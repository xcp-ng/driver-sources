...
extern int fnic_get_debug_info(struct stats_debug_info *debug_buffer,
                struct fnic *fnic);
extern int fnic_get_stats_data(struct stats_debug_info *debug,
                        struct fnic *fnic);
extern int fnic_get_nvmef_info(struct fnic *fnic, struct fnic_nvmef_info *info);
...
int fnic_debugfs_init(void)
{
	...
	if (fc_trc_flag) {
		...
		printk(KERN_ALERT
			"fnic_debugfs_init: Mem Alloc failure\n");
		return rc;
	}
                                       ...
                                       struct file *file)
{
	...
	info = kzalloc(sizeof (struct fnic_nvmef_info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	...
	info->buffer_len = fnic_get_nvmef_info(fnic, info);
...
#if FNIC_HAVE_TIMESPEC64_SUB
		ktime_get_ts64(&t64);
		stats->stats_timestamps.last_reset_time=t64;
#else
#if FNIC_HAVE_TIMESPEC_SUB
		getnstimeofday(&stats->stats_timestamps.last_reset_time);
#else
		getnstimeofday(&t);
		stats->stats_timestamps.last_reset_time=timespec_to_timespec64(t);
#endif
#endif
	}
					...
					struct file *file)
{
	...
	debug->buffer_len = fnic_get_stats_data(debug, fnic);
...
int 
fnic_nvmef_debugfs_init(struct fnic *fnic)
{
	int rc = -1;
	char name[16];
	snprintf(name, sizeof (name), "host%d", fnic->fnic_num);
	if (!fnic_nvmef_debugfs_root) {
		printk(KERN_DEBUG "fnic_nvmef root doesn't exist\n");
		return rc;
	}
	fnic->fnic_nvmef_debugfs_host = debugfs_create_dir(name,
				fnic_nvmef_debugfs_root);
	if (!fnic->fnic_nvmef_debugfs_host) {
		printk(KERN_DEBUG "Cannot create host directory for nvmef\n");
		return rc;
	}
	...
	if (!fnic->fnic_nvmef_debugfs_file) {
		printk(KERN_DEBUG "Cannot create host nvmef file\n");
		return rc;
	}
