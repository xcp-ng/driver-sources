...
#ifndef NO_APEX
/**
 * lpfc_chk_params - memcmp two fields.
 * @ptra: uint8_t pointer.
 * @ptrb: uint8_t pointer.
 ...
 * @ptra: uint8_t pointer to portname.
 * @ptrb: uint8_t pointer to portname.
 * @size: size to compare.
 * returns 1 if equal, 0 if not equal
 **/
static int
 ...
 * @ptra: uint8_t pointer to 1st lun.
 * @ptrb: uint8_t pointer to 2nd lun.
 * @size: size to compare.
 * returns 1 if equal, 0 if not equal
 **/
static int
 ...
 * @ndlp: pointer to nodelist structure
 * @first_lun: 1st lun.
...
/**
 * lpfc_set_ndlps_fcp_priority - set the fcp priority for each rule.
 * @ndlp: pointer to nodelist structure
 *
 * This function walks the rules list and attempts to apply them to every vport
 * and ndlp.
 ...
 **/
static void
lpfc_clr_vports_fcp_priority(struct lpfc_hba *phba)
{
	struct lpfc_vport **vports;
	int i = 0;
...
#endif
{
	...
	struct lpfc_rport_data *rdata = NULL;
...
#if defined(BUILD_3_10_KERN)
	if (job->rport)
		rdata = job->rport->dd_data;
#else
	if (fc_bsg_to_rport(job))
		rdata = fc_bsg_to_rport(job)->dd_data;
#endif
	if (rdata)
		ndlp = rdata->pnode;
	...
	if (elscmd == ELS_CMD_RAW) {
		 ...
		 * The RAW IO intends the NPort_Id to be in the second word.
		 */
		did = raw_data[1];
		if (!did) {
			rc = -ENODEV;
		...
		ndlp = lpfc_refnode_did(vport, did);
		if (!ndlp) {
		...
		if (bsg_request->msgcode == FC_BSG_HST_ELS_NOLOGIN) {
			did = (bsg_request->rqst_data.h_els.port_id[0] << 16) |
			...
			/* This is not a RAW IO but might be a NOLOGIN request and
			 * the driver is not guaranteed the caller passed an NDLP in
			 * the job structure so needs to protect against that.
			 */
			if (!ndlp) {
				rc = -ENODEV;
				goto free_dd_data;
	...
	if (elscmd == ELS_CMD_RAW) {
		 ...
		 */
		ptr = (uint32_t *)cmdiocbq->cmd_dmabuf->virt;
		while (cmdsize > 0) {
			*ptr = *(ptr + 2);
			ptr++;
	...
	if (bsg_request->msgcode == FC_BSG_HST_ELS_NOLOGIN) {
		bf_set(wqe_els_did, &cmdiocbq->wqe.els_req.wqe_dest, did);
		cmdiocbq->cmd_flag |= LPFC_IO_RAW; /* To force IO by VPI */
	}

	/* Ensure the command timeout is < the bsg request timeout */
	maxtmo = FC_DEFAULT_BSG_TIMEOUT / HZ - 1;
	timeout = min(((2 * phba->fc_edtov) / 1000), maxtmo);

	rpi = ndlp->nlp_rpi;

	if (phba->sli_rev == LPFC_SLI_REV4) {
		bf_set(wqe_tmo, &cmdiocbq->wqe.generic.wqe_com, timeout);
...
#endif
{
	...
	ndlp = lpfc_refnode_did(phba->pport, phba->ct_ctx[tag].SID);
	if (!ndlp) {
		lpfc_printf_log(phba, KERN_WARNING, LOG_ELS,
...
#endif
{
	...
	if (phba->fcp_priority_rules) {
		lpfc_clr_vports_fcp_priority(phba);
...
#endif
{
	...
	if ((mbxstatus == MBX_SUCCESS) && (pmboxq->u.mb.mbxStatus == 0)) {
		...
		if (link_flags == INTERNAL_LOOP_BACK)
			pmboxq->u.mb.un.init_link.link_flags = cpu_to_le16(FLAGS_LOCAL_LB);
		else
			pmboxq->u.mb.un.init_link.link_flags =
				cpu_to_le16(FLAGS_TOPOLOGY_MODE_LOOP);
			 ...
			 uint16_t *txxri, uint16_t * rxxri)
{
	...
	*rxxri = 0;
	...
	if (!cmdiocbq || !rspiocbq || !dmabuf || !bpl ||
	    !ctreq || !dmabuf->virt) {
		ret_val = -ENOMEM;
...
/**
 * lpfc_bsg_mbox_ext_session_reset - clean up context of multi-buffer mbox
 *                                   session
 * @phba: Pointer to HBA context object.
 *
...
/**
 * lpfc_manage_auth_pwd_update - Update password information.
 * @vport: Pointer to a vport object.
...
lpfc_manage_auth_pwd_update(struct lpfc_vport *vport, uint8_t *buf)
{
	...
	if (entry) {
		...
		if (ndlp &&
			...
			/* Release lookup reference from auth_findnode */
			lpfc_nlp_put(ndlp);
 ...
 * @phba: Pointer to HBA context object.
 * @buf: Pointer to a list of configuration descriptors.
 * @wwpn: Pointer to the wwpn to be matched
	...
	for (i = 0; i < item_cnt_p; i++) {
		...
		for (j = 0; j < d_cfg->hdr.entry_cnt; j++) {
			...
			if ((memcmp(&d_item->local_wwn[0],
				...
				/* Release lookup ref from auth_findnode */
				if (ndlp)
					lpfc_nlp_put(ndlp);
...
static int
#if defined(BUILD_3_10_KERN)
lpfc_manage_auth_cfg(struct fc_bsg_job *job)
#else
...
static int
#if defined(BUILD_3_10_KERN)
lpfc_get_cgnbuf_info(struct fc_bsg_job *job)
#else
