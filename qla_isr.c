...
static irqreturn_t qla83xx_msix_atio_q(int irq, void *dev_id);
...
static inline void display_Laser_info(scsi_qla_host_t *vha,
	...
	if (SFP_DISABLE_LASER_INITIATED == mb1)
		ql_log(ql_log_warn, vha, 0xf0a2, "SFP temperature (%d C) \
			reached/exceeded the threshold (%d C). Laser is \
			disabled.\n", mb3, mb2);
	if (SFP_ENABLE_LASER_INITIATED == mb1)
		ql_log(ql_log_warn, vha, 0xf0a3, "SFP temperature (%d C) \
			reached normal operating level. Laser is enabled.\n",
			mb3);
}
...
qla27xx_copy_multiple_pkt(struct scsi_qla_host *vha, void **pkt,
		      struct rsp_que **rsp, bool is_purls, bool byte_order)
{
...
skip_rio:
	...
	case MBA_PORT_UPDATE:		/* Port database update */
		...
		if (mb[2] == 0x7) {
			...
			if ((mb[3] & MPU_REASON_MASK) == MPU_REASON_LOGO_RCV)
				fcport->tgt_send_logo_cnt++;
...
out:
	if (logit)
		ql_dbg(ql_dbg_io, fcport->vha, 0x3022,
		       "FCP command status: 0x%x-0x%x (0x%x) nexus=%ld:%d:%llu portid=%02x%02x%02x oxid=0x%x cdb=%10phN len=0x%x rsp_info=0x%x resid=0x%x fw_resid=0x%x sp=%px cp=%p.\n",
		       comp_status, scsi_status, res, vha->host_no,
...
process_err:
		...
		case PUREX_IOCB_TYPE:
			...
			case ELS_COMMAND_FPIN:
				...
				pure_item = qla27xx_copy_multiple_pkt(vha,
							  (void **)&pkt, &rsp,
							  false, true);
