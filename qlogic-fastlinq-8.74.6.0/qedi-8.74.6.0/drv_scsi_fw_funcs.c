...
{ /* no need to check for sgl_task_params->sgl validity */
    ...
    /* sgl params */
    ctx_sgl_params->sgl_addr.lo        = HWAL_CPU_TO_LE32(sgl_task_params->sgl_phys_addr.lo);
    ctx_sgl_params->sgl_addr.hi        = HWAL_CPU_TO_LE32(sgl_task_params->sgl_phys_addr.hi);
    ctx_sgl_params->sgl_total_length = HWAL_CPU_TO_LE32(sgl_task_params->total_buffer_size);
	...
	ctx_data_desc->sge[sge_index].sge_addr.lo = HWAL_CPU_TO_LE32(sgl_task_params->sgl[sge_index].sge_addr.lo);
	ctx_data_desc->sge[sge_index].sge_addr.hi = HWAL_CPU_TO_LE32(sgl_task_params->sgl[sge_index].sge_addr.hi);
