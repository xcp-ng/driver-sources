	...
	struct qedf_rport *fcport)
{
	...
	/* e_d_tov + 1000 ms */
	conn_info.rec_tov_timer_val = qedf->lport->e_d_tov + 1000;
	...
	if (fcport->dev_type == QEDF_RPORT_TYPE_TAPE) {
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_CONN,
		    ...
		    fcport->rdata->ids.port_id);
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_CONN,
		    "flags=0x%x portid=%06x.\n",
		    fcport->rdata->flags, fcport->rdata->ids.port_id);
		conn_info.flags |= 1 <<
		    FCOE_CONN_OFFLOAD_RAMROD_DATA_B_CONF_REQ_SHIFT;
		conn_info.flags |=
		    ((fcport->rdata->flags & FC_RP_FLAGS_RETRY) ? 1 : 0) <<
	...
	*shost)
{
	...
	qed_ops->common->get_vport_stats(qedf->cdev, eth_stats, true);
...
void qedf_process_cqe(struct qedf_ctx *qedf, struct fcoe_cqe *cqe)
{
	...
	case FCOE_DUMMY_CQE_TYPE:
		...
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO,
		    "Dummy CQE.\n");
		if((io_req->cmd_type == QEDF_SEQ_CLEANUP) && (fcport->dev_type == QEDF_RPORT_TYPE_TAPE))
		{
			QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO,
			    "Received completion for SEQ cleanup task.\n");
			qedf_process_seq_cleanup_compl(qedf, cqe, io_req);
			/* Revert the cmd type back to get the completion */
			io_req->cmd_type = QEDF_SCSI_CMD;
		}
		break;
