...
static int qedf_initiate_srr(struct qedf_rport *fcport, unsigned int op,
	...
	struct qedf_els_cb_arg *cb_arg, uint32_t timer_msec)
{
	...
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_ELS, "Sending SRR\n");
	...
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_ELS, "initiate_srr srr_req = "
	...
	/* Ring doorbell */
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_ELS, "Ringing doorbell for SRR "
		   "req\n");
...
int qedf_send_rrq(struct qedf_ioreq *aborted_io_req)
{
	...
	/* Remove this code after 12.40 completes. */
	if (!fcport) {
#ifdef KREF_READ
		refcount = kref_read(&aborted_io_req->refcount);
#else
		refcount = atomic_read(&aborted_io_req->refcount.refcount);
#endif
...
static void qedf_srr_compl(struct qedf_els_cb_arg *cb_arg)
{
	...
	case ELS_LS_RJT:
		...
		qedf_initiate_cleanup(orig_io_req, false);
		break;
	}
...
static int qedf_send_srr(struct qedf_ioreq *orig_io_req, u32 offset, u8 r_ctl)
{
	...
	rc = qedf_initiate_srr(fcport, ELS_SRR, &srr, sizeof(srr),
...
void qedf_initiate_seq_cleanup(struct qedf_ioreq *orig_io_req,
	u32 offset, u8 r_ctl)
{
	struct qedf_rport *fcport;
	unsigned long flags;
	...
	struct fcoe_cqe *cqe, struct qedf_ioreq *io_req)
{
	...
	/* Set the sequence cleanup, since we get a cleanup */
...
static void qedf_rec_compl(struct qedf_els_cb_arg *cb_arg)
{
	...
	if (rec_req->event == QEDF_IOREQ_EV_ELS_TMO) {
		QEDF_ERR(&(qedf->dbg_ctx),
		    ...
		    orig_io_req, orig_io_req->xid);
		/* REC should be retried only once */
		if (atomic_read(&orig_io_req->state) == QEDFC_CMD_ST_REC_ACTIVE) {
			if (orig_io_req->retry_rec) {
				qedf_send_rec(orig_io_req);
				orig_io_req->retry_rec = 0;
			}
		}
		goto out_put;
	}
...
out_put:
	...
	kref_put(&orig_io_req->refcount, qedf_release_cmd);
	kref_put(&rec_req->refcount, qedf_release_cmd);
out_free:
	kfree(cb_arg);
}
...
int qedf_send_rec(struct qedf_ioreq *orig_io_req)
{
	...
	/* Set state to REC ACTIVE, to check on timeout */
	atomic_set(&orig_io_req->state, QEDFC_CMD_ST_REC_ACTIVE);
	cb_arg->aborted_io_req = orig_io_req;
	...
	rc = qedf_initiate_els(fcport, ELS_REC, &rec, sizeof(rec),
	    qedf_rec_compl, cb_arg, 2 * r_a_tov);
