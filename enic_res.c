...
#define GET_CONFIG(m) \
	...
	if (enic_is_sriov_pf(enic)) {
		...
		if (c->mq_subvnic_count) {
			netdev_warn(enic->netdev, "Misconfig detected: SRIOV and VMMQ can not be configured at the same time. Please disable VMMQ in order to be able to use SRIOV\n");
			enic_sriov_deinit(enic);
			enic_sriov_misconfig_detected(enic);
		}
	}
...
{
	enic->wq_count[ENIC_DATA_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_WQ);
	enic->rq_count[ENIC_DATA_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_RQ);
	enic->cq_count[ENIC_DATA_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_CQ);
	...
	dev_info(enic_get_dev(enic),
		...
		enic->wq_count[ENIC_DATA_QP], enic->rq_count[ENIC_DATA_QP],
		enic->cq_count[ENIC_DATA_QP], enic->total_intr_count);

	enic->wq_count[ENIC_ADMIN_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_ADMIN_WQ);
	enic->rq_count[ENIC_ADMIN_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_ADMIN_RQ);
	enic->cq_count[ENIC_ADMIN_QP] = vnic_dev_get_res_count(enic->vdev,
							      RES_TYPE_ADMIN_CQ);

	dev_info(enic_get_dev(enic),
		"vNIC admin resources avail: wq %d rq %d cq %d\n",
		enic->wq_count[ENIC_ADMIN_QP], enic->rq_count[ENIC_ADMIN_QP],
		enic->cq_count[ENIC_ADMIN_QP]);
}
...
void enic_print_resources(struct enic *enic)
{
	...
	netdev_info(enic->netdev, "vNIC resources used: wq %d rq %d cq %d qp %d intr %d rq_desc %d wq_desc %d intr mode %s\n",
		    enic->wq_count[ENIC_DATA_QP], enic->rq_count[ENIC_DATA_QP],
		    enic->cq_count[ENIC_DATA_QP], enic->qp_count,
		    enic->intr_count, enic->config.rq_desc_count,
		    enic->config.wq_desc_count, intr_mode);
}

int enic_has_admin_res(struct enic *enic)
{
	int err = 0;

	if (vnic_has_admin_res(enic->vdev) &&
	    enic_sriov_requirements_ok(enic, false,
				       &err)) /* Ignoring err */
		return 1;
...
static const char vnic_res_type_str[RES_TYPE_MAX][32] = {
	"EOL",
	"WQ",
	"RQ",
	"CQ",
	...
	"RDMA_CQ",
	"RDMA_RKEY_TABLE",
	"RDMA_RQ_HEADER_TABLE",
	"RDMA_RQ_TABLE",
	"RDMA_RD_RESP_HEADER_TABLE",
	"RDMA_RD_RESP_TABLE",
	"RDMA_QP_STATS_TABLE",
	"WQ_MREGS",
	"GRPMBR_INTR",
...
static const char *vnic_res_type_str_unknown = "Unknown";

const char *enic_get_vnic_res_name(enum vnic_res_type res_type)
{
	if (res_type < RES_TYPE_MAX)
		return vnic_res_type_str[res_type];
	else
		return vnic_res_type_str_unknown;
}
