	...
	struct vnic_dev_bar *bar)
{
	...
	printk("vnic dev resources : RES_TYPE_WQ :%d RES_TYPE_RQ: %d RES_TYPE_CQ: %d RES_TYPE_INTR_CTRL:%d\n", 
			vdev->res[RES_TYPE_WQ].count, vdev->res[RES_TYPE_RQ].count, vdev->res[RES_TYPE_CQ].count, vdev->res[RES_TYPE_INTR_CTRL].count);
	return 0;
}
...
int vnic_dev_cmd1(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd,
	int wait)
{
	...
	int wait)
{
	...
	if (posted == 0xFFFFFFFF || fetch_index == 0xFFFFFFFF) { /* check for hardware gone  */
		...
		printk(KERN_ERR "%s: wq is full while issuing devcmd2 command %d, "
	...
	if (new_posted == fetch_index) {
		printk(KERN_ERR "wq is full while issuing devcmd2 command %d, "
			"fetch index: %u, posted index: %u\n",
		...
		if (result->color == color) {
			if (result->error) {
				...
				if (err != ERR_ECMDUNKNOWN || cmd != CMD_CAPABILITY)
					printk(KERN_ERR "%s:Error %d devcmd %d\n",
						pci_name(vdev->pdev),
						err, _CMD_N(cmd));
				return err;
			}
	...
	printk(KERN_ERR "%s:Timed out devcmd %d\n", pci_name(vdev->pdev), _CMD_N(cmd));
...
int vnic_dev_init_devcmd2(struct vnic_dev *vdev)
{
	...
	if (fetch_index == 0xFFFFFFFF) { /* check for hardware gone  */
		printk(KERN_ERR "Fatal error in devcmd2 init - hardware surprise removal");
		return -ENODEV;
	}
	...
	struct vnic_devcmd_fw_info **fw_info)
{
	...
	int wait = 1000;
	int err = 0;
	u64 a1 = sizeof(struct vnic_devcmd_fw_info);
...
int vnic_dev_cmd_init(struct vnic_dev *vdev)
{
	...
	if (p) {
		printk(KERN_ERR "fnic: DEVCMD2 resource found!\n");
		err = vnic_dev_init_devcmd2(vdev);
	}
	else {
		printk(KERN_ERR "fnic: DEVCMD2 resource not found, fall back to devcmd\n");
