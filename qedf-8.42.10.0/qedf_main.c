...
static int qedf_reset_fipvlan_retries(struct qedf_ctx *qedf)
{
...
static void qedf_link_update(void *dev, struct qed_link_output *link)
{
	...
	if (link->link_up) {
		...
		/* Check to see if DCBX already converged */
		dcbx_mode = qedf_dcbx_get_mode(qedf);
		if (dcbx_mode > 0) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "Current DCBx mode is 0x%x.\n", dcbx_mode);
...
static void qedf_dcbx_handler(void *dev, struct qed_dcbx_get *get, u32 mib_type)
{
	...
	if (get->operational.enabled && get->operational.valid) {
		...
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_INFO, "Mode = 0x%x.\n",
...
static int qedf_eh_abort(struct scsi_cmnd *sc_cmd)
{
	...
	if (!io_req) {
		QEDF_ERR(&(qedf->dbg_ctx), "sc_cmd not queued with lld, sc_cmd=%p,"
	...
	if (!rval || io_req->sc_cmd != sc_cmd) {
		QEDF_ERR(&(qedf->dbg_ctx), "Freed/Incorrect io_req, io_req->sc_cmd=%p,"
			" sc_cmd=%p, port_id=%06x, bailing out.\n",
...
#endif
		QEDF_ERR(&(qedf->dbg_ctx), "rport not ready, io_req=%p, xid=0x%x sc_cmd=%p "
	...
	QEDF_ERR(&(qedf->dbg_ctx), "Aborting io_req=%p sc_cmd=%p xid=0x%x "
...
void qedf_schedule_hw_err_handler(void *dev, enum qed_hw_err_type err_type)
{
	...
	case QED_HW_ERR_FW_ASSERT:
		...
		schedule_delayed_work(&qedf->grcdump_work, 0);
		break;
	default:
		break;
	}
