...
static void qedf_cmd_timeout(struct work_struct *work)
{
	...
	case QEDF_ELS:
		...
		/*
		 * FIXME? Port becomes unrepsonsive if we try to cleanup
		 * a timed out RRQ when simulating an ELS timeout
		 */
		if (op != ELS_RRQ)
			qedf_initiate_cleanup(io_req, true);
...
void qedf_release_cmd(struct kref *ref)
{
	...
	if (io_req->cmd_type == QEDF_SCSI_CMD) {
		QEDF_WARN(&(fcport->qedf->dbg_ctx),
		    "Cmd released called without scsi_done called, io_req %p "
		    "xid=0x%x.\n", io_req, io_req->xid);
	...
	struct qedf_ioreq *io_req)
{
	...
	if (!test_bit(QEDF_CMD_OUTSTANDING, &io_req->flags) ||
		...
		QEDF_ERR(&(qedf->dbg_ctx), "io_req xid=0x%x sc_cmd=%p already "
	...
	int result)
{
	...
	if (!virt_addr_valid(sc_cmd->device)) {
		QEDF_ERR(&qedf->dbg_ctx, "Device pointer for sc_cmd %p is bad.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}

	if (!sc_cmd->sense_buffer) {
		QEDF_ERR(&qedf->dbg_ctx, "sc_cmd->sense_buffer for sc_cmd %p is NULL.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}

	if (!virt_addr_valid(sc_cmd->sense_buffer)) {
		QEDF_ERR(&qedf->dbg_ctx, "sc_cmd->sense_buffer for sc_cmd %p is bad.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}

	if (!sc_cmd->scsi_done) {
		QEDF_ERR(&qedf->dbg_ctx, "sc_cmd->scsi_done for sc_cmd %p is NULL.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}
...
void qedf_flush_active_ios(struct qedf_rport *fcport, int lun)
{
	...
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO, "Flush active i/o's num=0x%x "
	    "fcport=0x%p port_id=0x%06x scsi_id=%d.\n", atomic_read(&fcport->num_active_ios),
...
//			(io_req != QEDF_ABTS)) {
		...
		if (qedf->dbg_null_device) {
			...
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO,
			    "Simulating null sc_cmd->device pointer xid=0x%xi sc_cmd=%p.\n",
		...
		if (fcport->rport->scsi_target_id != io_req->sc_cmd->device->id) {
			QEDF_ERR(&qedf->dbg_ctx,
			    "Crashing !!! (scsi_targe_id)0x%x != 0x%x(device_id), fcport=%p io_req=%p "
			...
			if (wait_cnt == 0) {
				...
				for (i = 0; i < FCOE_PARAMS_NUM_TASKS; i++) {
					...
					if (io_req->fcport != NULL &&
						...
						QEDF_ERR(&qedf->dbg_ctx,
							"Outstanding io_req =%p xid=0x%x "
							"flags=0x%lx, sc_cmd=%p refcount=%d "
							"cmd_type=%d.\n",
							io_req, io_req->xid,
							io_req->flags,
...
int qedf_initiate_tmf(struct scsi_cmnd *sc_cmd, u8 tm_flags)
{
	...
	struct fc_rport_priv *rdata = fcport->rdata;
        ...
        QEDF_ERR(NULL, "qedf_initiate_tmf tm_flags 0x%x sc_cmd %p "
...
#endif
		QEDF_ERR(NULL, "qedf_initiate_tmf orig io_req = %p xid = 0x%x "
