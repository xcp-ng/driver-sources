...
proc nvmcfgT::allowedListToString {id} {
	...
	set bb_index [string first "BB" $aList([lindex $keys $i])]
	set ah_index [string first "AH" $aList([lindex $keys $i])]
	
	if {($bb_index != -1) && ($ah_index == -1)} {
		#Option only for BB		
		if {$chip_num != "57980"} {
			#If chip isn't 57980, don't show BB options.
			...
			#If chip is 57980, remove BB prefix from string.
			set aList([lindex $keys $i]) [string replace $aList([lindex $keys $i]) $bb_index [expr {$bb_index+2}] ""]			
		}
		
	} elseif {($bb_index == -1) && ($ah_index != -1)} {
		#Option only for AH
		if {$chip_num != "57940"} {			
			#If chip isn't 57940, don't show AH options.
			#If this is the last option value, chop the last comma
			if {[expr {$i+1}]==[llength $keys]} {
				set ans [string trimright $ans ", "]
			}
			...
			#If chip is 57940, remove AH prefix from string
			set aList([lindex $keys $i]) [string replace $aList([lindex $keys $i]) $ah_index [expr {$ah_index+2}] ""]			
		}
	} elseif {($bb_index != -1) && ($ah_index != -1)} {
		#Remove BB_AH prefix
		set aList([lindex $keys $i]) [string replace $aList([lindex $keys $i]) $bb_index [expr {$bb_index+2}] ""]	
        ...
        if {[string equal $nvm_cfg($id,listType) "bitfield"]} {
            append ans [format "%s(0x%x)" $aList([lindex $keys $i]) [lindex $keys $i]]
        } else {
            append ans [format "%s($base)" $aList([lindex $keys $i]) [lindex $keys $i]]
        }
        if {[expr {$i+1}]!=[llength $keys]} {
            append ans ", "
        }
        ...
        if {[info exist nvm_cfg($id,allowedList)] && [string equal $nvm_cfg($id,listType) "enum"] && $verbose} {
            ...
            for {set i 0} {$i<[llength $formats]} {incr i} {
                if {[info exist $nvm_cfg($id,allowedList)([lindex $values $i])]} {
                    append formatVal [subst $$nvm_cfg($id,allowedList)([lindex $values $i])]
                } else {
...
proc nvmcfgT::set_driversim_mode {} {
	set chip_num [getChipNum]
       if {$chip_num == 57980} {
       	...
       	nvm cfg -all 9=0 15=0 23=1 59=0 105=1 128=3 92=0 83=5 118=0x29
	} elseif {$chip_num == 57940} {
		puts "Applying Driversim mode for AH"
		nvm cfg -all 9=0 15=0 23=1 59=0 105=1 128=3 92=0 191=5 118=0x70 
	} elseif {$chip_num == 51000} {
